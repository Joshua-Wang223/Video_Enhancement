...
output            最终合并阶段：编码器、CRF、预设、像素格式、音频策略
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `src/main_video_optimized.py` | ★ **当前主入口** | 统一调度 IFRNet（`ifrnet_processor_v6_1_single`）+ Real-ESRGAN DS（`realesrgan_processor_video_optimized` → `external/realesrgan_video_ds`） |
...
| `realesrgan_processor_video_optimized.py` | ★ **当前** | Real-ESRGAN 视频超分；底层 **`external/realesrgan_video_ds/main.py`**，支持 **`create_video_enhancer` / `run_pipeline_for_video`** 多片段复用 |
...
### `external/realesrgan_video_ds/` — Real-ESRGAN 优化流水线（当前超分后端）

| 文件 | 状态 | 说明 |
|------|------|------|
| `main.py` | ★ **当前后端** | CLI、`main_optimized`、`create_video_enhancer()`、`run_pipeline_for_video(enhancer, …)`（多片段引擎复用）；标注 v6.4 架构 |
...
| `inference_realesrgan_video_v6_3_optimized.py`、`inference_realesrgan_video_v6_4_optimized.py` | ⚠️ 历史/实验 | 优化版单文件推理（与 `realesrgan_video_ds` 并行演进） |
...
### `external/realesrgan_video/`（备选）

模块化实验目录，含独立 `main.py`、`pipeline` 等；**当前推荐主流程不依赖此目录**，与 `realesrgan_video_ds` 区分使用。
...
external/realesrgan_video_ds/*_bak*.py
external/realesrgan_video_ds/main - Copy.py
```
