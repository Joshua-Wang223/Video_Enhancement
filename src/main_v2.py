...
from video_utils import (
    ...
    extract_audio, add_audio_to_video, verify_video_integrity,
    split_video_by_time
)
sys.path.insert(0, "/workspace/Video_Enhancement/src/processors")
from ifrnet_processor_v2 import IFRNetProcessor
from realesrgan_processor_v2 import RealESRGANProcessor
...
class VideoProcessor:
    ...
    def __init__(self, config: Config):
        ...
        self.esrgan = RealESRGANProcessor(config)
    ...
    def process_single_video(self, input_video: str) -> bool:
        ...
        if video_info and video_info.has_audio:
            print(f"\n🎵 提取音频...")
            audio_path = str(self.config.get_temp_dir() / f"{Path(input_video).stem}_audio.aac")
        ...
        if self.mode == "interpolate_then_upscale":
            success = self._process_interpolate_then_upscale_optimized(input_video, audio_path)
        elif self.mode == "upscale_then_interpolate":
            success = self._process_upscale_then_interpolate_optimized(input_video, audio_path)
        else:
    ...
    def _process_interpolate_then_upscale_optimized(self, input_video: str, 
        ...
        # 追踪临时目录
        self.temp_files_tracker['segment_dirs'].append(
            self.ifrnet.segment_dir
        )
        self.temp_files_tracker['checkpoint_files'].append(
            self.ifrnet.checkpoint_file
        )
        ...
        # 追踪临时目录
        self.temp_files_tracker['segment_dirs'].append(
            self.esrgan.processed_dir
        )
        self.temp_files_tracker['checkpoint_files'].append(
            self.esrgan.checkpoint_file
        )
        ...
        from video_utils import merge_videos_by_codec
        
        print(f"🔗 合并 {len(upscaled_segments)} 个处理后的分段...")
        
        # if not merge_videos(upscaled_segments, final_output, audio_path):
        output_config = self.config.get_section("output", {})
        # if not merge_videos(upscaled_segments, final_output, audio_path, config=output_config):
    ...
    def _process_upscale_then_interpolate_optimized(self, input_video: str, 
        ...
        # if not merge_videos(interpolated_segments, final_output, audio_path):
        output_config = self.config.get_section("output", {})
        # if not merge_videos(interpolated_segments, final_output, audio_path, config=output_config):
...
示例:
  python main_v2.py -c config.json -i video.mp4
  python main_v2.py -c config.json --input-dir ./videos --batch
        """
    )
    ...
    parser.add_argument("--input-dir", type=str, help="输入视频目录（批量处理）")
    ...
    parser.add_argument("--interpolation-factor", type=int, choices=[2, 4, 8, 16],
                       help="插帧倍数")
    parser.add_argument("--upscale-factor", type=int, choices=[2, 4], help="超分倍数")
