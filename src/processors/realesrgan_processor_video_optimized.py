...
class RealESRGANVideoProcessor:
    ...
    def __init__(self, config):
        ...
        self.encode_preset      = config.get("models", "realesrgan", "encode_preset",      default="medium")
        ...
        self.force_use_compile      = config.get("models", "realesrgan", "force_use_compile",      default=False)
        self.force_use_cuda_graph   = config.get("models", "realesrgan", "force_use_cuda_graph",   default=False)
    ...
    def process_video(self, input_video: str, output_video: str) -> bool:
        ...
        """
        print("\n" + "=" * 65)
        print("🎨 Real-ESRGAN 视频超分处理（完整流程）—— 优化版")
        ...
        face_hint = (
            ...
            f" det_thresh={self.face_det_threshold},"
            f" gfpgan_trt={self.gfpgan_trt})"
        ...
        if self.adaptive_batch:
            print(f"   自适应批处理: 已启用")
        if self.preview:
            print(f"   实时预览: 启用 (间隔 {self.preview_interval} 帧)")
        if self.report_json:
    ...
    def process_segments_directly(self, input_segments: List[str],
        ...
        for _p in sorted(input_segments):
            try:
                _st = os.stat(_p)
                _fp_parts.append(f"{_p}:{_st.st_size}:{_st.st_mtime_ns}")
            except OSError:
                _fp_parts.append(f"{_p}:missing")
            ...
            if verify_video_integrity(output_path):
                ...
                if not dec_ok:
                    print("   ❌ 解码级验收失败: "
                          ...
                          f"expected={dec_report.get('expected_frames')} "
    ...
    def _save_checkpoint(self, checkpoint: dict):
        ...
        [P1-FIX-ATOMIC] tmp + os.replace 原子写，防 kill 中途留下截断 JSON。
        """
        if self.checkpoint_file:
    ...
    def _run_esrgan_video(self, input_path: str, output_path: str,
        ...
        Namespace 字段与底层 main.py argparse 严格对齐：
          ...
          推理: tile, tile_pad, pre_pad, face_enhance, no_fp16, fps,
                batch_size, prefetch_factor
          face_enhance: gfpgan_model, gfpgan_weight, gfpgan_batch_size,
                        face_det_threshold, adaptive_batch, gfpgan_trt
          加速: no_compile, use_tensorrt, no_cuda_graph
          硬件: use_hwaccel, codec, crf, encode_preset, ffmpeg_bin
          缓存: trt_cache_dir
          预览/报告: preview, preview_interval, report
        ...
        try:
            ...
            # ── 基本路径 ─────────────────────────────────────────────────────
            # main_optimized 直接使用 args.input / args.output
            ns.input  = input_path
            ...
            # TRT Engine 缓存目录；底层 main_optimized 通过 getattr 读取
            ns.trt_cache_dir   = self.trt_cache_dir or None

            # ── 硬件解/编码参数 ────────────────────────────────────────────
            # 注意：底层 argparse 字段名为 codec（与 ifrnet 统一）
            ns.use_hwaccel    = self.use_hwaccel
            ns.codec          = self.codec
            ns.crf            = self.crf
            ns.encode_preset  = self.encode_preset
            ...
            #   1. gfpgan_trt 启用时强制 no_fp16=True 防止 FP16 溢出噪斑
            #   2. no_adaptive_batch 时置 adaptive_batch=False
            # 由于我们直接调用 main_optimized()，需在此处复刻。
            if ns.gfpgan_trt and torch.cuda.is_available():
                ns.no_fp16 = True
                print("   [GFPGAN-TRT] 为防止 FP16 溢出噪斑，已自动禁用半精度并启用 FP32 推理。")

            # adaptive_batch 已在 __init__ 中从 config 解析为最终有效值，
            # 无需再处理 no_adaptive_batch 标志。

            # ── 动态导入 main_optimized ────────────────────────────────────
            # 使用 importlib 按文件路径加载，避免与 Python 内置 'main' 模块名冲突。
            # 底层 main.py 文件名为 'main.py'，直接 import main 会导致
            # 导入 Python 自身的 __main__ 模块或产生 ModuleNotFoundError。
            _main_py = str(self.esrgan_dir / "main.py")
            if not os.path.isfile(_main_py):
                raise FileNotFoundError(
                    f"底层脚本不存在: {_main_py}\n"
            ...
            main_optimized = _mod.main_optimized

            print(f"   🔧 加载模型: {self.model_name}")
            print(f"   🖥️  设备: {self.device} | "
                  f"FP16: {not ns.no_fp16} | "
            ...
            if self.adaptive_batch:
                print(f"   🔄 自适应批处理: 已启用")
            if ns.preview:
                print(f"   👁️  实时预览: 启用 (间隔 {ns.preview_interval} 帧)")
            if ns.report:
...
def main():
    ...
    示例：
      ...
      # 指定配置文件 + 开启人脸增强 + GFPGAN TRT
      python realesrgan_processor_video_optimized.py -c config.json \\
             -i input.mp4 -o output.mp4 --face-enhance --gfpgan-trt

      # 覆盖超分倍数、模型、人脸检测阈值
      python realesrgan_processor_video_optimized.py -i input.mp4 -o output.mp4 \\
             --upscale-factor 4 --model realesr-animevideov3 \\
             --face-enhance --face-det-threshold 0.7

      # TensorRT 加速 + 大批量
      python realesrgan_processor_video_optimized.py -i input.mp4 -o output.mp4 \\
             --use-tensorrt --batch-size 8 --prefetch-factor 48
...
特性：
  ...
  · face_enhance：批量GFPGAN + 人脸检测置信度过滤 + 自适应批处理
  · GFPGAN TensorRT 子进程加速
  · SR H2D 预取重叠
...
""",
    ...
    parser.add_argument("--batch-size", type=int,
                        help="SR 批处理大小（覆盖配置，默认 6）")
    parser.add_argument("--prefetch-factor", type=int,
                        help="读帧预取队列深度（建议 ≥ batch_size×2，覆盖配置，默认 48）")
    ...
    # ── 新增：预览与报告 ─────────────────────────────────────────────────────
    parser.add_argument("--report", dest="report_json", metavar="PATH",
                        help="JSON 性能报告输出路径")
    parser.add_argument("--preview", action="store_true",
                        help="启用实时预览窗口（显示最终输出，按 q 退出）")
    parser.add_argument("--preview-interval", type=int, default=30, metavar="N",
                        help="预览帧间隔（每多少帧刷新一次，默认 30）")
    ...
    if args.encode_preset:
        config.set("models", "realesrgan", "encode_preset",  value=args.encode_preset)
    if args.rate_mode:
    ...
    # 预览与报告
    if args.report_json:
        config.set("models", "realesrgan", "report_json",      value=args.report_json)
    if args.preview:
        config.set("models", "realesrgan", "preview",          value=True)
    if args.preview_interval:
        config.set("models", "realesrgan", "preview_interval", value=args.preview_interval)
    ...
    face_hint = (
        ...
        f" det_thresh={processor.face_det_threshold},"
        f" gfpgan_trt={processor.gfpgan_trt})"
        if face_on else ""
    )
    print(f"   face_enhance: {face_on}{face_hint}")
    if processor.adaptive_batch:
        print(f"   自适应批处理: 已启用")
    print(f"   encode_preset: {processor.encode_preset}")
    if processor.preview:
        print(f"   实时预览: 启用 (间隔 {processor.preview_interval} 帧)")
