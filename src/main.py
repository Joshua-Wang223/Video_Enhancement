...
# sys.path.insert(0, f"{base_dir}/src/processors")
from ifrnet_processor import IFRNetProcessor
from realesrgan_processor import RealESRGANProcessor
...
class VideoProcessor:
    ...
    def process_single_video(self, input_video: str) -> bool:
        ...
        if self.mode == "interpolate_then_upscale":
            success = self._process_interpolate_then_upscale(input_video, audio_path)
        elif self.mode == "upscale_then_interpolate":
            success = self._process_upscale_then_interpolate(input_video, audio_path)
        else:
    ...
    def _process_interpolate_then_upscale(self, input_video: str, 
        ...
        if audio_path and os.path.exists(audio_path):
            print("\n🎵 添加音频到最终视频...")
            if add_audio_to_video(str(upscaled_video), audio_path, output_video):
    ...
    def _process_upscale_then_interpolate(self, input_video: str, 
        ...
        if audio_path and os.path.exists(audio_path):
            print("\n🎵 添加音频到最终视频...")
            if add_audio_to_video(str(interpolated_video), audio_path, output_video):
                print(f"✅ 最终视频: {output_video}")
            else:
                print("⚠️  音频添加失败，使用无音频版本")
                import shutil
...
示例:
  ...
  # 批量处理目录中的所有视频
  python main.py --config config.json --input-dir ./videos --batch
  
  # 使用先超分再插帧模式
  python main.py --config config.json --input video.mp4 --mode upscale_then_interpolate
        """
    )
