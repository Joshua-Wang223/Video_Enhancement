...
================================================================
基于 ifrnet_versions_benchmark.py 升级：

  v2 新增：
    1. 精准帧数检测：参考 video_pipeline_analyzer_v3.py，所有版本处理完成后
  ...
  v3 新增：
    6. 新增 --rate-mode 参数：支持 constqp / vbr_hq / qvbr 三种码率控制模式
       同时对比（透传给 IFRNet 各 version 的 IFRNetVideoProcessor，非 .1
       版本自动屏蔽并按 constqp 执行）。
    7. 综合比较矩阵扩展：FPS 矩阵保留，新增 File_Size 矩阵；支持 Codec ×
       LA × Rate_Mode 多组合横向对比（未涉及的组合项显示 "—"），
       两种矩阵均写入 PDF 报告。
...
用法:
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --scale 4 --model IFRNet_L_Vimeo90K
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --versions v6.4.5.1 --no-warmup
"""
...
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # 脚本在 Accessory/<分类>/ 下，需回退两层到项目根
...
# 支持 NVENC lookahead（LA）的版本：仅 .1 backport 版本（v6.4.3.1/4.1/5.1）
# 为 VBR_HQ/QVBR + LA 实现；其余版本（v6.4.3/4/5 CONSTQP-only，v6.4.2/1/3.5
# 无 lookahead_depth 构造参数）在硬件层面禁用 LA，须按 la=0 执行。
_LA_CAPABLE_VERSIONS = {
    "process_video_v6_4_3_1_single",
    "process_video_v6_4_4_1_single",
    "process_video_v6_4_5_1_single",
}
...
def version_supports_la(version_module: str) -> bool:
    """判断版本是否支持 lookahead（LA）。

    仅 .1 backport 版本支持 VBR_HQ/QVBR + LA；非 .1 版本为 CONSTQP-only
    （或纯 FFmpeg），LA 被硬件静默禁用，须按 la=0 执行。
    """
    return version_module in _LA_CAPABLE_VERSIONS


# 支持完整 rate_mode（constqp/vbr_hq/qvbr）的版本：与 LA 能力一致（.1 backport）。
# v6.4.3/4/5 虽构造有 rate_mode 参数但实例级硬编码 constqp（[CONSTQP-ONLY]）；
# v6.3.5/4.1/4.2 为纯 FFmpeg / Level2 路径，无 rate_mode 概念。
_RM_CAPABLE_VERSIONS = _LA_CAPABLE_VERSIONS


def version_supports_rate_mode(version_module: str) -> bool:
    """判断版本是否支持完整 rate_mode（constqp/vbr_hq/qvbr）。

    仅 .1 backport 版本支持；非 .1 版本自动屏蔽并强制 constqp 执行。
    """
    return version_module in _RM_CAPABLE_VERSIONS


def _compose_display_label(version_label: str, codec, la, rm,
                           multi_codec: bool, multi_la: bool,
                           multi_rm: bool) -> str:
    """组合主表显示标签：多编码/多 la/多 rate_mode 时追加后缀以区分行。"""
    if not (multi_codec or multi_la or multi_rm):
        return version_label
    parts = [version_label]
    if multi_codec:
        parts.append(codec if codec else "auto")
    if multi_rm:
        parts.append(rm if rm else "default")
    if multi_la:
        parts.append(f"la{la}" if la is not None else "la-default")
    return "/".join(parts)


# ═══════════════════════════════════════════════
# 0. GPU 基础设施与错误关键词（移植自 video_pipeline_analyzer_v3.py）
# ═══════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# 系统资源探测（移植自 video_pipeline_analyzer_v3.py）
# ═══════════════════════════════════════════════════════════════════════════
...
def _print_task_progress(done: int, total: int, task: dict, result,
                         task_duration: float, name_w: int = 0, idx_w: int = 0):
    """逐任务进度打印（列对齐：序号 [n/total] 与文件名均按最大显示宽度补齐）。"""
    name = Path(task['path']).name
    _iw = idx_w if idx_w else len(str(total))
    _idx = f"[{done:>{_iw}}/{total}]"
    _name = _pad(name, name_w, '<') if name_w else name
    if task['kind'] == 'ffprobe':
        print(f"  {_idx} ffprobe/{task['mode']} {_name}: {result}  (用时 {task_duration:.1f}s)")
    elif isinstance(result, dict):
        print(f"  {_idx} ffmpeg {_name}: frame={result.get('frame_count')}, "
              f"dup={result.get('dup_count')}, drop={result.get('drop_count')}, "
              f"errors={len(result.get('errors', []))}  (用时 {task_duration:.1f}s)")
    else:
        print(f"  {_idx} ffmpeg {_name}: {result}  (用时 {task_duration:.1f}s)")
...
def run_tasks_parallel(tasks: list, workers: int, phase_label: str = '') -> dict:
    ...
    # 预计算列宽：序号位(按 total 位数) 与 文件名最大显示宽度，保证逐行对齐
    _idx_w = len(str(total))
    _max_name_w = max((_display_width(Path(t['path']).name) for t in tasks), default=0)
    ...
    if workers == 1:
        for i, t in enumerate(tasks, 1):
            ...
            _print_task_progress(i, total, t, results[t['key']],
                                 time.time() - t0, _max_name_w, _idx_w)
        return results
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(fut2task):
            ...
            _print_task_progress(done, total, t, results[t['key']],
                                 task_dur, _max_name_w, _idx_w)
    return results
...
def run_version(version_module: str, input_path: str, output_path: str,
                ...
                la: int | None = None,
                rate_mode: str | None = None) -> dict:
    """Run a single IFRNet version in a subprocess and return timing results.

    la: lookahead 深度（None=使用版本默认值）。仅 .1 版本实际生效；
    非 .1 版本由调用方强制传 la=0（或 None），此处按构造参数能力条件注入。
    rate_mode: 码率控制模式（None=使用版本默认值）。仅能力版本（.1）支持
    constqp/vbr_hq/qvbr；非 .1 版本由调用方强制传 constqp，此处按构造参数
    能力（v6.4.5.1）或实例属性兜底（v6.4.3.1/4.1）注入。
    """
    inner_script = f'''
import importlib
...
# lookahead_depth 条件注入：旧版本（v6.3.5/4.1/4.2）无此构造参数，
# 且仅 .1 版本（v6.4.3.1/4.1/5.1）实际支持 LA；非 .1 版本由外部强制 la=0。
_la_requested = {la!r}
if _la_requested is not None and 'lookahead_depth' in _inspect.signature(
        mod.IFRNetVideoProcessor.__init__).parameters:
    _proc_kwargs['lookahead_depth'] = _la_requested

# rate_mode 条件注入：仅能力版本（.1 backport）支持 constqp/vbr_hq/qvbr；
# 非 .1 版本由外部强制 constqp。优先构造参数（v6.4.5.1）；
# v6.4.3.1/4.1 无构造参数，决策层 getattr(self,'_rate_mode',模块常量) 兜底
# → 实例化后直接设置实例属性（与 lookahead 处理方式一致，仅对能力版本生效）。
_rm_requested = {rate_mode!r}
_rm_kw_supported = ('rate_mode' in _inspect.signature(
    mod.IFRNetVideoProcessor.__init__).parameters)
if _rm_requested is not None and _rm_kw_supported:
    _proc_kwargs['rate_mode'] = _rm_requested

proc = mod.IFRNetVideoProcessor(**_proc_kwargs)

if _rm_requested is not None and not _rm_kw_supported and {version_supports_rate_mode(version_module)}:
    proc._rate_mode = _rm_requested
...
result = {{
    ...
    "la": getattr(proc, '_la_depth', {la!r}),
}}
...
def _matrix_grid(results: list, codec_list: list, la_list: list, rm_list: list,
                 lang: str = 'zh', metric: str = 'fps'):
    """构建综合比较矩阵原始数据（供控制台打印与 PDF 报告共用）。

    行 = 版本（label），列 = codec/rate_mode/la 组合，单元格 = metric 值。
    - metric='fps'  : 单元格为 FPS（优先精准解码帧数计算值）
    - metric='size' : 单元格为输出文件大小 (MB)
    - constqp 模式硬件静默禁用 LA（NVENCEncoder 强制 la_depth=0），其列 la
      恒为 0（"constqp 对应 /la0"）。
    - 未涉及的测试组合项以 "—" 显示。

    返回 dict（含 versions/col_specs/col_labels/rows）或 None（数据不足/
    无多维度时返回 None）。
    """
    lang = _normalize_lang(lang)
    ...
    if len(ok) < 2:
        return None
    multi_codec = len(codec_list) > 1
    multi_la = len(la_list) > 1
    multi_rm = len(rm_list) > 1
    if not (multi_codec or multi_la or multi_rm):
        return None

    # 列定义（保持请求顺序；constqp 列 la 折叠为 0 并去重）
    col_specs = []
    for c in codec_list:
        for rm in rm_list:
            for l in la_list:
                if rm == "constqp" and l not in (0, None):
                    l = 0
                spec = (c, rm, l)
                if spec not in col_specs:
                    col_specs.append(spec)

    # 列标签：精炼短名，避免长列名超出显示窗口折返。
    #   codec 去 "_nvenc"；rate_mode 缩写(constqp→c, qvbr→q, vbr_hq→v)；
    #   constqp 硬件静默禁用 LA，其列省略 la 维度。
    #   示例：h264_nvenc/constqp/la0→h264/c，hevc_nvenc/qvbr/8→hevc/q/la8
    _CODEC_SHORT = {"h264_nvenc": "h264", "hevc_nvenc": "hevc", "av1_nvenc": "av1"}
    _RM_SHORT = {"constqp": "c", "qvbr": "q", "vbr_hq": "v", "vbr": "v", "cq": "cq"}

    def _short_col_label(c, rm, l):
        parts = []
        if multi_codec:
            parts.append(_CODEC_SHORT.get(c, c or "auto"))
        if multi_rm:
            parts.append(_RM_SHORT.get(rm, rm or "default"))
        if multi_la and rm != "constqp":   # constqp 列省略 la 维度
            parts.append(f"la{l}" if l is not None else "la0")
        return "/".join(parts)

    col_labels = [_short_col_label(c, rm, l) for c, rm, l in col_specs]

    # 版本行顺序（按结果出现顺序去重）
    versions = []
    for r in ok:
        if r.get("label") not in versions:
            versions.append(r.get("label"))

    def _match(r, c, rm, l):
        return (r.get("codec_requested") == c and
                r.get("rm_requested") == rm and
                r.get("la_requested") == l)

    def _cell(version, c, rm, l):
        for r in ok:
            if r.get("label") == version and _match(r, c, rm, l):
                if metric == 'size':
                    _v = r.get("output_size_mb")
                    if isinstance(_v, (int, float)):
                        return f"{_v:.1f}"
                else:  # fps
                    _v = r.get("fps_precise") or r.get("fps")
                    if isinstance(_v, (int, float)):
                        return f"{_v:.1f}"
                return "—"
        return "—"
    ...
    rows = {}
    for v in versions:
        rows[v] = [_cell(v, c, rm, l) for c, rm, l in col_specs]
    ...
    return {
        ...
        'rows': rows,
        'multi_codec': multi_codec,
        'multi_la': multi_la,
        'multi_rm': multi_rm,
    }


def print_comparison_matrix(results: list, codec_list: list, la_list: list,
                            rm_list: list, lang: str = 'zh', metric: str = 'fps'):
    """输出综合比较矩阵（多编码 / 多 la / 多 rate_mode 组合时触发）。

    行 = 版本（label），列 = codec/rate_mode/la 组合，单元格 = metric 值
    （metric='fps' → FPS；metric='size' → 输出文件大小 MB）。
    constqp 模式硬件静默禁用 LA，其列 la 恒为 0（"constqp 对应 /la0"）；
    未涉及的测试组合项以 "—" 显示。
    """
    grid = _matrix_grid(results, codec_list, la_list, rm_list, lang, metric=metric)
    if grid is None:
        return
    versions = grid['versions']
    col_labels = grid['col_labels']
    rows = grid['rows']
    metric_title = _t('fps', lang) if metric == 'fps' else _t('file_size', lang)

    vw = max(10, max(_display_width(v) for v in versions) + 2)
    cw = max(9, max(_display_width(cl) for cl in col_labels) + 2)
    header = "  " + _pad(_t('version', lang), vw, '<') + "  " + "  ".join(
        _pad(cl, cw, '>') for cl in col_labels)

    print()
    print("  ── " + _t('comparison_matrix', lang) + f" ({metric_title}) ──")
    print(header)
    print("  " + "-" * (vw + 2 + cw * len(col_labels) + 2 * (len(col_labels) - 1)))
    for v in versions:
        cells = rows[v]
        row = "  " + _pad(v, vw, '<') + "  " + "  ".join(_pad(cell, cw, '>') for cell in cells)
        print(row)
    print()
...
def print_results(results: list[dict], input_path: str, scale: float,
    ...
    # 列对齐方向：标签列(版本/encoder)左对齐，其余数值列右对齐
    aligns = ['<', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '>', '<']

    # ── 预扫描：先构造全部数据行，再按"表头与各单元格最大显示宽度"计算列宽，
    #    保证任意长度的组合标签(如 v6.4.3.1/h264_nvenc/vbr_hq/la16)也不会溢流错位 ──
    _ok_rows = []      # 成功行: (label, cols)
    _failed = []       # 失败行: (label, err)
    best_fps = 0.0
    best_label = ""

    for r in results:
        label = r.get("display_label") or r.get("label", "?")
        if not r.get("success"):
            _failed.append((label, r.get("error", "unknown")))
            continue
        ...
        _ok_rows.append((label, cols))

        if isinstance(fps, (int, float)) and fps > best_fps:
            best_fps = fps
            best_label = label

    # 列宽 = max(表头显示宽度, 各数据单元格最大显示宽度)；首列额外纳入失败标签宽度
    _all_labels = [lab for lab, _ in _ok_rows] + [lab for lab, _ in _failed]
    W = [max(_display_width(H[0]), max((_display_width(lab) for lab in _all_labels), default=0))]
    for i in range(1, len(H)):
        W.append(max(_display_width(H[i]),
                     max((_display_width(row[i]) for _, row in _ok_rows), default=0)))
    ...
    # 编码器一致性检查（仅当非显式多编码对比时才提示）
    codecs = set(r.get("codec", "") for r in ok_results)
    req_codecs = set(r.get("codec_requested") for r in ok_results)
    if len(codecs) > 1 and len(req_codecs) <= 1:
    ...
    if _verify_results:
        ...
        # 列宽 = max(表头宽, 各单元格最大显示宽)，避免长组合标签/状态串溢出错位
        _vW = [max(_display_width(_vH[i]),
                   max((_display_width(row[i]) for row in _vrows), default=0))
               for i in range(len(_vH))]
    ...
    if _diag_versions:
        ...
        for r in _diag_versions:
            _label = r.get("display_label") or r.get("label", "?")
            ...
            _prows.append([_label, str(_rp), str(_ib), str(_ip), str(_wf),
                           str(_pipe_expected), f"{_pipe_delta:+d}", _total_str, _out_str, _verify])

        # 列宽 = max(表头宽, 各单元格最大显示宽)
        _pW = [max(_display_width(_pH[i]),
                   max((_display_width(row[i]) for row in _prows), default=0))
               for i in range(len(_pH))]
        _dh = "  " + "  ".join(_pad(h, w, a) for h, w, a in zip(_pH, _pW, _pA))
        print(_dh)
        _sep_len = sum(_pW) + 2 * (len(_pW) - 1)
        print("  " + "-" * _sep_len)
        for _dcols in _prows:
        ...
        if _gpu_stay:
            ...
            for r in _gpu_stay:
                ...
                _nrows.append([_label, str(_gs), str(_nv), str(_eh),
                               f"{_pipe_delta2:+d}", str(_lf), _note_str])

            # 列宽 = max(表头宽, 各单元格最大显示宽)（末列说明串可超宽，不截断不影响前序列对齐）
            _nW = [max(_display_width(_nH[i]),
                       max((_display_width(row[i]) for row in _nrows), default=0))
                   for i in range(len(_nH))]
            _dh2 = "  " + "  ".join(_pad(h, w, a) for h, w, a in zip(_nH, _nW, _nA))
            print(_dh2)
            _sep2_len = sum(_nW) + 2 * (len(_nW) - 1)
            print("  " + "-" * _sep2_len)
            for _ncols in _nrows:
    ...
    if _level_versions:
        ...
        _lvH = [_t('version', lang), _t('level', lang), _t('description', lang)]
        _lvA = ['<', '>', '<']
        _lvrows = []
        for r in _level_versions:
            ...
            _lvrows.append([_label, str(_lv), _desc])
        # 列宽 = max(表头宽, 各单元格最大显示宽)，避免长组合标签错位
        _lvW = [max(_display_width(_lvH[i]),
                    max((_display_width(row[i]) for row in _lvrows), default=0))
                for i in range(len(_lvH))]
        _dh_lv = "  " + "  ".join(_pad(h, w, a) for h, w, a in zip(_lvH, _lvW, _lvA))
        print(_dh_lv)
        print("  " + "-" * (sum(_lvW) + 2 * (len(_lvW) - 1)))
        for cols in _lvrows:
            print("  " + "  ".join(_pad(c, w, a) for c, w, a in zip(cols, _lvW, _lvA)))
...
def generate_pdf_report(results: list[dict], input_path: str, output_path: str,
                        ...
                        codec_list: list = None, la_list: list = None,
                        rm_list: list = None):
    """生成 PDF 基准测试报告。

    codec_list / la_list / rm_list: 测试组合维度（多编码/多 la/多 rate_mode 时
    在报告中追加 FPS 与 File_Size 综合比较矩阵）。
    """
    lang = _normalize_lang(lang)
    if not HAS_REPORTLAB:
    ...
    for r in ok_results:
        ...
        fps = round(fps_base / elapsed, 1) if elapsed and fps_base else r.get('fps', 0)
        main_data.append([
            r.get('display_label') or r.get('label', '?'),
    ...
    if _level_versions:
        ...
        for r in _level_versions:
            ...
            if _rm:
                _desc += f" (rc={_rm})"
            l_data.append([r.get('display_label') or r.get('label', '?'), str(_lv), _desc])
    ...
    if len(ok_results) >= 1:
        ...
        labels = [r.get('display_label') or r.get('label', '?') for r in ok_results]
    ...
    # ── Comparison Matrices (FPS + File_Size) ──
    # 多编码 / 多 la / 多 rate_mode 组合时输出综合比较矩阵
    # constqp 对应 /la0（硬件静默禁用 LA）；未涉及的组合项显示 "—"
    _mg = _matrix_grid(results, codec_list or [None], la_list or [None],
                       rm_list or [None], lang, metric='fps')
    if _mg:
        story.append(Paragraph(
            f"{_t('comparison_matrix', lang)} (FPS)", heading_style))
        _mdata = [[_t('version', lang)] + _mg['col_labels']]
        for _v in _mg['versions']:
            _mdata.append([_v] + _mg['rows'][_v])
        _ncols = max(1, len(_mg['col_labels']))
        _mcw = [2.4 * cm] + [max(1.0 * cm, (17.0 - 2.4) / _ncols * cm)] * _ncols
        story.append(_build_table(_mdata, _mcw, font_name, first_col_header=True))
        story.append(Spacer(1, 0.5 * cm))

        _mg2 = _matrix_grid(results, codec_list or [None], la_list or [None],
                            rm_list or [None], lang, metric='size')
        if _mg2:
            story.append(Paragraph(
                f"{_t('comparison_matrix', lang)} ({_t('file_size', lang)}, MB)",
                heading_style))
            _mdata2 = [[_t('version', lang)] + _mg2['col_labels']]
            for _v in _mg2['versions']:
                _mdata2.append([_v] + _mg2['rows'][_v])
            story.append(_build_table(_mdata2, _mcw, font_name, first_col_header=True))
            story.append(Spacer(1, 0.5 * cm))

    # ── Conclusion ──
    story.append(Paragraph(_t('sec_conclusion', lang), heading_style))
    fps_values = [(r.get('fps_precise', r.get('fps', 0)) or 0, r.get('display_label') or r.get('label', '?')) for r in ok_results]
    if fps_values:
...
示例:
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --scale 4
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --versions v6.4.5.1 --no-warmup
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --codec libx264 h264_nvenc
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --versions v6.4.5.1 --la 0 8
    python ifrnet_versions_benchmark_v2.py -i test.mp4 -o benchmark_output/ --rate-mode constqp vbr_hq qvbr
""",
    )
    ...
    parser.add_argument("--warmup-duration", type=int, default=10,
                        help="预热视频时长（秒，默认 10）")
    parser.add_argument("--codec", nargs="+", default=None,
                        help="编码器，可指定多个进行对比（默认: None=自动选择最佳编码器）。"
                             "如 --codec libx264 h264_nvenc；单个编码 --codec libx264")
    parser.add_argument("--la", nargs="+", type=int, default=None,
                        help="NVENC lookahead 深度，可指定多个进行对比（默认: None=使用各版本默认值）。"
                             "如 --la 0 8。仅 .1 版本（v6.4.3.1/4.1/5.1）支持，非 .1 版本自动屏蔽并按 la=0 执行")
    parser.add_argument("--rate-mode", nargs="+", default=None,
                        choices=["constqp", "vbr_hq", "qvbr"],
                        help="NVENC 码率控制模式，可指定多个进行对比（默认: None=使用各版本内置默认）。"
                             "如 --rate-mode constqp vbr_hq qvbr。仅 .1 版本（v6.4.3.1/4.1/5.1）完整支持三种模式，"
                             "非 .1 版本自动屏蔽并按 constqp 执行")
    ...
    # --codec 合并原 --codecs：支持一次指定多个编码器（默认 None=自动选择）
    codec_list = args.codec if args.codec else [None]
    # --la 支持一次指定多个 lookahead 深度（默认 None=使用各版本默认值）
    la_list = args.la if args.la else [None]
    # --rate-mode 支持一次指定多个码率控制模式（默认 None=使用各版本内置默认）
    rate_mode_list = args.rate_mode if args.rate_mode else [None]
    ...
    _codec_desc = ", ".join(c for c in codec_list if c) or "auto"
    _la_desc = ", ".join(str(l) for l in la_list if l is not None) or "版本默认"
    _rm_desc = ", ".join(rm for rm in rate_mode_list if rm) or "版本默认"
    print(f"编码器: {_codec_desc}  |  lookahead: {_la_desc}  |  rate-mode: {_rm_desc}")
    if args.no_verify:
        ...
        try:
            ...
            if warmup_video and warmup_video.exists():
                ...
                _result = run_version(
                    ...
                    crf=args.crf,
                    preset=args.preset,
                    la=la_list[0] if version_supports_la(warmup_version) else 0,
                    rate_mode=(rate_mode_list[0] if version_supports_rate_mode(warmup_version)
                               else "constqp"),
                )
    ...
    multi_codec = len(codec_list) > 1
    multi_la = len(la_list) > 1
    multi_rm = len(rate_mode_list) > 1

    # 展平运行组合：版本 × 编码 × la × rate_mode
    # 非 .1 版本：la 强制 0、rate_mode 强制 constqp；
    # constqp 硬件静默禁用 LA（NVENCEncoder 强制 la_depth=0）→ la 折叠为 0
    run_combos = []
    for version_module in selected:
        label = VERSION_LABELS[version_module]
        if version_supports_la(version_module):
            version_la_list = la_list
        else:
            version_la_list = [0]
            if args.la is not None and any(l not in (0, None) for l in la_list):
                print(f"  [INFO] {label} 非 .1 版本，自动屏蔽 --la 并按 la=0 执行")
        if version_supports_rate_mode(version_module):
            version_rm_list = rate_mode_list
        else:
            version_rm_list = ["constqp"]
            if args.rate_mode is not None and any(rm != "constqp" for rm in rate_mode_list):
                print(f"  [INFO] {label} 非 .1 版本，自动屏蔽 --rate-mode 并按 constqp 执行")
        for codec in codec_list:
            for rm in version_rm_list:
                for la in version_la_list:
                    _la_eff = 0 if (rm == "constqp" and la not in (0, None)) else la
                    combo = (version_module, label, codec, _la_eff, rm)
                    if combo not in run_combos:
                        run_combos.append(combo)

    total_runs = len(run_combos)

    for run_idx, (version_module, label, codec, la, rm) in enumerate(run_combos):
        # 唯一输出路径：多编码/多 la/多 rate_mode 时追加对应后缀，避免互相覆盖
        _fname_parts = [input_stem, label, f"x{args.scale:.0f}"]
        if multi_codec:
            _fname_parts.append(codec if codec else "auto")
        if multi_rm:
            _fname_parts.append(rm if rm else "default")
        if multi_la:
            _fname_parts.append(f"la{la}" if la is not None else "la-default")
        output_path = output_dir / ("_".join(_fname_parts) + ".mp4")

        print(f"[{run_idx+1}/{total_runs}] 测试 {label} "
              f"(codec={codec or 'auto'}, rm={rm or 'default'}, "
              f"la={la if la is not None else 'default'}) ...\n",
              end=" ", flush=True)

        display_label = _compose_display_label(label, codec, la, rm, multi_codec, multi_la, multi_rm)
        ...
        result["codec_requested"] = codec
        result["la_requested"] = la
        result["rm_requested"] = rm
    ...
    # 多编码 / 多 la / 多 rate_mode 组合时输出综合比较矩阵（FPS + File_Size）
    if multi_codec or multi_la or multi_rm:
        print_comparison_matrix(results, codec_list, la_list, rate_mode_list,
                                lang=args.language, metric='fps')
        print_comparison_matrix(results, codec_list, la_list, rate_mode_list,
                                lang=args.language, metric='size')
