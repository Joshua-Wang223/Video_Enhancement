...
N_FRAMES = 2000
N_WARMUP = 30
TOP_N = 3           # top-N: number of ranked techniques to produce output video files for
PRESET = "p1"        # NVENC preset: p1(poor)=lowest latency, p7=highest quality
QP = 23              # H.264 QP / VBR target quality
RATE_MODE = "vbr_hq"  # constqp / vbr_hq / qvbr
...
def run_technique(tech_key, W, H, fps, rate_mode, la_depth, n_frames, n_warmup,
    ...
    # ── v3: Per-combo version mapping (technique × pipe × la → IFRNet version) ──
    # [FIX-VERSION-LABEL] 所有版本默认使用 ce-pipeline deferred 路径（非 sync-batch）。
    # sync-batch 是历史对比基线，sync-batch pipe=1/4 LA=0 不代表任何版本的默认行为。
    _VERSION_MAP = {
        ('ce-pipeline', 4, 0): 'v6.4.3/4/5.1',     # 所有 CONSTQP 版本的生产路径
        ('ce-pipeline', 1, 8): 'v6.4.3.1/4.1',      # v6.4.3.1/4.1 LA=8 → pipe=1 (安全)
        ('ce-pipeline', 4, 8): 'v6.4.x exp.',        # pipe=4+LA=8 实验性 (0.6%空帧)
                ...
                if h264_data is None:
                    ...
                    if PLAN_A_MODE:
                        ...
                        for _rtry in range(2):
                            try:
                                h264_rt = enc.encode_frame(_nv12_t, force_idr=(fi == 0))
    ...
    # ── v3: Frame loss accounting ──
    _submitted = n_frames
    _recovered = success_count  # flush_frames already added to success_count (line above); empty_count is diagnostic-only, not recovery
    _lost = max(0, _submitted - _recovered)  # frames that entered encoder but produced zero H.264 output
    if _lost > 0:
        print(f'  [LOSS] {_lost}/{_submitted} frames unrecoverable '
              f'(submitted={_submitted} recovered={_recovered}='
              f'{success_count-flush_frames}valid+{la_count}LA+{flush_frames}flush+{empty_count}empty)', flush=True)
...
def main():
    ...
    ap = _ap.ArgumentParser(description='NVENC completionEvent 3D Full-Matrix Verification (v3) — batch-size + frame loss detection')
    ...
    ap.add_argument('--batch-size', '-b', type=int, default=BATCH_SIZE,
                    help=f'Frames per encode_*() call (default {BATCH_SIZE}). '
                         ...
                         f'Use {N_FRAMES} for single-call baseline.')
    ...
    _skipped = len(TECHNIQUES) * len(_Pipes) * len(_LAs) - _total_rounds
    # Adjust skip count for conditional techniques
    if not PLAN_B_MODE:
        _skipped -= 2  # la-harvest × 2 pipes × 1 LA (LA=8 only)
    else:
        _skipped -= 1  # la-harvest × 2 pipes × 1 LA (LA=0 skipped by min_la_depth)

    print("=" * 70)
    print("NVENC completionEvent 3D Full-Matrix Verification v3")
    src_label = f"real video: {args.input}" if args.input else "synthetic GPU tensors"
    _combo_info = f"{len(TECHNIQUES)} techniques x 2 pipes x 2 LAs = {_total_rounds} valid combos"
    if _skipped > 0:
        _combo_info += f" (skipped {_skipped}: phase4-slot/pfce × pipe=1)"
    print(f"Config: {W}x{H}@{FPS:.1f}fps, {RATE_MODE}, batch_size={args.batch_size}, {_combo_info}")
    ...
    # ── 3D full matrix: technique × pipe × la (valid combos only) ──
    _round = 0

    for tech_key, pipe, la in _test_combos:
        _round += 1
        _label = f"Round {_round}/{_total_rounds}  {tech_key}: pipe={pipe} LA={la}"
    ...
    if _can_topn:
        _sorted_pre = sorted(results, key=lambda rr: (0 if rr['empty_rate'] < 0.005 else (1 if rr['empty_rate'] < 0.02 else 2), -rr['fps']))
        _topn_qualify = [r for r in _sorted_pre[:TOP_N] if r['empty_rate'] < 0.02]
        _topn_total = len(_topn_qualify)
        if _topn_total == 0:
            print("  (no qualifying techniques with empty_rate < 2%)", flush=True)
        else:
            print(f"\n{'='*70}")
            print(f"TOP-{TOP_N}: Re-encoding full video via best techniques...")
            print(f"{'='*70}")
            _topn_round = 0
            for rank_idx in range(min(TOP_N, len(_sorted_pre))):
                r = _sorted_pre[rank_idx]
                if r['empty_rate'] >= 0.02:
                    continue
                try:
                    _topn_round += 1
                    ...
                    try:
                        _enc = EncCls(W, H, FPS, preset=PRESET, qp=QP,
                                      rate_mode=RATE_MODE, la_depth=la_val,
                        ...
                        # Flush — skip trivial fragments (LA residual < 1KB = no real frame data)
                        # ★ Keep stdout alive: [NVENC-FLUSH] is diagnostic, not noise ★
                        _flush = _enc.flush()
                        if _flush and len(_flush) > 1000:
                    ...
                    t3_info['rc_mode'] = RATE_MODE.upper()
    ...
    # [PLAN-A] When defense recovers all lost frames (lost==0), treat as OK
    def _sort_key(rr):
        er = rr['empty_rate']
        lost = max(0, rr.get('lost_frames', 0))
        if rr.get('plan_a') and rr.get('tier1b_success', 0) + rr.get('tier1a_fallback', 0) > 0:
            # Defense active: consider defended frames as recovered
            _defended = rr.get('tier1b_success', 0) + rr.get('tier1a_fallback', 0)
            _raw_empty = rr.get('empty', 0)
            # If defense recovered ALL empty frames, treat as OK regardless of raw empty_rate
            if lost == 0:
                verdict_order = 0
            else:
                verdict_order = 1 if er < 0.02 else (3 if lost > 0 else 2)
        else:
            verdict_order = 0 if (er < 0.005 and lost == 0) else (1 if (er < 0.02 and lost == 0) else (3 if lost > 0 else 2))
        return (verdict_order, -rr['fps'])
    ...
    print("=" * 120)
    print(f"3D FULL MATRIX — {W}x{H}@{FPS:.0f}fps N={N_FRAMES} batch_size={args.batch_size} — Technique x Pipe x LA — FPS Ranking")
    print("=" * 120)
    if PLAN_A_MODE:
        _hdr = (f"  {'Rank':>4}  {'Test Case':<48} {'CE':>11} {'Empty':>8} "
                f"{'Lost':>5} {'Def':>7}  {'FPS':>7} {'Rel%':>6}  {'Verdict'}")
        _sep = (f"  {'-'*4}  {'-'*48} {'-'*11} {'-'*8} "
                f"{'-'*5} {'-'*7}  {'-'*7} {'-'*6}  {'-'*22}")
    else:
        _hdr = (f"  {'Rank':>4}  {'Test Case':<55} {'CE':>11} {'Empty':>8} {'Lost':>5} "
    ...
    for _rank, r in enumerate(_sorted):
        ...
        if PLAN_A_MODE:
            ...
            print(f"  {_crown}{_rank+1:>2}  {r['label']:<48} {r['ce_info']:>11} "
                  f"{_er:>7.1%} {_lost_str:>5} {_def_str:>7} {r['fps']:>7.1f} {_rel:>5.1f}%  {_badge}")
        else:
            print(f"  {_crown}{_rank+1:>2}  {r['label']:<55} {r['ce_info']:>11} "
                  f"{_er:>7.1%} {_lost_str:>5} {r['fps']:>7.1f} {_rel:>5.1f}%  {_badge}")

    print(_sep)

    # ── Top-3 Stats Summary Table ──
    if topn_stats:
        print()
        print("=" * 115)
        print("TOP-N OUTPUT FILES — Detailed Statistics")
        print("=" * 115)
        # Simple format-string based table (no complex padding logic)
        print(f"  {'Rank':>4}  {'Input':>8}  {'Output':>8}  {'Container':>9}  "
              f"{'FPS':>8}  {'File Size':>10}  {'RC Mode':>8}  {'Encoder':>12}  {'Filename'}")
        print(f"  {'-'*4}  {'-'*8}  {'-'*8}  {'-'*9}  "
              f"{'-'*8}  {'-'*10}  {'-'*8}  {'-'*12}  {'-'*38}")
        for s in topn_stats:
            _rk = s.get('rank', '?')
            ...
            _enc = s.get('encoder', '?')
            _fn = s.get('filename', '?')
            print(f"  {_rk:>4}  {_in:>8}  {_out:>8}  {_cf:>9}  "
                  f"{_fp:>8.1f}  {_sz:>8.1f} MB  {_rc:>8}  {_enc:>12}  {_fn}")
        print()
    ...
    print(f"  Lost = n_submitted − (valid + la_buf + flush_recovered + empty). Frames NVENC encoded but never exposed via LockBitstream.")
