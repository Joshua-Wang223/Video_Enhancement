...
运行:
  pytest Accessory/probe/nvenc_sdk_realesrgan_suite.py -v                    # 所有测试
  pytest Accessory/probe/nvenc_sdk_realesrgan_suite.py -v -m gpu              # 仅 GPU 测试
  pytest Accessory/probe/nvenc_sdk_realesrgan_suite.py -v -m "not gpu"        # 仅无 GPU 测试
...
class TestFrameConservation:
    ...
    N_FRAMES = 16  # 测试用帧数（176x144 编码更慢，使用合理帧数）

    def _make_test_frames(self, n, h=144, w=176):
    ...
    @gpu
    @pytest.mark.skip(reason="已知 T4 / CUDA 13.0 / nvenc 580.65.06 下 qp=0 (CONSTQP) 导致 SIGSEGV，非代码回归；见 memory/nvenc-qp0-crash-workaround.md 与 Accessory/probe/nvenc_qp0_segv_repro.py")
    ...
    def test_encode_frame_returns_data_la0(self):
        ...
        try:
            import torch
            nv12 = torch.randint(0, 255, (216, 176), dtype=torch.uint8, device='cuda')
...
class TestLAAccumulation:
    ...
    def test_la_accumulation_no_cross_batch_loss(self):
        ...
        try:
            ...
            n = 32  # 减少帧数加速测试
            nv12_h = 144 + 72  # 216
            frames = [torch.randint(0, 255, (nv12_h, 176), dtype=torch.uint8, device='cuda')
                      for _ in range(n)]
    ...
    def test_f0_stash_and_insert(self):
        ...
        try:
            ...
            f0_nv12 = torch.randint(0, 255, (nv12_h, 176), dtype=torch.uint8, device='cuda')
            ...
            assert len(results) == 17, \
                f"With f0: expected 17, got {len(results)}"
            print(f"  f0+16 frames = {len(results)} total, "
    ...
    def test_nvenc_writer_write_frame_batch(self):
        ...
        try:
            ...
            try:
                ...
                frames = [np.random.randint(0, 256, (144, 176, 3), dtype=np.uint8)
                          for _ in range(16)]
    ...
    def test_nvenc_writer_multi_batch(self):
        ...
        try:
            ...
            try:
                ...
                for _ in range(8):
                    frame = np.random.randint(0, 256, (144, 176, 3), dtype=np.uint8)
                    writer.write_frame(frame)

                # Write a batch
                batch = [np.random.randint(0, 256, (144, 176, 3), dtype=np.uint8)
                         for _ in range(8)]
