#!/usr/bin/env python3
"""
NVENC completionEvent multi-mode verification — full matrix comparison.

Each Mode tests the 2×2 matrix: pipe ∈ {4,1} × la ∈ {8,0} (except Mode1: pipe=1 only).

  Mode 0: sync batch (NO CE) — v6.4.5.1 production pattern, golden baseline
  Mode 1: single-slot CE — pipe=1 per-frame CE, baseline
  Mode 2: batch per-frame CE — per-frame CE across slots, sync harvest
  Mode 3: PHASE4 async — slot-reuse CE (pre-created event, broken for pipe=4)
  Mode 4: PHASE4 + per-frame CE — fresh CE per frame, immediate harvest
  Mode 5: CE-pipeline — per-frame CE with deferred harvest at slot reuse
  Mode 6: sync batch (NO CE) — same as Mode0, separate independent baseline

Configurable via top-level constants: W/H, N_FRAMES, PRESET, QP, RATE_MODE.

Usage (on GPU server):
  python nvenc_completion_event_matrix.py
...
def run_mode0(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """Synchronous batch (NO CE) — golden baseline."""
    label = f"Mode0: pipe={pipeline_depth} sync batch LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    # Use base NVENCEncoder with specified pipeline_depth (sync encode_frames_batch)
    enc = NVENCEncoder(W, H, fps, preset=preset, qp=qp,
                       rate_mode=rate_mode, la_depth=la_depth,
                       pipeline_depth=pipeline_depth)

    # Warmup with encode_frame
    for wi in range(n_warmup):
        rgb = _make_test_rgb(H, W, wi)
        nv12 = _rgb_to_nv12_gpu(rgb, input_is_bgr=False)
        torch.cuda.synchronize()
        enc.encode_frame(nv12)
    torch.cuda.synchronize()
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        results = enc.encode_frames_batch(batch_tensors,
    ...
    _summarize_empty("Mode0", empty_frames)
...
def run_mode1(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """Per-frame CE encoding (pipe-agnostic, uses slot[0])."""
    label = f"Mode1: pipe={pipeline_depth} single CE LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    enc = NVENCEncoderMode1(W, H, fps, preset=preset, qp=qp,
                            rate_mode=rate_mode, la_depth=la_depth,
                            pipeline_depth=pipeline_depth)
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        for i, nv12 in enumerate(batch_tensors):
            ...
            h264_data = enc.encode_frame_with_ce(nv12)
            if not h264_data:
                if fi < la_depth and la_depth > 0:
                    la_count += 1
                else:
    ...
    _summarize_empty("Mode1", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
def run_mode2(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """Batch per-frame CE (expect empty frames at pipe=4)."""
    label = f"Mode2: pipe={pipeline_depth} batch CE LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    enc = NVENCEncoderMode2(W, H, fps, preset=preset, qp=qp,
                            rate_mode=rate_mode, la_depth=la_depth,
                            pipeline_depth=pipeline_depth)

    # Warmup with base class batch method (synchronous)
    for wi in range(n_warmup):
        rgb = _make_test_rgb(H, W, wi)
        nv12 = _rgb_to_nv12_gpu(rgb, input_is_bgr=False)
        torch.cuda.synchronize()
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        results = enc.encode_frames_batch_with_ce(batch_tensors,
                                                   force_idr_first=(batch_start == 0))
        for i, h264_data in enumerate(results):
            fi = batch_start + i
            if h264_data is None:
    ...
    _summarize_empty("Mode2", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
def run_mode3(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """PHASE4 async submit/harvest (feasibility test)."""
    label = f"Mode3: pipe={pipeline_depth} PHASE4 async LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    enc = NVENCEncoderMode3(W, H, fps, preset=preset, qp=qp,
                            rate_mode=rate_mode, la_depth=la_depth,
                            pipeline_depth=pipeline_depth)

    # Warmup with base class batch method
    for wi in range(n_warmup):
        rgb = _make_test_rgb(H, W, wi)
        nv12 = _rgb_to_nv12_gpu(rgb, input_is_bgr=False)
        torch.cuda.synchronize()
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        results = enc.encode_frames_batch_phase4(batch_tensors,
                                                  force_idr_first=(batch_start == 0))
        for i, h264_data in enumerate(results):
            fi = batch_start + i
            if h264_data is None:
    ...
    _summarize_empty("Mode3", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
# ============================================================================
# Mode 4: pipeline_depth=4 PHASE4 async + per-frame CE + LA8
# ============================================================================
...
def run_mode4(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """PHASE4 + per-frame CE (no event reuse) — test: does per-frame CE fix the event reuse corruption?"""
    label = f"Mode4: pipe={pipeline_depth} PHASE4+pfCE LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    enc = NVENCEncoderMode4(W, H, fps, preset=preset, qp=qp,
                            rate_mode=rate_mode, la_depth=la_depth,
                            pipeline_depth=pipeline_depth)
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        results = enc.encode_frames_batch_phase4_pfce(batch_tensors,
                                                        force_idr_first=(batch_start == 0))
        for i, h264_data in enumerate(results):
            fi = batch_start + i
            if h264_data is None:
    ...
    _summarize_empty("Mode4", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
# ============================================================================
# Mode 5: CE pipeline — per-frame CE with deferred LockBitstream (pipe=4 + LA)
# ============================================================================
...
def run_mode5(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """CE pipeline: per-frame CE with deferred harvest."""
    label = f"Mode5: pipe={pipeline_depth} CE-pipeline LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    enc = NVENCEncoderMode5(W, H, fps, preset=preset, qp=qp,
                            rate_mode=rate_mode, la_depth=la_depth,
                            pipeline_depth=pipeline_depth)
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        results = enc.encode_frames_batch_ce_pipeline(batch_tensors,
                                                        force_idr_first=(batch_start == 0))
        for i, h264_data in enumerate(results):
            fi = batch_start + i
            if h264_data is None:
    ...
    _summarize_empty("Mode5", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
def run_mode6(W, H, fps, rate_mode, la_depth, n_frames, n_warmup, pipeline_depth=4, preset="p1", qp=23):
    """Sync batch (NO CE) — v6.4.5.1 production baseline."""
    label = f"Mode6: pipe={pipeline_depth} sync LA={la_depth} {preset} QP{qp}"
    print(f"\n{'='*60}")
    print(f"[{label}]")
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, pipe={pipeline_depth}, N={n_frames}")
    print(f"{'='*60}")

    # Direct NVENCEncoder with specified pipeline_depth — mirrors v6.4.5.1 encode_frames_batch
    enc = NVENCEncoder(W, H, fps, preset=preset, qp=qp,
                       rate_mode=rate_mode, la_depth=la_depth,
                       pipeline_depth=pipeline_depth)
    ...
    _summarize_empty("Mode6", empty_frames)
    flush_frames, flush_bytes = _flush_and_count(enc)
    success_count += flush_frames
    t_elapsed = time.time() - t_start
    enc.close()
...
def main():
    print("=" * 70)
    print("NVENC completionEvent Multi-Mode Verification")
    print(f"Config: {W}x{H}, VBR_HQ, la_depth={LA_DEPTH}, N={N_FRAMES}")
    print("=" * 70)
    ...
    # ── Full 2×2 matrix: pipe ∈ {4,1} × la ∈ {LA_DEPTH, LA_DEPTH0} for every Mode ──
    _LAs = [LA_DEPTH, LA_DEPTH0]
    _Pipes = [4, 1]

    # Mode 0: sync batch (NO CE)
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode0 pipe={pipe} LA={la}", run_mode0, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 1: per-frame CE — full pipe×LA matrix
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode1 pipe={pipe} LA={la}", run_mode1, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 2: batch per-frame CE
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode2 pipe={pipe} LA={la}", run_mode2, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 3: PHASE4 async submit/harvest
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode3 pipe={pipe} LA={la}", run_mode3, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 4: PHASE4 + per-frame CE
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode4 pipe={pipe} LA={la}", run_mode4, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 5: CE-pipeline deferred harvest
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode5 pipe={pipe} LA={la}", run_mode5, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # Mode 6: sync batch (NO CE) — same as Mode0, kept as separate baseline
    for pipe in _Pipes:
        for la in _LAs:
            _run(f"Mode6 pipe={pipe} LA={la}", run_mode6, W, H, FPS, RATE_MODE, la, N_FRAMES, N_WARMUP, pipeline_depth=pipe, preset=PRESET, qp=QP)

    # ── Summary ──
    _sorted = sorted(results, key=lambda r: r['fps'], reverse=True)
    _best_fps = _sorted[0]['fps'] if _sorted else 1.0

    print("\n" + "=" * 100)
    print("ANALYSIS MATRIX — Comparison Dimensions & FPS Ranking")
    print("=" * 100)
    _hdr = f"  {'Rank':>4}  {'Mode':<42} {'Pipe':>4} {'LA':>3} {'CE':>11} {'Empty':>9} {'FPS':>7} {'Rel%':>6}  {'Verdict'}"
    _sep = f"  {'-'*4}  {'-'*42} {'-'*4} {'-'*3} {'-'*11} {'-'*9} {'-'*7} {'-'*6}  {'-'*12}"
    print(_hdr)
    print(_sep)
    ...
    for _rank, r in enumerate(_sorted):
        ...
        # Parse pipe / LA / CE type from label
        _label = r['label']
        _pipe_match = re.search(r'pipe=(\d+)', _label)
        _la_match = re.search(r'LA=(\d+)', _label)
        _pipe_info = _pipe_match.group(1) if _pipe_match else "?"
        _la_info = _la_match.group(1) if _la_match else "?"

        if 'Mode1' in _label:      _ce_info = "per-frame"
        elif 'Mode2' in _label:    _ce_info = "batch"
        elif 'Mode3' in _label:    _ce_info = "slot-reuse"
        elif 'Mode4' in _label:    _ce_info = "per-frame"
        elif 'Mode5' in _label:    _ce_info = "deferred"
        elif 'Mode6' in _label:    _ce_info = "none"
        elif 'Mode0' in _label:    _ce_info = "none"
        else:                      _ce_info = "?"
        ...
        # Highlight production baseline (Mode6 pipe=4 LA=0)
        if 'Mode6' in _label and 'pipe=4' in _label:
            _badge += " 🏭"

        print(f"  {_crown}{_rank+1:>2}  {_label:<42} {_pipe_info:>4} {_la_info:>3} {_ce_info:>11} "
              f"{_er:>8.1%} {r['fps']:>7.1f} {_rel:>5.1f}%  {_badge}")

    print(_sep)
    print(f"  Legend: 👑=fastest  ✅ OK=0% empty  ⚠️=<2% empty  ❌=broken  🏭=v6.4.5.1 baseline")
