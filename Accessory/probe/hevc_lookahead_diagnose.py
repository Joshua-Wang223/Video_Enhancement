...
# -*- coding: utf-8 -*-
"""
HEVC + LA 最小化 GPU 诊断/验证脚本（Accessory/ 临时诊断脚本，非 pytest）。
...
本脚本参照 Accessory/probe/nvenc_la_frame_conservation_suite.py 的 MinimalTestEncoder，
...
-----------------------------------------
  ...
  timeout 90 python Accessory/probe/hevc_lookahead_diagnose.py --variant delayed \
  ...
  python Accessory/probe/hevc_lookahead_diagnose.py --run-all --frames 700 --timeout 75
...
try:
    import nvenc_la_frame_conservation_suite as ref
...
def main():
    ...
    if args.self_test:
        return self_test()
    if not _REF_LOADED:
        print(f"[FATAL] 无法导入 nvenc_la_frame_conservation_suite.py "
