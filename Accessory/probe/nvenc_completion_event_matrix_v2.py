...
def run_technique(tech_key, W, H, fps, rate_mode, la_depth, n_frames, n_warmup,
    ...
    return _make_result(label, success_count, empty_count, la_count,
                        ...
                        ce_info=ce_info, is_baseline=is_baseline, extra=extra)


def _make_result(label, success, empty, la, flush_f, flush_b, t, n,
                 ce_info="?", is_baseline=False, extra=None):
...
def main():
    ...
    ap = _ap.ArgumentParser(description='NVENC completionEvent 3D Full-Matrix Verification (v2)')
    ...
    if args.input:
        ...
        N_FRAMES = min(2000, _total) if _total else 2000
        ...
        FPS = 30.0
        N_FRAMES = 2000
    N_WARMUP = min(30, N_FRAMES // 3)

    output_dir = args.output or os.path.join(os.getcwd(), 'temp')

    # ── Build valid test matrix: filter phase4 × pipe=1 ──
    ...
    print("=" * 70)
    print("NVENC completionEvent 3D Full-Matrix Verification v2")
    ...
    print(f"Config: {W}x{H}@{FPS:.1f}fps, {RATE_MODE}, {_combo_info}")
    ...
    print("=" * 120)
    print("3D FULL MATRIX — Technique x Pipe x LA — FPS Ranking")
    print("=" * 120)
