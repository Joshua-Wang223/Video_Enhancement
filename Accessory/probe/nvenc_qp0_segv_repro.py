...
`Plan/NVENC硬件测试隔离_立项Prompt.md` 原本把 `pytest Accessory/` 的 SIGSEGV 归因为
**跨测试类的状态污染**，并以「逐文件隔离」为方案 A。2026-09-16 在 Linux + T4
（驱动 580.65.06 / CUDA 13.0）实测**推翻**了该归因：

- `Accessory/probe/nvenc_sdk_realesrgan_suite.py` **单独**跑也会崩（3/8），
  连**单个测试**独立循环也崩（3/10）；
- 崩溃与测试顺序无关，与前置文件无关。
...
## 跑法
    ...
    python Accessory/probe/nvenc_qp0_segv_repro.py                 # 三模式各 15 次
    python Accessory/probe/nvenc_qp0_segv_repro.py --iters 30
    python Accessory/probe/nvenc_qp0_segv_repro.py --modes ctor_only ce_pipeline
    python Accessory/probe/nvenc_qp0_segv_repro.py --qp 23         # 对照组
