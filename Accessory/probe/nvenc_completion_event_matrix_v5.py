#!/usr/bin/env python3
"""
NVENC completionEvent multi-mode verification -- 4D full matrix comparison (v5: Frame Conservation Fix).

4-dimensional matrix: 3 RC modes x 6 techniques x 4 pipes x 3 LAs = up to 216 combinations.

  Technique       Description
  ----------      ----------
  sync-batch      Synchronous batch (NO CE) -- historical comparison baseline [v5: +drain loop]
  single-ce       Single-slot per-frame CE (create->sync->destroy) [v5: +drain loop]
  batch-ce        Batch per-frame CE across all slots, sync harvest [v5: +drain loop]
  ce-pipeline     Per-frame CE with deferred harvest at slot reuse [v5: +drain loop]
  la-harvest      LA-Aware Harvest -- completion-driven slot reuse [v5: +drain loop]
  sync-drain-loop Sync drain loop -- per-frame EncodePicture (no CE) + blocking Lock until NEED_MORE_INPUT

v5 FRAME CONSERVATION FIXES (2026-07-01):
  Based on Accessory/probe/nvenc_la_frame_conservation_suite.py and the NVENC Lookahead summary.

  1. pipeline_depth = max(pipeline_depth, LA+1) -- SDK hardware safety requirement
  2. _drain_outputs() -- loop blocking LockBitstream until NEED_MORE_INPUT after each EncodePicture
  3. Removed NEED_MORE_INPUT forced Lock/Unlock -- was silently discarding valid frames
  4. EOS flush uses blocking Lock per slot (doNotWait=0) -- recovers ALL LA-pending frames
  5. Frame accounting: total_output (encode + EOS flush) == total_input (strict conservation)

  These fixes ensure Input == Output under ALL RC modes x LA depths x pipe configs.

Configurable via top-level constants: W/H, N_FRAMES, PRESET, QP, RATE_MODE.

Usage (on GPU server):
  python nvenc_completion_event_matrix_v5.py
  python nvenc_completion_event_matrix_v5.py --rate-modes vbr_hq --la 8 --frames 200
  python nvenc_completion_event_matrix_v5.py --rate-modes constqp,vbr_hq,qvbr --frames 300
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        # [v5-FIX] pipeline_depth >= LA+1 per NVENC SDK hardware safety requirement
        # 启用 lookahead 时比特流缓冲区数量至少 lookaheadDepth+1，防止硬件覆盖未读数据
        if la_depth > 0:
            pipeline_depth = max(pipeline_depth, la_depth + 1)
        self._pipeline_depth = max(1, min(32, pipeline_depth))
        self._slots: list = []

        # [v5-FIX] 输出槽位指针: 追踪下一个预期输出 slot，保证帧顺序
        # 遵循 NVENC SDK "送一帧 -> 循环 LockBitstream 直到 NEED_MORE_INPUT" 规则
        self._output_slot_idx = 0
        # [v5-FIX-FLAT] _pending_outputs_v5 已移除 — 扁平输出模式不需要跨 batch 缓冲
        ...
        # [v5-FIX4] Dynamic H264 codec GUID query — match known H264 GUIDs from driver enumeration
        # (conservation test pattern: query GetEncodeGUIDs, find H264 by known Data1 values)
        _KNOWN_H264_DATA1 = (0x6bc82762, 0x6b9c211b)
        codec_guid = None
        _h264_matched = False
        for _gi in range(n_guids):
            if guid_array[_gi].Data1 in _KNOWN_H264_DATA1:
                codec_guid = guid_array[_gi]
                _h264_matched = True
                break
        if not _h264_matched:
            raise RuntimeError(
                "[NVENCEncoder] H264 codec GUID not found in driver's GetEncodeGUIDs response. "
                "Available GUIDs: %s" % [(hex(g.Data1), g.Data2, g.Data3) for g in guid_array])
        print("[NVENCEncoder] Driver codec GUID: %08x-%04x-%04x (matched)" % (
    ...
    # ── [v5-FIX] Frame conservation methods (from nvenc_la_frame_conservation_suite.py) ──

    def _lock_bitstream_once_blocking(self, bs_handle):
        """[v5] 单次 blocking LockBitstream (doNotWait=0). 返回 (h264_bytes, status).

        NVENC SDK 规范: 使用 doNotWait=0 (blocking) 确保取回完整编码帧。
        与 _lock_bitstream_with_retry 不同，此方法不做指数退避重试 --
        因为 blocking Lock 本身就会等待硬件完成。
        """
        _LockBitstreamProto_raw = ctypes.CFUNCTYPE(
            c_uint32, c_void_p, ctypes.POINTER(ctypes.c_uint8 * 1544))
        _lock_fn = _LockBitstreamProto_raw(self._func_ptrs[_FUNC_IDX["LockBitstream"]])
        _unlock_fn = _NvEncUnlockBitstreamProto(self._func_ptrs[_FUNC_IDX["UnlockBitstream"]])

        lock_raw = (ctypes.c_uint8 * 1544)()
        ctypes.memset(lock_raw, 0, 1544)
        ctypes.cast(lock_raw, ctypes.POINTER(ctypes.c_uint32))[0] = NV_ENC_LOCK_BITSTREAM_VER
        ctypes.cast(ctypes.byref(lock_raw, 4), ctypes.POINTER(ctypes.c_uint32))[0] = 0  # doNotWait=0 (blocking)
        ctypes.cast(ctypes.byref(lock_raw, 8), ctypes.POINTER(ctypes.c_void_p))[0] = bs_handle

        bs_status = _lock_fn(self._encoder, ctypes.cast(lock_raw,
                            ctypes.POINTER(ctypes.c_uint8 * 1544)))

        h264_data = b""
        if bs_status == NV_ENC_SUCCESS:
            bitstream_size = ctypes.cast(ctypes.byref(lock_raw, 36), ctypes.POINTER(ctypes.c_uint32))[0]
            if bitstream_size > 0:
                _raw_bsptr = ctypes.cast(ctypes.byref(lock_raw, 56), ctypes.POINTER(ctypes.c_void_p))[0]
                bs_ptr_val = _raw_bsptr if isinstance(_raw_bsptr, int) else (_raw_bsptr.value or 0)
                if bs_ptr_val:
                    buf_type = ctypes.c_uint8 * bitstream_size
                    h264_data = bytes(buf_type.from_address(bs_ptr_val))
            _unlock_fn(self._encoder, bs_handle)

        return h264_data, bs_status

    def _drain_slot(self, bs_handle) -> list:
        """[v5] 循环 blocking Lock 指定 slot 直到 NEED_MORE_INPUT。

        用于单 slot 技术（如 single-ce），所有帧通过同一 slot → 需反复
        Lock 该 slot 直到 NEED_MORE_INPUT。
        """
        outputs = []
        while True:
            data, status = self._lock_bitstream_once_blocking(bs_handle)
    ...
    def _drain_outputs(self) -> list:
        """[v5] 循环 blocking Lock 当前输出 slot 直到 NEED_MORE_INPUT。

        NVENC SDK 规范: 每送入一帧后必须反复 LockBitstream (blocking)，
        直到返回 NEED_MORE_INPUT，确保无滞留帧。

        收集所有已完成帧，推进 _output_slot_idx 指针。
        返回该次排空操作取回的所有 H.264 bytes 列表。
        """
        outputs = []
        pd = self._pipeline_depth
        while True:
            slot_idx = self._output_slot_idx % pd
    ...
    def encode_frames_batch(self, nv12_tensors: list, force_idr_first: bool = False) -> list:
        ...
        Uses blocking EncodePicture (no completionEvent) + _drain_outputs() loop.
        Frames distributed across pipeline slots round-robin to reduce buffer contention.

        [v5-FIX-FLAT] Returns flat list of H.264 bytes in drain order (NOT 1:1 mapped to input).
        NVENC may produce 0, 1, or multiple outputs per input frame submission.
        The total output count + EOS flush should equal total input count.
        """
        ...
        try:
            with self._lock:
                ...
                # [v5-FIX-FLAT] 扁平输出列表：按 drain 产出顺序收集所有 H.264 帧。
                # NVENC 每次 submit 后 _drain_outputs() 可能返回 0/1/多个帧。
                # 不再维持 1:1 输入→输出映射 — 调用者通过总帧数守恒验证正确性。
                all_outputs = []
                ...
                for fi in range(n_frames):
                    # [v5-FIX-SLOT] 使用全局 _frame_idx 而非局部 fi 选择 slot，
                    # 确保跨 batch 时 slot 分配与 _output_slot_idx drain 对齐。
                    # 参见 nvenc_la_frame_conservation_suite.py MinimalTestEncoder。
                    ...
                    if _ep_status == NV_ENC_ERR_NEED_MORE_INPUT:
                        # [v5-FIX] 移除强制 Lock/Unlock — NEED_MORE_INPUT 时 buffer 无有效输出，
                        # 强制操作可能误弃其他 slot 已完成但未取走的帧 (Bug 2 from summary).
                        # 仅记录 LA 缓冲计数，不触碰 bitstream buffer.
                        ...
                        if self._la_buffered <= 3 or self._la_buffered == self._la_depth:
                            print(f'[NVENC-Enc] 前向帧预看缓冲中 ({self._la_buffered}/{self._la_depth})',
                                  flush=True)
                        # [v5-FIX2] Still attempt to drain previously buffered frames
                        new_outputs = self._drain_outputs()
                        if new_outputs:
                            all_outputs.extend(new_outputs)
                        # [v5-FIX-FLAT] 不再插入 b"" 占位 — LA 缓冲帧不产生输出
                        continue
                    ...
                    # ── [v5-FIX] drain loop: 循环 blocking LockBitstream 直到 NEED_MORE_INPUT ──
                    # NVENC SDK 规范: 每送入一帧后必须反复 LockBitstream (blocking)
                    # 直到返回 NEED_MORE_INPUT，确保所有已完成帧立即取走。
                    new_outputs = self._drain_outputs()
                    if new_outputs:
                        # [v5-FIX-FLAT] 全部 drain 帧追加到扁平输出列表
                        # SPS/PPS 注入：首个已完成的帧如果是 IDR，预挂缓存的 SPS+PPS
                        _first_out = new_outputs[0]
                        if force_idr and self._cached_sps_pps is not None:
                            _first_out = self._cached_sps_pps + _first_out
                        elif force_idr and self._cached_sps_pps is None and _first_out:
                            self._cached_sps_pps = self._extract_sps_pps(_first_out)
                            if self._cached_sps_pps:
                                print("[NVENCEncoder] Cached SPS+PPS: %d bytes" % len(self._cached_sps_pps),
                                      flush=True)
                        elif not force_idr and self._cached_sps_pps is None and _first_out:
                            self._cached_sps_pps = self._extract_sps_pps(_first_out)
                            if self._cached_sps_pps:
                                print("[NVENCEncoder] Cached SPS+PPS: %d bytes" % len(self._cached_sps_pps),
                                      flush=True)
                        new_outputs[0] = _first_out
                        all_outputs.extend(new_outputs)
                    else:
                        # [v5] drain 可能返回空（CES 竞态），记录诊断
    ...
    def encode_frame(self, nv12_gpu_tensor, force_idr: bool = False) -> List[bytes]:
        """Encode one NV12 GPU tensor → List[H.264 ES bytes] (synchronous, backward compat).
        ...
        Returns all drained frames (may be 0, 1, or more with LA>0).
        ...
        try:
            with self._lock:
                ...
                # [v5-FIX] 先检查 NEED_MORE_INPUT，再决定是否 drain
                if status == NV_ENC_ERR_NEED_MORE_INPUT:
                    # lookahead: encoder needs more frames to fill the lookahead window.
                    # [v5] NO forced Lock/Unlock — buffer 中无有效输出
                    self.__dict__.setdefault('_la_buffered', 0)
                    self._la_buffered += 1
                    if self._la_buffered <= 3 or self._la_buffered == self._la_depth:
                        print(f'[NVENC-Enc] 前向帧预看缓冲中 ({self._la_buffered}/{self._la_depth})',
                ...
                # ── [v5-FIX] drain loop: 循环 blocking LockBitstream 直到 NEED_MORE_INPUT ──
                # [v5-FIX-FLAT] Return all drained frames — with LA>0, drain may produce
                # multiple buffered frames at once. Callers extend their output list.
                new_outputs = self._drain_outputs()
                h264_outputs = new_outputs if new_outputs else []
                if not h264_outputs:
                ...
                # [SEGMENT-REUSE] Apply SPS+PPS to first output frame (IDR)
                if h264_outputs:
                    first = h264_outputs[0]
                    if force_idr and self._cached_sps_pps is not None:
                        h264_outputs[0] = self._cached_sps_pps + first
                    elif force_idr and self._cached_sps_pps is None and first:
                        # [FIX-SPS-CACHE] 首个 IDR 帧: 从码流提取并缓存 SPS+PPS
                        self._cached_sps_pps = self._extract_sps_pps(first)
                        if self._cached_sps_pps:
                            print("[NVENCEncoder] Cached SPS+PPS: %d bytes" % len(self._cached_sps_pps),
                                  flush=True)
                    elif not force_idr and self._cached_sps_pps is None and first:
                        self._cached_sps_pps = self._extract_sps_pps(first)
                        if self._cached_sps_pps:
                            print("[NVENCEncoder] Cached SPS+PPS: %d bytes" % len(self._cached_sps_pps),
                                  flush=True)
    ...
    def flush(self) -> bytes:
        """[v5-FIX] EOS flush: send EOS -> drain all slots with blocking LockBitstream.

        NVENC SDK 规范: "The client must call nvEncEncodePicture() with a NULL
        input picture, then repeatedly call nvEncLockBitstream (doNotWait=0 blocking)
        to retrieve all remaining encoded frames from the lookahead queue."

        [v5] 使用 blocking Lock (doNotWait=0) 逐 slot 循环排空直到 NEED_MORE_INPUT，
        确保回收所有 LA-pending 帧。顺序按 _output_slot_idx 指针保证帧顺序。

        返回所有回收帧数据拼接的 bytes。同时存储 _flush_frame_count 供诊断。
        """
        # [FIX-FLUSH-CTX] 跨线程 CUDA context 保护
        _need_pop = False
        _primary = getattr(self, '_primary_ctx', None)
        if _primary is not None and _primary.value is not None:
            try:
        ...
        try:
            with self._lock:
                ...
                # ── [v5-FIX-FLAT] 不再有 _pending_outputs_v5 需要排空，
                # 扁平输出模式下所有 drain 帧已直接追加到输出列表。
                result_parts = []
                ...
                ctypes.memset(pic_buf, 0, 3360)
                cast(pic_buf, ctypes.POINTER(c_uint32))[0] = NV_ENC_PIC_PARAMS_VER
                cast(byref(pic_buf, 16), ctypes.POINTER(c_uint32))[0] = 0x8  # EOS flag
                cast(byref(pic_buf, 40), ctypes.POINTER(c_void_p))[0] = c_void_p(None)
                cast(byref(pic_buf, 48), ctypes.POINTER(c_void_p))[0] = self._slots[0]['bs_buf']

                encode_picture = _NvEncEncodePictureProto(self._func_ptrs[_FUNC_IDX["EncodePicture"]])
                encode_picture(self._encoder, cast(pic_buf, ctypes.POINTER(_NvEncPicParams)))

                # ── [v5] 按输出指针顺序逐 slot blocking Lock 排空 ──
                # NVENC SDK 规范: 发送 EOS 后必须 blocking Lock 直到 NEED_MORE_INPUT
                _total_recovered_frames = 0
                _total_recovered_bytes = 0
                pd = self._pipeline_depth
                start_slot = self._output_slot_idx % pd
                drain_order = [(start_slot + i) % pd for i in range(pd)]
                ...
                for slot_idx in drain_order:
                    ...
                    while True:
                        ...
                        self._output_slot_idx += 1
                        _total_recovered_frames += 1
                        _total_recovered_bytes += len(data)
                ...
                # Store exact frame count for diagnostics
...
# [v5-FIX-FLAT] PLAN_A / PLAN_B removed — Tier 1-B/A defense no longer needed with flat output.
# la-harvest (Mode6) permanently excluded from default matrix (experimental dead-end per project memory).
...
class NVENCEncoderMode1(NVENCEncoder):
    ...
    def encode_frame_with_ce(self, nv12_gpu_tensor, force_idr=False):
        ...
        Returns List[bytes] — all drained frames (may be 0, 1, or more with LA>0)."""
        ...
        try:
            with self._lock:
                ...
                # [v5-FIX] Use _drain_outputs() instead of _drain_slot to drain all slots.
                # With LA>0, NVENC may route outputs to different slot buffers.
                new_outputs = self._drain_outputs()
                if new_outputs:
                    h264_outputs = new_outputs
                else:
                    h264_outputs = []
...
class NVENCEncoderMode2(NVENCEncoder):
    ...
    def encode_frames_batch_with_ce(self, nv12_tensors, force_idr_first=False):
        ...
        [v5-FIX-FLAT] 返回扁平 H.264 列表（drain 产出顺序），不再 1:1 映射。
        使用 _drain_outputs() 替代单 slot LockBitstream — LA 模式下 NVENC 可能
        将输出路由到不同 slot，必须遍历所有 slot 才能全部取回。
        """
        import ctypes as _ct
        n_frames = len(nv12_tensors)
        if n_frames == 0:
            ...
            with self._lock:
                ...
                for fi in range(n_frames):
                    ...
                    if status == NV_ENC_ERR_NEED_MORE_INPUT:
                        ...
                        if new_outputs:
                            all_outputs.extend(new_outputs)
                        # [v5-FIX-FLAT] 不插入占位符，LA 缓冲帧不产生输出
                        continue
                    ...
                    # [v5-FIX-FLAT] 使用 _drain_outputs() 替代单 slot LockBitstream.
                    # LA 模式下 NVENC 可能将输出路由到不同的 bitstream buffer，
                    # 必须遍历所有 slot 才能取回所有已完成帧。
                    new_outputs = self._drain_outputs()
                    if new_outputs:
                        all_outputs.extend(new_outputs)
...
def _flush_and_count(enc):
    """[v5] Flush encoder and return (exact_frame_count, total_bytes).

    Uses encoder._flush_frame_count from the drain loop for exact frame count,
    falling back to NAL start code heuristic for encoders without the field.
    """
    flush_data = enc.flush()
    if not flush_data:
        return 0, 0
    # [v5] Prefer exact count from drain loop
    _exact = getattr(enc, '_flush_frame_count', None)
    if _exact is not None and _exact > 0:
        return _exact, len(flush_data)
    # Fallback: NAL start code heuristic
...
class NVENCEncoderMode5(NVENCEncoder):
    ...
    def __init__(self, width: int, height: int, fps: float,
                 preset: str = "p1", qp: int = 0,
                 codec: str = "h264", pipeline_depth: int = 4,
                 rate_mode: str = "constqp", la_depth: int = 8):
        super().__init__(width, height, fps, preset=preset, qp=qp,
                         codec=codec, pipeline_depth=pipeline_depth,
                         rate_mode=rate_mode, la_depth=la_depth)
    ...
    def encode_frames_batch_ce_pipeline(self, nv12_tensors, force_idr_first=False):
        ...
        [v5-FIX-FLAT] 返回扁平 H.264 列表（drain 产出顺序），不再 1:1 映射。
        收割时使用 _drain_outputs() 替代单 slot LockBitstream — LA 模式下 NVENC 可能
        将输出路由到不同 slot，必须遍历所有 slot 才能全部取回。
        """
        import ctypes as _ct
        n_frames = len(nv12_tensors)
        ...
        try:
            with self._lock:
                ...
                # Per-slot pending state: (ce_handle, ep_status) — frame_index dropped; outputs go flat.
                ...
                for fi in range(n_frames):
                    ...
                    if _slot_pending[slot_idx] is not None:
                        _prev_ce, _prev_ep_s = _slot_pending[slot_idx]
                        ...
                        # [v5-FIX-FLAT] 使用 _drain_outputs() 替代单 slot LockBitstream
                        # LA 模式下 NVENC 可能将输出路由到不同 slot，必须遍历所有 slot
                        new_outputs = self._drain_outputs()
                        if new_outputs:
                            all_outputs.extend(new_outputs)
                    ...
                    _slot_pending[slot_idx] = (_ce, _ep_s)
                # ── Phase 3: Drain remaining pending slots ──
                for slot_idx in range(pd):
                    if _slot_pending[slot_idx] is not None:
                        _pending_ce, _pending_ep_s = _slot_pending[slot_idx]
...
class NVENCEncoderMode6(NVENCEncoder):
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        super().__init__(width, height, fps, preset=preset, qp=qp,
                         codec=codec, pipeline_depth=_expanded_pd,
                         rate_mode=rate_mode, la_depth=la_depth)
    ...
    def encode_frames_batch_la_harvest(self, nv12_tensors, force_idr_first=False):
        ...
        try:
            with self._lock:
                ...
                # Per-slot pending state: (ce, ep_status) — frame_index dropped; outputs go flat.
                _slot_pending = [None] * total_slots
                ...
                for fi in range(n_frames):
                    ...
                    for si in range(total_slots):
                        ...
                        if _pce.value and self._libcuda.cuEventQuery(_pce) == 0:
                            ...
                            # [v5-FIX-FLAT] _drain_outputs() 替代单 slot LockBitstream
                            new_outputs = self._drain_outputs()
                            if new_outputs:
                                all_outputs.extend(new_outputs)
                    ...
                    # ═══ Phase 0.5: Fallback — wait ANY pending CE if pool exhausted ═══
                    while not _free_slots_queue:
                        # Find any pending slot (no frame_index tracking needed for flat output)
                        _oldest_si = None
                        for si in range(total_slots):
                        ...
                        _oce, _oeps = _slot_pending[_oldest_si]
                        if _oce.value:
                            self._libcuda.cuEventSynchronize.restype = _ct.c_uint32
                            self._libcuda.cuEventSynchronize.argtypes = [_ct.c_void_p]
...
# ============================================================================
# Mode 8: sync-drain-loop — synchronous EncodePicture + per-frame drain loop
# ============================================================================
class NVENCEncoderMode8(NVENCEncoder):
    """[sync-drain-loop] 同步逐帧 drain 模式 — 每帧 EncodePicture(sync, no CE)
    + 循环 LockBitstream(blocking) 直到 NEED_MORE_INPUT。

    来源于 Accessory/probe/nvenc_la_frame_conservation_suite.py 的 MinimalTestEncoder 架构。
    核心差异（对比 sync-batch / ce-pipeline）：
    1. 无 completionEvent — EncodePicture 同步返回
    2. 每帧后立即 _drain_outputs() — 循环 blocking LockBitstream 直到 NEED_MORE_INPUT
    3. _output_slot_idx 指针推进 — 保证输出帧顺序与硬件严格一致
    4. EOS flush — 逐 slot blocking LockBitstream loop 排空所有 LA 滞留帧

    NVENC SDK 规范："送一帧 → 循环 LockBitstream 直到 NEED_MORE_INPUT"
    pipeline_depth = LA+1 (SDK 硬件安全要求，防止缓冲区覆盖)
    """
    ...
    def __init__(self, width: int, height: int, fps: float,
                 ...
                 codec: str = "h264", pipeline_depth: int = 4,
                 rate_mode: str = "constqp", la_depth: int = 0):
        # pipeline_depth = LA+1 per SDK hardware safety requirement
        if la_depth > 0 and pipeline_depth < la_depth + 1:
            pipeline_depth = la_depth + 1
        super().__init__(width, height, fps, preset=preset, qp=qp,
                         codec=codec, pipeline_depth=pipeline_depth,
                         rate_mode=rate_mode, la_depth=la_depth)
        # _output_slot_idx 由 base class __init__ 初始化 (line ~489)
        # [v5-FIX-FLAT] _pending_outputs 不需要 — 所有 drain 帧直接追加到扁平输出列表

    # _lock_bitstream_once_blocking() — 继承自 base class (line ~1019)
    # _drain_outputs() — 继承自 base class (line ~1069)，实现完全相同

    def encode_frames_sync_drain_loop(self, nv12_tensors, force_idr_first=False):
        """同步逐帧 drain 模式：每帧 encode → drain → 扁平追加到输出列表。

        [v5-FIX-FLAT] 返回扁平 H.264 列表（drain 产出顺序），不再 1:1 映射。
        与 nvenc_la_frame_conservation_suite.py MinimalTestEncoder 架构完全对齐：
        encode_frame() 返回 List[bytes] → 调用者 extend 累积，不维护 1:1 映射。
        """
        import ctypes as _ct
        n_frames = len(nv12_tensors)
        if n_frames == 0:
        ...
        try:
            with self._lock:
                ...
                # [v5-FIX-PERSLOT-IDR] 追踪每个 slot 是否已获得 IDR 首帧
                _slots_warmed_sd = set()
                ...
                for fi in range(n_frames):
                    ...
                    slot_idx = self._frame_idx % self._pipeline_depth
                    slot = self._slots[slot_idx]
                    # [v5-FIX-PERSLOT-IDR] 每个 slot 首次使用时强制 IDR，确保 DPB 引用链完整
                    force_idr = force_idr_first and (slot_idx not in _slots_warmed_sd)
                    ...
                    # ── EncodePicture (sync, NO completionEvent) ──
                    pic_buf = (_ct.c_uint8 * 3360)()
                    _ct.memset(pic_buf, 0, 3360)
                    _ct.cast(pic_buf, _ct.POINTER(_ct.c_uint32))[0] = NV_ENC_PIC_PARAMS_VER
                    _ct.cast(_ct.byref(pic_buf, 4), _ct.POINTER(_ct.c_uint32))[0] = _W
                    ...
                    _ep_s = ep(self._encoder, _ct.cast(pic_buf, _ct.POINTER(_ct.c_uint8 * 3360)))
                    self._frame_idx += 1
                    # [v5-FIX-PERSLOT-IDR] 标记该 slot 已获得 IDR 首帧
                    _slots_warmed_sd.add(slot_idx)

                    if _ep_s != NV_ENC_SUCCESS and _ep_s != NV_ENC_ERR_NEED_MORE_INPUT:
                        raise RuntimeError("EncodePicture[%d] failed: %d" % (slot_idx, _ep_s))

                    # ── ★ 关键：立即排空所有已完成的输出帧 ──
                    # [v5-FIX-FLAT] 直接追加到扁平输出列表，不维护 1:1 映射
                    _new_outputs = self._drain_outputs()
                    if _new_outputs:
                        all_outputs.extend(_new_outputs)
    ...
    def flush_eos_sync_drain(self):
        """EOS flush: 发送 EOS → 按 _output_slot_idx 顺序逐 slot blocking LockBitstream
        ...
        try:
            with self._lock:
                ...
                _ct.memset(pic_buf, 0, 3360)
                _ct.cast(pic_buf, _ct.POINTER(_ct.c_uint32))[0] = NV_ENC_PIC_PARAMS_VER
                _ct.cast(_ct.byref(pic_buf, 16), _ct.POINTER(_ct.c_uint32))[0] = 0x8  # EOS flag
                _ct.cast(_ct.byref(pic_buf, 40), _ct.POINTER(_ct.c_void_p))[0] = _ct.c_void_p(None)
                _ct.cast(_ct.byref(pic_buf, 48), _ct.POINTER(_ct.c_void_p))[0] = self._slots[0]['bs_buf']

                _NvEP = _ct.CFUNCTYPE(_ct.c_uint32, _ct.c_void_p,
                                      _ct.POINTER(_ct.c_uint8 * 3360))
                ep = _NvEP(self._func_ptrs[_FUNC_IDX["EncodePicture"]])
                ep(self._encoder, _ct.cast(pic_buf, _ct.POINTER(_ct.c_uint8 * 3360)))
                ...
                # [v5-FIX-FLAT] _pending_outputs removed — drain 帧已直接追加到批量输出列表
    ...
    def flush(self):
        """Override flush() to use EOS sync drain for correct LA frame recovery."""
        eos_frames = self.flush_eos_sync_drain()
        self._flush_frame_count = len(eos_frames)  # [v5-FIX-FLAT] 设置精确帧计数
        return b"".join(eos_frames)
...
TECHNIQUES = {
    'sync-batch': {
        ...
        'per_frame': False,
        'ce_info': 'none',
        'desc': 'sync batch (NO CE) [v5: drain loop + EOS flush fix]',
    },
    # 'single-ce': {
    ...
    #     'per_frame': True,
    #     'ce_info': 'per-frame',
    #     'desc': 'single-slot CE [v5: drain loop on slot 0] ⚠️ known issue: LA>0 loses ~LA frames (single-slot cannot drain multi-buffer LA output)',
    # },
    'batch-ce': {
        ...
        'per_frame': False,
        'ce_info': 'batch',
        'desc': 'batch per-frame CE [v5: drain loop + EOS flush fix]',
    },
    # 'phase4-slot': {
    ...
    'ce-pipeline': {
        ...
        'per_frame': False,
        'ce_info': 'deferred',
        'desc': 'CE-pipeline [v5: drain loop + EOS flush fix]',
    },
    # 'la-harvest': {
    ...
    #     'min_pipeline_depth': 4,
    #     'min_la_depth': 8,
    #     'desc': 'LA-Aware Harvest [v5: drain loop + EOS flush fix]',
    # },
    # 'ce-pipeline-v2': {
    ...
    # },
    'sync-drain-loop': {
        'encoder_cls': NVENCEncoderMode8,
        'encode_fn_name': 'encode_frames_sync_drain_loop',
        'per_frame': False,
        'ce_info': 'sync-drain',
        'desc': 'Sync drain loop — per-frame EncodePicture (no CE) + blocking LockBitstream drain until NEED_MORE_INPUT (reference implementation from nvenc_la_frame_conservation_suite.py)',
    },
}
...
# ============================================================================
def run_technique(tech_key, W, H, fps, rate_mode, la_depth, n_frames,
    ...
    _VERSION_MAP = {
        ...
        ('ce-pipeline', 4, 8): 'v6.4.3.1/4.1',      # VBR_HQ/QVBR + LA=8 + pipe=4 — 奇数小版本生产路径
        ('ce-pipeline', 1, 8): 'pipe1-safe-path',    # pipe=1 + LA=8 安全路径 (0% 空帧, 非生产默认)
        ('la-harvest', 4, 8): 'v6.4.x exp.',         # LA-Aware Harvest 实验性
        ...
        ('sync-drain-loop', 4, 0): 'v6.4.x-drain',   # sync-drain-loop: CONSTQP, 同步逐帧 drain
        ('sync-drain-loop', 4, 8): 'v6.4.x-drain',   # sync-drain-loop: VBR_HQ/QVBR + LA=8, 同步逐帧 drain
        ('sync-drain-loop', 9, 0): 'v6.4.x-drain',   # sync-drain-loop: CONSTQP, pd=LA+1=9
        ('sync-drain-loop', 9, 8): 'v6.4.x-drain',   # sync-drain-loop: VBR_HQ/QVBR + LA=8, pd=LA+1=9
    }
    # [v5-FIX] 计算实际 pipeline_depth（LA>0 时自动升级为 LA+1）
    _actual_pd = max(pipeline_depth, la_depth + 1) if la_depth > 0 else pipeline_depth

    version_tag = _VERSION_MAP.get((tech_key, pipeline_depth, la_depth), '')

    label = f"{tech_key}: pipe={_actual_pd} LA={la_depth} {preset} QP{qp}"
    _header = label_prefix if label_prefix else label
    print(f"\n{'='*60}")
    print(f"[{_header}]  {spec['desc']}")
    _upgrade_note = f" (requested pipe={pipeline_depth})" if _actual_pd != pipeline_depth else ""
    print(f"  {W}x{H}, {rate_mode}, {preset} QP{qp}, la={la_depth}, "
          f"pipe={_actual_pd}{_upgrade_note}, N={n_frames}")
    print(f"{'='*60}")

    # ── Encoder construction args ──
    _enc_kwargs = dict(
        width=W, height=H, fps=fps, preset=preset, qp=qp,
        rate_mode=rate_mode, la_depth=la_depth,
        pipeline_depth=pipeline_depth,
    )

    # ── Fresh encoder for measurement ──
    enc = EncoderCls(**_enc_kwargs)
    empty_count = 0
    empty_frames = []
    la_count = 0
    success_count = 0
    ...
    for batch_start in range(0, n_frames, batch_size):
        ...
        if per_frame:
            ...
            # [v5-FIX5/6] encode_frame_with_ce now returns List[bytes]
            for i, nv12 in enumerate(batch_tensors):
                fi = batch_start + i
                frame_outputs = _encode_func(nv12)
                if not frame_outputs:
                    # Empty list from NEED_MORE_INPUT
                    ...
                    success_count += len(frame_outputs)
        else:
            # ★ Batch encode (all other techniques) ★
            # [v5-FIX-FLAT] 扁平输出：results 是 List[bytes]，按 drain 产出顺序，
            # 不包含 None/b"" 占位符。帧守恒通过 total_recovered == n_frames 验证。
            results = _encode_func(batch_tensors,
                                   force_idr_first=(batch_start == 0))
            for h264_data in results:
                # 防御性检查（扁平模式不应出现 None 或空字节）
                if h264_data is None or h264_data == b"":
                    continue
    ...
    # [v5-FIX] EOS flush frames ARE valid output frames — they're the LA-delayed tail
    # frames recovered by the blocking drain loop. They contain proper IDR/DPB context
    # and are independently decodable. Add them to the total recovered count.
    enc.close()
    torch.cuda.synchronize()
    t_elapsed = time.time() - t_start


    # ── [v5-FIX] Frame conservation accounting ──
    # NVENC SDK: "The total number of output frames equals the number of input frames."
    # With correct drain loop + EOS flush, ALL input frames produce output.
    # LA-buffered frames are NOT "consumed" — they're delayed and recovered via drain/flush.
    _submitted = n_frames
    _total_recovered = success_count + flush_frames  # encode outputs + EOS flush recovery
    _lost = max(0, _submitted - _total_recovered)
    if _lost > 0:
        print(f'  [LOSS] {_lost} frames UNRECOVERABLE '
              f'(submitted={_submitted} recovered={_total_recovered} '
              f'encode_valid={success_count} LA_buf={la_count} '
              f'empty={empty_count} flush_frames={flush_frames} '
              f'flush_bytes={flush_bytes})', flush=True)
    elif la_count > 0 or flush_frames > 0:
        print(f'  [CONSERVATION] ✅ Frame conservation VERIFIED '
              f'(submitted={_submitted} recovered={_total_recovered} '
              f'encode_valid={success_count} LA_buf={la_count} '
              f'flush_frames={flush_frames} empty={empty_count})', flush=True)
    ...
    extra = {
        ...
        'total_recovered': _total_recovered,  # [v5] includes EOS flush frames
        'expected_frames': _submitted,         # [v5] ALL input frames should produce output
        'la_buffered': la_count,               # frames buffered by LA (not lost, just delayed)
        'flush_frames': flush_frames,          # [v5] valid frames recovered by EOS drain
...
def main():
    ...
    ap.add_argument('--no-warmup', action='store_true',
                    help='跳过 GPU 预热（默认会先用短视频预热 NVENC + CUDA context）')
    ap.add_argument('--warmup-duration', type=int, default=10,
                    help='预热视频时长（秒，默认 10）。仅当 --input 提供时生效')
    args = ap.parse_args()
    ...
    # ── Build valid test matrix: filter phase4 × pipe=1, la-harvest always excluded ──
    _test_combos = []
    for tech_key in TECHNIQUES:
        ...
        # la-harvest permanently excluded (experimental dead-end)
        if tech_key == 'la-harvest':
            continue
        for pipe in _Pipes:
            if pipe < _min_pd:
                continue
    ...
    # ── [v5-CONSOLIDATE] LA>0 + VBR_HQ/QVBR: pipe自动归一化为LA+1, 所有pipe值等价 → 整合去重 ──
    _consolidation_modes = [rm for rm in _rate_modes if rm in ('vbr_hq', 'qvbr')]
    _consolidated_skip = 0
    _consolidated_details = []  # (tech_key, la, pipes_list)
    if _consolidation_modes:
        _consol_seen = {}
        for tech_key, pipe, la in _test_combos:
            if la > 0:
                _ckey = (tech_key, la)
                if _ckey in _consol_seen:
                    _consolidated_skip += 1
                    _consol_seen[_ckey].append(pipe)
                else:
                    _consol_seen[_ckey] = [pipe]
        for _ckey, _pipes in _consol_seen.items():
            if len(_pipes) > 1:
                _consolidated_details.append((_ckey[0], _ckey[1], sorted(set(_pipes))))
    _consolidated_skip_total = _consolidated_skip * len(_consolidation_modes)

    print("=" * 70)
    print("NVENC completionEvent 4D Full-Matrix Verification v5")
    ...
    _total_rounds_all = _total_rounds * len(_rate_modes) - _consolidated_skip_total
    ...
    if _consolidated_skip_total > 0:
        print(f"Total rounds: {_total_rounds_all} ({_total_rounds} combos × {len(_rate_modes)} RC modes − {_consolidated_skip_total} consolidated)")
        print(f"  ═══ Consolidation: LA>0 + VBR_HQ/QVBR → pipe自动归一化为LA+1 ═══")
        for _tech, _la, _pipes in _consolidated_details:
            _pipes_str = '/'.join(str(p) for p in _pipes)
            print(f"  📦 {_tech}: LA={_la} → pipe归一化为LA+1={_la+1}, 原pipe=[{_pipes_str}] 整合为1条")
        print(f"  ℹ️  每个 (technique, LA) 仅保留首个pipe入口参与测试，{len(_consolidation_modes)}个RC模式({', '.join(_consolidation_modes)})均适用此归一化规则")
    else:
        print(f"Total rounds: {_total_rounds_all} ({_total_rounds} combos × {len(_rate_modes)} RC modes)")
    # ── 预热信息 ──
    if args.no_warmup:
        print(f"GPU warmup: DISABLED (--no-warmup)")
    elif args.input:
        print(f"GPU warmup: ENABLED ({args.warmup_duration}s clip from input → NVENC constqp/pipe=1/LA=0 → flush+close+clean)")
    else:
        print(f"GPU warmup: ENABLED (synthetic {200} frames → NVENC constqp/pipe=1/LA=0 → flush+close+clean)")
    print("=" * 70)
    ...
    if not args.no_warmup:
        ...
        warmup_video = None
        warmup_output_dir = pathlib.Path(output_dir)

        if args.input:
            # ── 真实视频模式: 从输入视频截取预热片段 ──
            warmup_video = warmup_output_dir / f"_warmup_{video_name}.mp4"
            try:
                _p = subprocess.run([
                    "ffmpeg", "-y",
                ...
                if _p.returncode == 0 and warmup_video.exists():
                    ...
                    if _p2.returncode != 0 or not warmup_video.exists():
                        print(f"  预热视频生成失败，回退到合成帧预热")
                        warmup_video = None
            except Exception as e:
                print(f"  预热视频生成异常: {e}，回退到合成帧预热")
                warmup_video = None

        # ── 执行预热编码 ──
        # 目标帧数按 warmup_duration × FPS；_read_video_frames_rgb 会自然截断到实际时长。
        # 不再设人工上限 — 由 warmup_duration 精确控制预热时长。
        warmup_frames = None
        _warmup_target = int(args.warmup_duration * FPS)
        warmup_n = _warmup_target

        if warmup_video and warmup_video.exists():
            try:
                # count=_warmup_target: 最多读取目标帧数；视频短于此值时自动返回全部实际帧
                warmup_frames = _read_video_frames_rgb(
                    str(warmup_video), start=0, count=_warmup_target, w=W, h=H)
                warmup_n = len(warmup_frames)
                if warmup_n < _warmup_target:
                    _actual_sec = warmup_n / max(1, FPS)
                    print(f"  预热帧: {warmup_n} frames / {_actual_sec:.1f}s "
                          f"(视频短于 {args.warmup_duration}s, 按实际时长预热)")
                else:
                    print(f"  预热帧: {warmup_n} frames / {args.warmup_duration}s (from video clip)")
            except Exception as e:
                print(f"  预热视频帧读取失败: {e}")
                warmup_frames = None

        # 合成帧回退（无 --input 时用 GPU tensor 预热）
        if warmup_frames is None:
            warmup_n = 200
            print(f"  预热帧: {warmup_n} frames (synthetic GPU tensors)")

        print(f"  预热配置: constqp, pipe=1, LA=0, QP={QP}, preset={PRESET}")
        print(f"  预热中...", end=" ", flush=True)

        t_warmup_start = time.perf_counter()
        _warmup_ok = False
        try:
            # 使用最简配置: constqp + pipe=1 + LA=0，确保稳定初始化/销毁
            _warmup_enc = NVENCEncoder(
                W, H, FPS,
                preset=PRESET, qp=QP,
            ...
            # 准备帧数据
            _warmup_nv12_list = []
            for fi in range(warmup_n):
                if warmup_frames and fi < len(warmup_frames):
                    rgb = warmup_frames[fi].contiguous()
                else:
                    rgb = _make_test_rgb(H, W, fi)
                nv12 = _rgb_to_nv12_gpu(rgb, input_is_bgr=False)
                _warmup_nv12_list.append(nv12)
            torch.cuda.synchronize()

            # 编码
            _warmup_result = _warmup_enc.encode_frames_batch(
                _warmup_nv12_list, force_idr_first=True)

            # Flush + close — 完整清理 NVENC encoder 及所有资源
            _warmup_enc.flush()
            _warmup_enc.close()

            # ── 彻底清理预热产生的临时状态 ──
            del _warmup_nv12_list
            del _warmup_enc
            if warmup_frames:
                del warmup_frames
                warmup_frames = None
            ...
            _warmup_ok = True
            t_warmup_elapsed = time.perf_counter() - t_warmup_start
            print(f"完成 ({t_warmup_elapsed:.1f}s)")
            print(f"  NVENC encoder 已构建/初始化/销毁，CUDA context 已预热并清理")
        except Exception as e:
            print(f"失败: {e}")
    ...
    for _rm in _rate_modes:
        # ── [v5-CONSOLIDATE] LA>0 + VBR_HQ/QVBR: 每(technique, LA)仅首个pipe入口参与测试 ──
        _consol_seen = set()
        for tech_key, pipe, la in _test_combos:
            if la > 0 and _rm in ('vbr_hq', 'qvbr'):
                _consol_key = (tech_key, la)
                if _consol_key in _consol_seen:
                    _actual_pd = max(pipe, la + 1)
                    print(f"\n  ⏭️  SKIP [{_rm}] {tech_key}: pipe={pipe} LA={la} → 归一化后pipe={_actual_pd}=LA+1"
                          f" (与已保留的测试条目等价，整合跳过)")
                    continue
                _consol_seen.add(_consol_key)
            _round += 1
            ...
            if _use_seg:
                ...
                _run(_label, run_technique, tech_key, W, H, FPS, _rm, la,
                     N_FRAMES, pipeline_depth=pipe, preset=PRESET, qp=_crf,
                ...
                try:
                    ...
                    for bi in range(0, _topn_frame_count, bs):
                        ...
                        if _per_frame:
                            ...
                            # [v5-FIX-FLAT] 扁平输出：_res 是 List[bytes]，不包含 None/b""
                            _res = _ef(_batch, force_idr_first=(bi == 0))
                            for _d in _res:
                                if _d is None or _d == b"":
                                    continue  # 防御性跳过（扁平模式不应出现）
                    ...
                    # [v5-FIX-FLAT] 正确的 drain loop + blocking EOS flush 回收的是
                    # 完整的 LA 滞留帧（含 SPS/PPS/IDR），可独立解码，应写入输出。
                    _flush = _enc.flush()
                    if _flush and len(_flush) > 0:
                        _flush_frame_count = getattr(_enc, '_flush_frame_count', 0)
                        print(f'  [NVENC-FLUSH] {len(_flush)} bytes drained '
                              f'({_flush_frame_count} frames, appended to output)', flush=True)
                        # 将 flush 数据作为额外输出帧追加（它们按 drain 顺序）
                        output_data.append(_flush)
    ...
    if topn_stats:
        ...
        print(f"TOP-N OUTPUT FILES — Detailed Statistics  "
              f"[{len(_perfect)}/{len(topn_stats)} perfect: Output=Container=Input]")
        if _imperfect > 0:
            print(f"  ({_imperfect} entries have frame-count mismatch "
                  f"— bitstream errors or encoding failures)")
        print("=" * 135)
