...
| PASS | R2 | FFmpeg 可用（≥4.3） | which + 版本查询 | ffmpeg 7.1 @ /usr/local/bin/ffmpeg |
| PASS | R3 | FFprobe 可用 | shutil.which | ffprobe @ /usr/local/bin/ffprobe |
...
| PASS | BEH-B5 | 重编码合并成功 | 行为执行 | （⚠️[merge] 告警为预期触发，已捕获：⚠️  [merge] 输出容器与编码策略不匹配，实际输出: /tmp/tmpe4sp5aw6/out.mp4 (请求: /tmp/tmpe4sp5aw6/out.mkv) |
| PASS | BEH-B6 | actual_output 已回传 | 行为执行 | actual=['/tmp/tmpe4sp5aw6/out.mp4'] |
...
| 状态 | ID | 验证项 | 方法 | 详情 |
|---|---|---|---|---|
| PASS | BEH-E1 | py_compile ×116 全通过 | 行为执行 | 116 个活跃文件语法全部通过（按目录自动收集） |
| PASS | BEH-E2 | 覆盖清单自检（116 文件，下限 95） | 行为执行 | 自动收集 ✓ / FILES 具名文件全覆盖 ✓ / external 无未纳管包 ✓ / tests/ 全覆盖（63 个）✓；无排除文件 |

## C-行为·调用契约

| 状态 | ID | 验证项 | 方法 | 详情 |
|---|---|---|---|---|
| PASS | BEH-H1 | 实参名被目标签名接受（116 文件 / 251 调用点） | 行为执行 | 116 个文件、251 个子进程/run_async 调用点的字面量参数名全部合法 |
