...
| PASS | BEH-B5 | 重编码合并成功 | 行为执行 | （⚠️[merge] 告警为预期触发，已捕获：⚠️  [merge] 输出容器与编码策略不匹配，实际输出: /tmp/tmphwtiiwhw/out.mp4 (请求: /tmp/tmphwtiiwhw/out.mkv) |
| PASS | BEH-B6 | actual_output 已回传 | 行为执行 | actual=['/tmp/tmphwtiiwhw/out.mp4'] |
...
| 状态 | ID | 验证项 | 方法 | 详情 |
|---|---|---|---|---|
| PASS | BEH-H1 | 实参名被目标签名接受（136 文件 / 254 调用点） | 行为执行 | 136 个文件、254 个子进程/run_async 调用点的字面量参数名全部合法 |
