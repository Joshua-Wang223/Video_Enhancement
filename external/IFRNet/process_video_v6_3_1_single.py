"""
IFRNet 视频插帧处理脚本 —— 终极优化版 v6.3.1（单卡版）
==========================================================
...
class GPUMonitor:
    ...
    def print_report(
        ...
        # ── result_queue（T3 输出缓冲）建议 ─────────────────────────────────
        if stats.stable_p95 > 85.0 and stable_std > 20.0:
    ...
    def get_queue_suggestions(
        ...
        """
        返回 (建议 pair_queue, 建议 result_queue)，与 print_report 规则完全一致。
        用于跨段自适应队列调整，不打印信息。
        """
        ...
        # result_queue 建议
        if stable_p95 > 85.0 and stable_std > 20.0:
...
class IFRNetVideoProcessor:
    ...
    def process_video(
        ...
        if _gpu_stats.sample_count > 0:
            ...
            final_result_q = max(
                min(result_gpu_sug,
                retune_result_q if retune_result_q is not None else 0),
                _cur_result_q
            )
            # 限制合理上限
...
def main():
    parser = argparse.ArgumentParser(
        description='IFRNet 视频插帧 —— 终极优化版 v6.3.1（单卡版）',
    ...
    print('=' * 65)
    print('  IFRNet 视频插帧 —— 终极优化版 v6.3.1（单卡版）')
    print('=' * 65)
