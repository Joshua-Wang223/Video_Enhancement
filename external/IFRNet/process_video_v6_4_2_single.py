"""
IFRNet 视频插帧处理脚本 —— 优化版 v6.4.2（单卡版）
==========================================================
基于 IFRNet（Intermediate Flow-based Recursive Network）的视频帧插值脚本，
面向单 GPU 生产环境的高性能实现。

【v6.4.2 新增优化（基于 v6.3.5）】
  [FIX-T3-V642]      Pinned Ring Buffer 零拷贝体系:
                   · 新增 PinnedRingBuffer 类: N 槽预分配 pinned memory 环形缓冲，
                     替代 PinnedResultPool + _PinnedResultItem + result_queue 三层结构。
                   · T2 推理结果 GPU 端裁剪后 D2H 直写 ring buffer slot（无 padding），
                     每帧数据在 pinned memory 中完全连续。
                   · T3 通过 memoryview 零拷贝读取 slot 后直接 write() 到 FFmpeg
                     stdin pipe，Python 层不再创建 numpy array / bytes 对象。
                   · FFmpegWriter 新增 write_memoryview() 方法，接受 memoryview
                     直接写入 pipe，消除 _queue 队列 + _write_loop 线程开销。
                   · 帧数据路径从 5 步降至 2 步: GPU→D2H→pinned_slot→memoryview→pipe
...
# ─────────────────────────────────────────────────────────────────────────────
# [FIX-T3-V642] Pinned Ring Buffer —— 锁页内存环形缓冲区（零拷贝体系）
# ─────────────────────────────────────────────────────────────────────────────
...
class IFRNetPipelineRunner:
    ...
    def _writer_loop(self, writer, pbar, n_seg_est, meter, timing_ref):
        ...
        try:
            while self.running or not self.result_queue.empty():
                ...
                # [FIX-T3-V642] 批量预读取 + 分类处理（ring buffer / legacy pool / fallback）
                items = [item]
                for _ in range(3):
                    try:
                ...
                # 批量等待所有 D2H DMA 完成（ring buffer + legacy pool）
                _has_sentinel = False
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) == 2 and _item[0] == 'RING':
                        # ring buffer: event 在 metadata 中，reader_acquire 后 synchronize
                        pass
                ...
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) == 2 and _item[0] == 'RING':
                        ...
                        # ── Legacy PinnedResultPool 路径（保留兼容）──
                        n_pairs = _item.B
    ...
    def run(
        ...
        # [FIX-T3-V642] 创建 PinnedRingBuffer（替换 PinnedResultPool）
        # [SEGMENT-REUSE] 通过缓存方法获取，跨段复用
        _ring_ok = False
        if (H_pad > 0 and W_pad > 0
                and getattr(proc, 'stream_d2h', None) is not None   # [STREAM-DUAL]
                and proc.device.type == 'cuda'):
            _max_BT    = effective_bs * len(timesteps)
            _ring_size = self.result_queue.maxsize + 2
            try:
                # [FIX-T3-V642] 使用原始分辨率（无 padding），每帧连续存储
                proc._ring_buf, _ring_created = proc._get_or_create_ring_buf(
                    _ring_size, _max_BT, H, W)
                _ring_ok = True
            except Exception as _pe:
                print(f'[IFRNet-Pipeline] PinnedRingBuffer 分配失败: {_pe}，回退同步 D2H',
                      flush=True)
                proc._ring_buf = None
        ...
        if proc._is_first_segment():
            print(
                ...
                f'T={len(timesteps)}× | '
                f'D2H={"pinned(ring)" if _ring_ok else "sync"}',
                flush=True,
            )
        elif not proc.quiet:
            print(
                f'[IFRNet-Pipeline] 段 {proc._segment_index}: 复用流水线参数 '
                f'(pair={self.pair_queue.maxsize} result={self.result_queue.maxsize} '
                f'bs={effective_bs} T={len(timesteps)}×)',
                flush=True,
            )
        ...
        # 持有（全局唯一），段间不释放。只清空段级引用，池实例存活至 cleanup()。
        # 无累积问题：全局只有一个 buffer，不会像旧代码那样每段创建新 buffer 叠加。
        proc._ring_buf = None
    ...
    def _infer_loop(
        ...
        try:
            while self.running:
                ...
                # [FIX-T3-V642] 处理 ring buffer 返回的轻量句柄
            ...
            # [FIX-POOL-LEAK] Ring buffer cleanup moved to run() after both threads are joined,
            # to avoid race between free() and the writer thread still draining result_queue.
...
class FFmpegWriter:
    ...
    def __init__(
        ...
        # [FIX-T3-V642] 去除内部 _queue 和 _write_loop 线程，直接管道写入
        self._error: Optional[Exception] = None
        self._write_count = 0
        ...
        self._stderr_thread = threading.Thread(target=self._drain_stderr, daemon=True)
        self._stderr_thread.start()
        # [FIX-T3-V642] 不再启动内部 _write_loop 线程，改为 write_direct() 直接写管道
    ...
    def write_direct(self, data):
        """[FIX-T3-V642] 直接写 bytes/memoryview 到 FFmpeg stdin pipe，零中间拷贝。"""
        if self._error is not None:
    ...
    def write(self, frame):
        """兼容旧接口：numpy array → tobytes() → write_direct()。"""
...
class IFRNetVideoProcessor:
    ...
    def __init__(
        ...
        self._ring_buf:     Optional[PinnedRingBuffer]     = None
        ...
        # [SEGMENT-REUSE] PinnedRingBuffer 缓存（无 NVENCEncoder）
    ...
    def _get_or_create_ring_buf(self, num_slots: int, max_frames_per_slot: int,
        ...
        ring = PinnedRingBuffer(num_slots, max_frames_per_slot, H, W)
    ...
    def _process_segment(
        ...
        if 'nvenc' in use_codec and self._is_first_segment():
    ...
    def process_video(
        ...
        # [SEGMENT-REUSE] 仅在最后一段输出报告，中间段静默计算队列建议
