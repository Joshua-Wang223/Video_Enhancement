"""
IFRNet 视频插帧处理脚本 —— 四级回退统一编码版 v6.4.5（单卡版）
============================================================
...
【v6.4.5 新增修复（基于 v6.4.4）】
  ...
  [PHASE4-CE-PIPELINE] encode_frames_batch_ce_pipeline() — per-frame CE 异步流水线：
               ...
               · Phase 1: Harvest 上一轮 pending CE + LockBitstream
               · Phase 2: LockInputBuffer → cuMemcpy2D → EncodePicture + 新建 CE
               · Phase 3: 批次结束 drain 所有 pending slots
               · GPU 验证 (T4, 720×576, VBR_HQ, pipe=4, LA=0):
                 523 FPS vs sync-batch 375 FPS (+39.5%)，0% 空帧。
               · _NVENCEncodeThread._loop() 已切换默认使用此路径。
               · Tier 1-B retry (encode_frame fallback) + Tier 1-A fallback (prev_h264)。
               · 从 v6.4.5.1 移植: encode_queue_depth 2→4, flush 帧数精确统计,
                 None/b""/bytes 三态空帧区分。
...
# ═══════════════════════════════════════════════════════════════════════════════
# [CONSTQP-ONLY] Level 1 NVENC 编码固定使用 CONSTQP 模式。
# CONSTQP 已 GPU 验证为最快(+29~66% vs vbr_hq)、最稳(零空帧)、文件最小。
# ═══════════════════════════════════════════════════════════════════════════════
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        """[CONSTQP-ONLY] NVENC GPU 直通编码器，固定使用 CONSTQP 码率控制。"""
        ...
        self._fps = fps
        self._qp = qp
        # [CONSTQP-ONLY] 硬化 CONSTQP，不再支持 VBR_HQ/QVBR/LA。

        print("[NVENCEncoder] CONSTQP: pipeline_depth=%d" % pipeline_depth, flush=True)
    ...
    def set_muxer_ref(self, muxer: object) -> None:
        """[FIX-SPS-PPS] 建立 muxer 回引用，用于 SPS/PPS V2 Writer-thread-side 注入。"""
    ...
    # batch 版（v6.4.5）CONSTQP-only、_la_depth 恒 0，rotation 为纯死代码
    ...
    def _lock_bitstream_with_retry(self, bs_handle, max_retries: int = 5, backoff_us: int = 1000):
        ...
        # [CONSTQP-ONLY] CONSTQP 零空帧 → 固定重试上限=2
                    ...
                    if not h264_data:
                        ...
                        if self._diag_empty <= 5 or self._diag_empty % 50 == 0:
                            print(f'[NVENC-Enc] ⚠️ 空帧 #{self._diag_empty} (batch fi={fi}/{n_frames} '
                                  ...
                                  f'bs_status={bs_status} frame_idx={_fidx}) '
                                  f'nv12_mean={_nv12_mean:.1f} std={_nv12_std:.1f} '
                                  f'min={_nv12_min} max={_nv12_max}', flush=True)
                        results.append(None)
                        continue

                    # [SEGMENT-REUSE] SPS/PPS caching and pre-pending (per-slot IDR aware)
    ...
    def encode_frames_batch_ce_pipeline(self, nv12_tensors: list, force_idr_first: bool = False) -> list:
        ...
        try:
            with self._lock:
                ...
                # [FIX-PIPE4-LA8] fi==0 only force_idr (不再 per-slot IDR)
                ...
                for slot_idx in range(pd):
                    if _slot_pending[slot_idx] is not None:
                        _pending_ce, _pending_fi, _pending_ep_s, _pending_idr = \
                        ...
                        h264_data, bs_status = self._lock_bitstream_with_retry(
...
class _NVENCEncodeThread:
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 4,
        ...
        self._writer  = writer
        # [FIX-LA-QUEUE-DEPTH] LA>0 encoding benefits from deeper queue to buffer
        # batches during chunk encoding. Explicitly-passed depth is always honored.
    ...
    def _loop(self):
        ...
        # [FIX-SPS-PPS-V2] 跨段重置换，支持 encoder 复用
        ...
        while True:
            ...
            try:
                ...
                # [FIX-LA-WINDOW-ROTATION] 防御性接入: CONSTQP-only（_la_depth=0）
                # 时走快速路径直通，零开销；未来启用 LA 时自动生效。
                # [FIX-STREAM-TS-REASSOC] 原 [FIX-LA-WINDOW-ROTATION] 防御性接入
                # 已移除（batch 版 CONSTQP-only、_la_depth 恒 0，永为直通）。
                _iter = enumerate(h264_list)
                for _pos, h264_data in _iter:
                    if h264_data is None:
                        # LockBitstream 超时空帧: Tier 1-A fallback (CONSTQP 跳过 Tier 1-B retry)
                        pass
                    if h264_data is None:
                        # 重试耗尽: Tier 1-A fallback (复制前一帧)
                        self._empty += 1
                        ...
                        # lookahead 缓冲空帧: 帧已被编码器接受，等待 lookahead 窗口填满
                        # 不写入 muxer，不计数为 empty
                        pass
                    else:
        ...
        # 确保 flush 与 encode_frames_batch 在同一 CUDA context 内执行，
        # 消除跨线程 context 栈差异导致的静默 flush 失败。
        # LA flush 返回的 bytes 是 NVENC 内部流水线残片，解码后为花屏/重复帧；
        # 仅做 EOS 清理，不写入 muxer 也不计入 written 计数。
        try:
            self._nvenc.flush()
        except Exception:
            pass
...
class IFRNetPipelineRunner:
                ...
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) >= 2 and _item[0] == 'GPU_RAW':
                        ...
                        if _enc_thread is None:
                            _enc_thread = _NVENCEncodeThread(_nvenc, writer, batch_frames=len(encode_order))  # [FIX-CHUNK-SAFE]
    ...
    def run(
        ...
        if self.auto_tune and H_pad > 0 and W_pad > 0:
            ...
            # from result_queue. Add headroom to prevent T2 blocking during encoding bursts.
    ...
    def __init__(
        ...
        if 'nvenc' in codec:
            ...
            if crf == 0:
                ...
                _rc_label = rc_mode.upper() if rc_mode in ('vbr_hq', 'qvbr') else 'VBR'
                _nvenc_info = (
                    f'[FIX-NVENC-PIPE] NVENC {_rc_label}(cq={crf}): '
                    f'preset={preset}  bf=0  '
...
class IFRNetVideoProcessor:
    ...
    def _get_or_create_nvenc_encoder(self, W: int, H: int, fps: float,
                                      preset: str, qp: int,
                                      pipeline_depth: int = _NVENC_LEVEL1_PIPELINE_DEPTH,
        ...
        key = (W, H, fps, preset, qp, pipeline_depth, codec)
    ...
    def _process_segment(
        ...
        _expected_out = int(frame_count * scale) - 1
        _out_loss = _expected_out - output_count
        if _out_loss == 0:
            _out_disp = f'{output_count}'
        else:
            _out_disp = f'{output_count} (期望≈{_expected_out}, 差{_out_loss})'
