...
【v6.4.4.2 新增修复（基于 v6.4.4）】
  ...
  [PHASE4-v645] 恢复多 slot 异步流水线 encode_frames_batch()：
               · v6.4.4 因 multi-slot 花屏问题禁用多 slot 架构；
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        Since v6.4.4.2: multi-slot ce-pipeline for 4-slot async encoding."""
        ...
        self._fps = fps
        self._qp = qp
        self._rate_mode = 'constqp'  # v6.4.4 is CONSTQP-only
    ...
    # batch 版（v6.4.4）CONSTQP-only、_la_depth 恒 0，rotation 为纯死代码
    ...
    def _lock_bitstream_with_retry(self, bs_handle, max_retries: int = 2, backoff_us: int = 500):
        ...
        底层竞态。[V6442-CONSTQP-FAST] CONSTQP 零空帧 → max_retries=2。
    ...
    def encode_frames_batch_ce_pipeline(self, nv12_tensors: list, force_idr_first: bool = False) -> list:
        ...
        Returns list where: bytes = valid H.264, None = empty.
        GPU 验证 (T4, 720×576, CONSTQP, pipe=4): 612 FPS vs sync-batch 519 FPS (+18%)。
        """
        n_frames = len(nv12_tensors)
        if n_frames == 0:
            return []
...
class _NVENCEncodeThread:
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 4,
        ...
        self._writer  = writer
        # [FIX-LA-QUEUE-DEPTH] LA>0 encoding benefits from deeper queue.
        # Explicitly-passed depth is always honored.
            ...
            try:
                ...
                for _pos, h264_data in _iter:
                    if h264_data is None:
                        # [V6442-CONSTQP-FAST] CONSTQP 零空帧 → 直接 fallback
                        self._empty += 1
...
class IFRNetPipelineRunner:
    ...
    def run(
        ...
        if self.auto_tune and H_pad > 0 and W_pad > 0:
            ...
            # Add headroom to prevent T2 blocking during encoding bursts.
