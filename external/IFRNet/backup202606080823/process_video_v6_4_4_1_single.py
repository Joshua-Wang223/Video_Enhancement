...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        print("[NVENCEncoder] Ready: %dx%d@%.1ffps H.264 CONSTQP QP=%d preset=%s (GPU direct SDK 13.0)" %
              (width, height, fps, _qp_val, self._preset_name), flush=True)
...
#   0  — 不偏移（文件约 4× 于 CONSTQP）
#   7  — CRF 21→28 (文件大小接近 CONSTQP 同 QP)
#   3  — 折中：文件约 2×，质量更高
