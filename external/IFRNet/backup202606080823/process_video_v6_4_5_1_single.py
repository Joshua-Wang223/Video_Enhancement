...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        if self._rate_mode == 'vbr_hq':
            ...
            rc_ptr[1] = 4                            # NV_ENC_PARAMS_RC_VBR_HQ
            # constQP fields @rc_ptr[2..4] — VBR_HQ ignores, left as preset defaults
            rc_ptr[5] = 0                            # averageBitRate @offset 20 (CQ: quality-driven, no bitrate target)
            rc_ptr[6] = 0                            # maxBitRate @offset 24
            rc_ptr[29] = min(51, _qp_val + _NVENC_VBR_QUALITY_OFFSET)  # targetQuality @offset 116
            ...
            rc_ptr[31] = min(51, _qp_val + _NVENC_VBR_QUALITY_OFFSET)  # qvbrQuality @offset 124
        else:
                ...
                for fi in range(n_frames):
                    ...
                    if bitstream_size == 0:
                        ...
                        if self._diag_empty <= 5 or self._diag_empty % 50 == 0:
                            print(f'[NVENC-Enc] ⚠️ 空帧 #{self._diag_empty} (fi={fi}, slot={slot_idx})', flush=True)
...
#
#   "vbr_hq" — NV_ENC_PARAMS_RC_VBR_HQ (mode=4) + targetQuality (CQ 模式)
...
# VBR_HQ/QVBR 的 targetQuality/qvbrQuality 量表与 CONSTQP QP 量表不同。
# 实测数据（T4, 720p, IFRNet 2x）：
#   CONSTQP QP=21 → ~8.5MB,   VBR_HQ targetQuality=21 → ~36.9MB (4.3×)
#   CONSTQP QP=23 → ~6.5MB,   VBR_HQ targetQuality=23 → ~28MB   (4.3×)
#
# 偏移量将 CRF 值映射到等效的 VBR targetQuality 以保持可比文件大小：
#   targetQuality = min(51, CRF + _NVENC_VBR_QUALITY_OFFSET)
#
#   0  — 不偏移（当前行为，文件约 4× 于 CONSTQP）
#   7  — CRF 21→28, CRF 23→30 (文件大小接近 CONSTQP 同 QP)
#   3  — 折中：文件约 2×，质量更高
#
# QVBR 额外需要 vbvBufferSize(@+28) 和 vbvInitialDelay(@+32)，当前自动设为 4MB/2MB。
_NVENC_VBR_QUALITY_OFFSET: int = 7
