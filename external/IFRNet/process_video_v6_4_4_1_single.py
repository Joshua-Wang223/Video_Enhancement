    ...
    def encode_frames_batch(self, nv12_tensors: list, force_idr_first: bool = False,
        ...
        try:
            with self._lock:
                ...
                # [FIX-LA-DRAIN-SYNC] self._batch_slot_pending maps drained output to actual frame index.
                # [FIX-LA-CHUNK-CROSS] 提升为实例变量 self._batch_slot_pending，
                # 使跨 chunk 的 drain 映射在 send_eos=False → send_eos=True 调用间保持连续。
...
class _NVENCEncodeThread:
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 4,
        ...
        # [FIX-FLUSH-GRANULARITY] backport from v6.4.5.2: 把"触发编码"的粒度从安全上限
