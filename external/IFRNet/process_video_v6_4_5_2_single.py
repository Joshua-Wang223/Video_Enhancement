"""
IFRNet 视频插帧处理脚本 —— 四级回退统一编码版 v6.4.5.2（单卡版）
============================================================
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        self._strm_slot_pending: list = [None] * _slot_count  # (fi, bs_buf, force_idr, ep_status)
        self._strm_slots_warmed: set = set()
    ...
    def _drain_outputs_blocking(self, max_slots: int = None) -> list:
        ...
        for _ in range(max_slots):
            ...
            if bitstream_ptr_val:
                ...
                # 估算 frame_index: 当前 output_slot_idx 对应的帧序号
    ...
    def _stream_begin(self, force: bool = False):
        ...
        下一段首块前必须重新 begin，否则 _strm_slots_warmed 残留会导致
        新段首帧不再强制 IDR（DPB 跨段污染，花屏）。
        """
        if force or not self._strm_active:
            self._strm_slot_pending = [None] * self._slot_count
            self._strm_slots_warmed = set()
        ...
        try:
            with self._lock:
                ...
                for _i in range(n_frames):
                    ...
                    # 参考 realesrgan_video _ensure_slot_free() 设计。
                    _drain_guard = 0
                    ...
                    self._strm_slot_pending[slot_idx] = (fi, slot['bs_buf'], force_idr, _ep_status)
                    ...
                    if _ep_status == NV_ENC_ERR_NEED_MORE_INPUT:
                        ...
                        self._strm_slots_warmed.add(slot_idx)
                    ...
                    for _est_fi, _h264_data in _drained:
                        ...
                        _pending = self._strm_slot_pending[_drain_slot]
                        if _pending is None:
                            continue
                        _actual_fi, _, _is_idr, _ep_s = _pending

                        if not _h264_data:
                            # 空帧诊断（防御分支，_drain_outputs_blocking 正常不产出空数据）
...
class _NVENCEncodeThread:
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 4,
        ...
        # [FIX-FLUSH-GRANULARITY] v6.4.5.2: 原设计"攒够 768 帧才编码一整块"
    ...
    def _loop(self):
        ...
        # 现 NVENCEncoder 的 slot 未决表/IDR 热身表已提升为实例持久状态
        ...
        def _drain_write(pairs):
            ...
            随后按流内帧号顺序写入，空帧执行 Tier 1-A 前一帧补偿。"""
        ...
        while True:
            ...
            if _la_mode:
                ...
                if _first_batch:
                    # 每段新流：重置 slot 未决表 / per-slot IDR 热身表 / 流内帧号
                    ...
                    print(f'[NVENC-Enc] LA={self._nvenc._la_depth} 分块流式编码: '
