...
【v6.4.3 Phase 1-3 修复（GPU-STAY 性能退化）】
  ...
  [PHASE4-CE-PIPELINE] NVENC 多 slot 异步流水线 (v6.4.3.1+ce-pipeline)：
               ...
               · [V6442-RC-ADAPTIVE] RC 自适应: CONSTQP+LA 保留 pipe=4
...
class _NvEncConfig(Structure):
    ...
    _fields_ = [
        ...
        ("reserved5",       c_uint32 * 172),  # [V6431] absorb mis-mapped enableTemporalAQ (=rcParams bitfield bit7, not a standalone NV_ENC_CONFIG field)
...
class NVENCEncoder:
    ...
    def _drain_outputs_blocking(self, max_slots: int = None) -> list:
        ...
        for _ in range(max_slots):
            ...
            cast(byref(lock_raw, 4), ctypes.POINTER(c_uint32))[0] = 0
    ...
    def _lock_bitstream_with_retry(self, bs_handle, max_retries: int = 5, backoff_us: int = 1000):
        ...
        [FIX-LA-LOCKBS] NEED_MORE_INPUT 立即返回不重试。
        ...
        try:
            with self._lock:
                ...
                for _i in range(n_frames):
                    ...
                    # sub(k+LA) 后就绪（tests/diagnose_hevc_la.py 实测）；锁未就绪/
                    # 空槽会触发驱动永久阻塞（test4 根因：空槽 Lock 不返回）。
                    # H.264 驱动对空槽返回 SUCCESS+size=0，保持原行为不变。
                ...
                if send_eos:
                    ...
                    _drain_slots = (sorted(self._batch_slot_pending.keys())
                                    if _hevc_eos else _drain_order)
                    for _ds in _drain_slots:
                        _bs_h = self._slots[_ds]['bs_buf']
                        while True:
                            if _hevc_eos and not self._batch_slot_pending.get(_ds):
    ...
    def encode_frames_batch_ce_pipeline(self, nv12_tensors: list, force_idr_first: bool = False) -> list:
        """[PHASE4-CE-PIPELINE] Per-frame CE async pipeline encoding with LA support.

        EncodePicture with per-frame CUDA completion event, returns immediately;
        LockBitstream deferred to next slot rotation. Supports NEED_MORE_INPUT.

        Phase 1 (Harvest) / Phase 2 (Submit) / Phase 3 (Drain).
        Returns: bytes = valid H.264, b"" = LA buffering, None = empty.
        """
        n_frames = len(nv12_tensors)
        if n_frames == 0:
            return []
        ...
        try:
            with self._lock:
                ...
                for fi in range(n_frames):
                    ...
                    # [FIX-PIPE4-LA8] fi==0 only force_idr (单 IDR 替代 per-slot ×4 IDR)
                    force_idr = force_idr_first and (fi == 0)
                    # Phase 1: Harvest pending frame from this slot
                    if _slot_pending[slot_idx] is not None:
                        ...
                        if h264_data:
                            ...
                            # bitstream_size==0 时旧逻辑直接丢帧 → flush
                            # 写出错位帧破坏 GOP 参考帧顺序。
                            ...
                            if self._diag_empty <= 5 or self._diag_empty % 50 == 0:
                                print(f'[NVENC-Enc] ⚠️ 空帧 #{self._diag_empty} (ce-pipe fi={_prev_fi} '
                                      f'slot={slot_idx} ep_s={_prev_ep_s} bs_s={bs_status} la_buf={_la_buf})', flush=True)
                ...
                for slot_idx in range(pd):
                    if _slot_pending[slot_idx] is not None:
                        _pc, _pf, _pe, _pi, _pb = _slot_pending[slot_idx]
                        ...
                        if h264_data:
                            ...
                            self._output_slot_idx += 1
                        elif _pe == NV_ENC_ERR_NEED_MORE_INPUT:
                            # [FIX-LA-PENDING] 同 Phase 1: blocking LockBitstream 再试
                            _h264_blk, _ = self._lock_bitstream_blocking(_pb, timeout_ms=5000)
                            if _h264_blk:
                                # [FIX-LA-SPS-BLKRETRY-PH3] Phase 3 blocking retry
                                # 取回的 IDR 帧同样需要 SPS/PPS 缓存
                                if _pi and self._cached_sps_pps is None:
                                    self._cached_sps_pps = self._extract_sps_pps(_h264_blk)
                                    if self._cached_sps_pps:
                                        print("[NVENCEncoder] Cached SPS+PPS: %d bytes" %
                                              len(self._cached_sps_pps), flush=True)
    ...
    def flush(self) -> bytes:
        with self._lock:
            if self._encoder.value is None:
                return b""
            # [FIX-LA-EOS] 若 CE pipeline 已送过 EOS 则跳过
            if not self._eos_sent:
                ...
                _NvEncEncodePictureProto(self._func_ptrs[_FUNC_IDX["EncodePicture"]])(
                    self._encoder, cast(pic_buf, ctypes.POINTER(_NvEncPicParams)))
                self._eos_sent = True
            result_parts = []
            _LockBS = ctypes.CFUNCTYPE(c_uint32, c_void_p, ctypes.POINTER(c_uint8 * 1544))
            ...
            # LA=0 路径（ce_pipeline + flush）下 _batch_slot_pending 为空 →
            # HEVC/AV1 直接跳过全部槽位，避免锁空槽死锁。
            _hevc_eos_flush = self._codec in ("hevc", "av1")
            _flush_pending = getattr(self, '_batch_slot_pending', {})
            _drain_slots = (sorted(_flush_pending.keys())
                            if _hevc_eos_flush else _drain_order)
            for _si in _drain_slots:
                _slot = self._slots[_si]
                _bh = _slot['bs_buf']
                _sp = []
                # [FIX-LA-FLUSH] 首次 blocking Lock，后续 non-blocking 加速
                _first_lock = True
                while True:
                    if _hevc_eos_flush and not _flush_pending.get(_si):
                        break  # [FIX-HEVC-EOS-FLUSH] pending 已取完，不再锁空槽
                    lr = (c_uint8 * 1544)()
                    ctypes.memset(lr, 0, 1544)
                    cast(lr, ctypes.POINTER(c_uint32))[0] = NV_ENC_LOCK_BITSTREAM_VER
                    cast(byref(lr, 4), ctypes.POINTER(c_uint32))[0] = 0 if _first_lock else 1
                    cast(byref(lr, 8), ctypes.POINTER(c_void_p))[0] = _bh
                    _st = _LockBS(self._func_ptrs[_FUNC_IDX["LockBitstream"]])(self._encoder, lr)
                    if _st == NV_ENC_ERR_NEED_MORE_INPUT:
                        break
                    if _st != NV_ENC_SUCCESS:
                        break
                    sz = cast(byref(lr, 36), ctypes.POINTER(c_uint32))[0]
                    if sz == 0:
                        _NvEncUnlockBitstreamProto(self._func_ptrs[_FUNC_IDX["UnlockBitstream"]])(self._encoder, _bh)
                        break
                    ptr = cast(byref(lr, 56), ctypes.POINTER(c_void_p))[0]
                    ptr = ptr if isinstance(ptr, int) else (ptr.value or 0)
                    if ptr:
                        _sp.append(bytes((c_uint8 * sz).from_address(ptr)))
                        # [FIX-HEVC-EOS-FLUSH] VCL 帧按 FIFO pop 对应 pending
                        # 条目（无 VCL 辅助块不 pop，仅缓存参数集）
                        if _hevc_eos_flush and \
                                self._nal_first_vcl_type(_sp[-1]) is not None:
                            _dq = _flush_pending.get(_si)
                            if _dq:
                                _dq.popleft()
                                if not _dq:
                                    del _flush_pending[_si]
                    _NvEncUnlockBitstreamProto(self._func_ptrs[_FUNC_IDX["UnlockBitstream"]])(self._encoder, _bh)
                    _first_lock = False
                if _sp:
                    _total_frames += len(_sp)
                    _total_bytes += sum(len(p) for p in _sp)
                    print(f'[NVENC-FLUSH] slot[{_si}] drained {len(_sp)} frames, {sum(len(p) for p in _sp)} bytes '
                          f'(SDK-compliant EOS drain)', flush=True)
                result_parts.extend(_sp)
            if _total_frames > 0:
                print(f'[NVENC-FLUSH] total drained: {_total_frames} frames, {_total_bytes} bytes '
                      f'(SDK-compliant LA EOS flush — valid encoded frames)', flush=True)
            # NVENC EOS flush 排出的帧是有效编码帧（SDK 合规排空逻辑下帧数守恒）。
            # 已修正：旧版误认为花屏残片（错误排空导致缓冲区覆盖），正确实现下可正常使用。
            self._flush_frame_count = _total_frames
            return b"".join(result_parts)
...
# ═══════════════════════════════════════════════════════════════════════════════
# [V6431-LEVEL1-RC] Level 1 (NVENCEncoder ctypes) 码率控制模式
#
...
#        _NVENC_LEVEL1_LOOKAHEAD = 8
#        # 注意：QVBR 额外需要 vbvBufferSize(@+20) 和 vbvInitialDelay(@+24)
...
class IFRNetPipelineRunner:
    ...
    def _writer_loop(self, writer, pbar, n_seg_est, meter, timing_ref):
        ...
        # [FIX-LA-ACCUMULATE] LA>0 时累积帧，segment 结束一次性编码
        _la_acc_nv12: list = []
        ...
        if _pending_f0 is not None:
            _nvenc_ref._pending_f0_nv12 = None
            _f0_idr = getattr(_nvenc_ref, '_pending_f0_force_idr', False)
            _nvenc_ref._pending_f0_force_idr = False
        else:
            _f0_idr = False
        try:
            while self.running or not self.result_queue.empty():
                ...
                for _item in items:
                    if _item is self._SENTINEL:
                        continue
                    # [FIX-GPU-STAY] GPU tensor → 批量RGB→NV12 → NVENC ce-pipeline
            ...
            if _nvenc_flush is not None and not _la_acc_nv12 and not _la_active:
                try:
                    ...
                    if flush_data and not _nvenc_flush._eos_sent:
                        writer.write(flush_data)
                        written += 1
                except Exception:
                    pass
        except Exception as e:
...
class FFmpegWriter:
    ...
    def __init__(
        ...
        rc_mode: str = "constqp",           # [FIX-RC-MAP] NVENC rate control mode for Level 2 fallback
    ):
        ...
        if crf == 0:
            ...
            # [FIX-RC-MAP] 根据 Level 1 的 RC 模式选择对应的 FFmpeg -rc:v 值
            # qvbr 在旧版 FFmpeg h264_nvenc 中不可用，回退到 vbr_hq
        ...
        if 'nvenc' in codec:
            ...
            if crf == 0:
                ...
                _rc_v = _rc_v_map.get(rc_mode, 'vbr')
                _nvenc_info = (
                    f'[FIX-NVENC-PIPE] NVENC {_rc_v}(cq={crf}): '
                    f'preset={preset}  bf=0  '
...
class IFRNetVideoProcessor:
    ...
    def _get_or_create_nvenc_encoder(self, W: int, H: int, fps: float,
                                      preset: str, qp: int,
                                      rate_mode: str = "constqp", la_depth: int = 0,
                                      pipeline_depth: int = _NVENC_LEVEL1_DEFAULT_SLOTS,
        ...
        if self._cached_nvenc_encoder is not None and self._cached_nvenc_key == key:
            if not self.quiet:
                print(f'   [NVENC] 复用已激活编码器 ({W}x{H}@{fps:.1f}fps rate={rate_mode} codec={codec})',
                      flush=True)
        ...
        encoder = NVENCEncoder(W, H, fps, preset=preset, qp=qp,
                               ...
                               pipeline_depth=pipeline_depth, codec=codec)
    ...
    def _process_segment(
        ...
        # [V6431-CRF0-DECOUPLE] 解耦 crf=0 与 rate_mode/lookahead：
        ...
        # LA>0: encode_frame 返回 0 bytes（LA 缓冲），暂存 encoder 由 writer_loop
        ...
        if not skip_first_output:
            if _nvenc_encoder is not None:
                ...
                # [FIX-ENC-SYNC] 同上：f0 首帧转换 kernel 完成后才可读取
                torch.cuda.current_stream().synchronize()
