...
【Backport 2026-07-24 — 自 v6.4.5.2 (process_video_v6_4_5_1_single.py) 回植】
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        # [FIX-LA-CHUNK-STREAM] 跨 chunk 持久化的 slot 未决表/IDR 热身表
    ...
    def encode_frames_batch(self, nv12_tensors: list, force_idr_first: bool = False,
        ...
        try:
            with self._lock:
                ...
                # [FIX-LA-CHUNK-STREAM] 跨 chunk 持久化状态：首个块初始化，后续块复用
                ...
                for _i in range(n_frames):
                    ...
                    # ── Global drain: per-frame blocking LockBitstream ──
                    # [FIX-DRAIN-UNSUBMITTED-SLOT] & [FIX-CONSTQP-FRAME-CE] 配合
                ...
                for _i in range(n_frames):
                    ...
                    while self._batch_slot_pending[slot_idx] is not None:
                        ...
                        if not _drained:
                            ...
                            if _drain_guard > self._slot_count * 4:
                                print(f'[NVENC-Enc]  slot backpressure wait exceeded '
                    ...
                    for _est_fi, _h264_data in _drained:
                        ...
                        if not _h264_data:
                            ...
                            self._diag_empty += 1
                            if self._diag_empty <= 5 or self._diag_empty % 50 == 0:
                                print(f'[NVENC-Enc]  空帧 #{self._diag_empty} (stream fi={_actual_fi} '
                            ...
                            if len(_la_acc_nv12) >= _CHUNK_FRAMES:
                                ...
                                for _fi, h264_data in pairs:
                                    if not h264_data:
                                        ...
                                        # 对齐 v6.4.4.1 _write_pairs，用前一帧补偿保持帧数守恒
