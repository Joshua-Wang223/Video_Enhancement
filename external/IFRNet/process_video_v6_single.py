...
【命令行使用示例】
  ...
  python process_video_v6_single.py \\
...
class HardwareCapability:
    ...
    def _probe_nvdec() -> bool:
        ...
        try:
            ...
            dec_cmd = [
                'ffmpeg', '-hwaccel', 'cuda', '-hwaccel_output_format', 'nv12',
...
def tensor_to_np(t, orig_H, orig_W, sync_stream=None) -> List[np.ndarray]:
    ...
    arr = t.clamp_(0.0, 1.0).mul_(255.0).round_().byte()  # [FIX-ROUND] 四舍五入避免截断偏暗
...
class IFRNetVideoProcessor:
    ...
    def _process_segment(
        ...
        # [FIX-PAD] 直接内联计算 pad 量，无需分配临时 11MB 数组
...
def main():
    parser = argparse.ArgumentParser(
        description='IFRNet 视频插帧 —— 终极优化版 v6（单卡版）',
