...
> **当前版本**: v2.0.0（优化版）｜IFRNet 后端 v6.3.0（双流深度流水线）/ Real-ESRGAN 后端 v6.4（深度模块化架构 + 独立优化版）  
> **最后更新**: 2026-05-06

一套完整的 AI 视频增强解决方案，整合了**视频插帧（IFRNet v6.3.0）**与**视频超分辨率（Real-ESRGAN v6.4）**两大模块，并针对单 GPU 环境进行了深度优化。IFRNet 后端采用双流深度流水线架构（H2D 预取 / D2H 输出独立流 + CudaEventPool），实现计算/传输三流全重叠。Real-ESRGAN 后端同时提供 `realesrgan_video_ds`（深度模块化架构，4 级并行处理）和独立优化版 v6.4。
...
| 🔀 **双流深度流水线** | IFRNet v6.3.0 双 transfer stream 架构（H2D 预取 + D2H 输出独立流），计算/传输三流全重叠 |
...
| 🚄 **批量升级** | IFRNet 默认 batch_size 24→48，充分利用空闲显存，理论吞吐提升 20–30% |
...
│   │
│   ├── processors/
│   │   ├── ifrnet_processor_v6_1_single.py          # ★ IFRNet 处理器 v6.1（对接 v6.3.0 后端）
│   │   ├── ifrnet_processor_v6_single.py            # IFRNet 处理器 v6（历史版本，保留参考）
│   │   ├── realesrgan_processor_video_optimized.py  # ★ Real-ESRGAN 处理器（优化版，对接 realesrgan_video_ds）
...
├── external/
│   ├── IFRNet/
│   │   ├── process_video_v6_3_0_single.py  # ★ IFRNet v6.3.0 后端核心（双流深度流水线）
...
| 操作 | 设置 | 10 分钟视频耗时 |
|------|------|--------------|
| 插帧 | 2x IFRNet_S (v6.3.0 bs=48) | ~5 分钟 |
| 插帧 | 4x IFRNet_S (v6.3.0 bs=48) | ~10 分钟 |
| 超分 | 2x | ~50 分钟 |
| 超分 | 4x | ~125 分钟 |
| **插帧 2x + 超分 4x（直连流水线）** | **推荐配置** | **~127 分钟** |

IFRNet v6.3.0 双流深度流水线 + 默认 batch_size 48（原 24）使插帧吞吐提升约 20–30%。优化版直连流水线相比 v1 节省约 30% 总时间和 38% 磁盘 I/O。Real-ESRGAN v6.4 深度流水线架构额外提升约 10–20% 吞吐（4 级并行 + SR H2D 预取重叠）。TRT 加速在 Engine 已缓存的情况下，推理吞吐量可额外提升 15–30%。
...
```bash
# ── IFRNet 独立插帧（v6.1 处理器，对接 v6.3.0 后端）──────────────────────────
...
**Q: IFRNet v6.3.0 相比旧版有哪些提升？**  
A: v6.3.0 引入了多项架构级优化：双 transfer stream 架构（H2D 预取 + D2H 输出独立流），实现三流计算/传输全重叠；CudaEventPool 消除每批次 Event 创建/销毁开销（约 0.5–1ms/批）；默认 batch_size 从 24 升级到 48，充分利用空闲显存；后台 GPU 监测线程自动统计利用率/显存并推荐 batch_size。

**Q: IFRNet `stream_transfer` 属性报错？**  
A: v6.3.0 将 `stream_transfer` 拆分为 `stream_h2d`（预取）和 `stream_d2h`（输出）。如有上层代码直接引用 `processor.stream_transfer`，需改为 `stream_h2d` 或 `stream_d2h` 分别使用。
