...
class HardwareCapability:
    ...
    def _probe_nvenc(codec: str) -> bool:
        # [FIX-PROBE-LAVFI]   修复1：去掉 color=d=0.1:r=1 的 d 参数（0.1s×1fps=0帧），
        #                      改为无限长源，由 -frames:v 1 截取第 1 帧。
        # [FIX-PROBE-PIXFMT]  修复2：显式 -pix_fmt yuv420p，避免 FFmpeg 5.0+ 中
        #                      color 源默认 yuv444p 与 NVENC 不兼容。
        # [FIX-PROBE-TMPFILE] 修复3：-f null - 在部分 NVENC 驱动版本下与 null muxer
        #                      存在兼容问题（stdout pipe 写入路径不同于文件路径）。
        #                      改为写入临时 .mp4 文件，与实际 FFmpegWriter 命令保持一致。
        # [FIX-PROBE-BFRAME]  修复4：h264_nvenc 默认可能启用 B 帧 lookahead，
        #                      单帧 encode 时 lookahead buffer 无法填满导致编码器报错。
        #                      加 -bf 0 禁用 B 帧，与实际 FFmpegWriter 命令一致。
        # [FIX-PROBE-STDERR]  新增：probe 失败时打印 ffmpeg stderr，方便后续诊断。
        import tempfile as _tempfile
        _tmp = None
        try:
            _fd, _tmp = _tempfile.mkstemp(suffix='.mp4')
            os.close(_fd)
            cmd = [
                'ffmpeg', '-y',
            ...
            if result.returncode != 0:
                ...
                if _err:
                    print(f'  [FIX-PROBE-STDERR] {codec} probe 失败原因: {_err}')
            return result.returncode == 0
        except Exception as e:
            print(f'  [FIX-PROBE-STDERR] {codec} probe 异常: {e}')
            return False
        finally:
