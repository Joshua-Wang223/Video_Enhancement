...
=====================================
对接 process_video_v6_3_2_single.py（IFRNetVideoProcessor），
...
【v6 变更说明（相对 v5）】
  - 对齐底层 process_video_v6_3_2_single.py v6.3.2（含 FIX-D2H / FIX-PAD）
  - 新增 preview / preview_interval 参数（透传至底层 process_video）
  - main() CLI 新增 --preview / --preview-interval
  - _process_segment 打印信息对齐 v6.3.2 版本标记
...
class IFRNetProcessor:
    ...
    def process_video(self, input_video: str, output_video: str) -> bool:
        ...
        print("\n" + "=" * 65)
        print("🎬 IFRNet 视频插帧处理（完整流程）—— v6.3.2")
        print(f"📹 输入  : {input_video}")
    ...
    def _get_or_create_video_processor(self):
        ...
        # 延迟导入，保证只在需要时加载
        from process_video_v6_3_2_single import IFRNetVideoProcessor
    ...
    def _process_segment(self, segment_path: str, output_path: str) -> bool:
        """
        调用 process_video_v6_3_2_single.IFRNetVideoProcessor 处理单个分段。
        ...
        try:
            ...
            print(f"   ❌ 无法导入 process_video_v6_3_2_single: {e}")
            return False
        except Exception as e:
            print(f"   ❌ 处理失败: {e}")
            import traceback
...
def main():
    ...
    底层对接 process_video_v6_3_2_single.IFRNetVideoProcessor。
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
底层脚本：external/IFRNet/process_video_v6_3_2_single.py（v6.3.2）
