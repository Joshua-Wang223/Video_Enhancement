...
class HardwareCapability:
    ...
    def _probe_nvenc(codec: str) -> bool:
        # [FIX-PROBE-LAVFI] 原命令 color=d=0.1:r=1 → 0.1s×1fps=0.1帧，实际输出 0 帧，
        # ffmpeg 因无帧可编码返回非零退出码，导致 _nvenc_ok() 误判 NVENC 不可用。
        # 修复1：去掉 d=0.1 参数，lavfi color 源变为无限长，-frames:v 1 截取第 1 帧。
        #
        # [FIX-PROBE-PIXFMT] FFmpeg 5.0+ 中 lavfi color 源默认输出 yuv444p，
        # 而 h264_nvenc / hevc_nvenc 原生仅支持 yuv420p、nv12、p010le 等格式，
        # 缺少 -pix_fmt yuv420p 时像素格式协商失败 → 非零退出 → 误判 NVENC 不可用。
        # 修复2：显式指定 -pix_fmt yuv420p，与实际 FFmpegWriter 工作命令保持一致。
        # 注：probe 本身不验证编码质量，yuv420p 已足够触发 NVENC 硬件单元。
        try:
            cmd = [
                'ffmpeg', '-f', 'lavfi', '-i', 'color=c=black:s=64x64:r=1',
