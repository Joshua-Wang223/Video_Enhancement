...
class GPUMonitor:
    ...
    · 采样间隔默认 2 秒，不显著增加 CPU 开销。
    · 优先使用 pynvml（需安装 nvidia-ml-py），不可用时回退至 torch.cuda.utilization。
    · 若完全无法获取 GPU 信息，相关字段显示为 N/A，不影响推理流程。
    · 所有采样、NVML 初始化与清理均在子线程内完成，保证线程安全。
    """

    def __init__(self, device: torch.device, interval: float = 2.0):
        self.device = device
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=self._sample, daemon=True, name="GPUMonitor")
        self._thread.start()
    ...
    def _sample(self):
        ...
        try:
            ...
            # pynvml 不可用，后续将尝试 torch.cuda API
            pass
        ...
        while not self._stop_event.is_set():
            ...
            try:
                if pynvml_handle is not None:
                    # 使用 pynvml 采集（精确）
                    util = float(
                        pynvml.nvmlDeviceGetUtilizationRates(pynvml_handle).gpu
                    )
                    mem_info = pynvml.nvmlDeviceGetMemoryInfo(pynvml_handle)
                    mem_used_gib = mem_info.used / (1024**3)
                elif torch.cuda.is_available():
                    # 回退到 PyTorch 内置 API（可能返回 0）
                    try:
    ...
    def summary(self) -> Tuple[float, float, float, float]:
        """
        返回统计值：
            (平均利用率 %, 最高利用率 %, 平均显存 GiB, 峰值显存 GiB)
        若无有效采样数据，打印警告并返回全 0。
        """
        if not self.util_samples:
            print("[GPU-MONITOR] 警告：未能获取任何 GPU 利用数据，请检查 pynvml 安装或驱动状态。")
            return 0.0, 0.0, 0.0, 0.0

        avg_util = round(float(np.mean(self.util_samples)), 1)
        max_util = round(float(np.max(self.util_samples)), 1)
        avg_mem = round(float(np.mean(self.mem_samples)), 2)
        max_mem = round(float(np.max(self.mem_samples)), 2)
        ...
        try:
            while self.running or not self.result_queue.empty():
                ...
                if isinstance(item, _PinnedResultItem):
                    # 等待 stream_d2h 上的 DMA 完成
                    item.event.synchronize()
                    n_pairs = item.B
                    arr = item.buf[:item.B * item.T].numpy()
                    ...
                    self._event_pool.release(item.event)
                    item.pool.release(item.buf)
                else:
...
class IFRNetVideoProcessor:
    ...
    def __init__(
        ...
        self._gpu_monitor = GPUMonitor(self.device, interval=2.0)
    ...
    def _build_trt_engine(self, input_shape: Tuple[int, int, int, int], cache_dir: str,
        ...
        tag       = f'{self.model_name}_B{B}_H{H}_W{W}_fp{"16" if self.use_fp16 else "32"}{_sm_tag}'   # ✅ 加入模型变体，避免跨模型加载错误 Engine
    ...
    def process_video(
        ...
        avg_u, max_u, avg_mem, max_mem = self._gpu_monitor.summary()
        if avg_u > 0 or avg_mem > 0:
            print(f'\n[GPU-MONITOR] 平均利用率: {avg_u}%  | 最高利用率: {max_u}%')
            ...
            if self.device.type == 'cuda':
                ...
                if avg_u < 70 and max_u < 95 and mem_ratio < 0.70:
...
def main():
    ...
    processor = IFRNetVideoProcessor(
        ...
        model_name     = args.model,   # 传递用户指定的模型名
