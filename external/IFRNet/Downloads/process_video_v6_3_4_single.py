...
class GPUMonitor:
    ...
    def get_queue_suggestions(
        ...
        hw_profile:       Optional['_HWProfile'] = None,  # [FIX-POOL-AUTOSCALE]
    ) -> Tuple[int, int]:
        """
        ...
        · [FIX-POOL-AUTOSCALE] 上限依 hw_profile GPU tier 自动缩放，不再硬编码 1 GiB。
        """
        # [FIX-POOL-AUTOSCALE] 动态上限：有 profile 时按 GPU 档位，否则保守回退 1024 MiB
        _MAX_POOL_MB = (_pool_limit_mb_for_profile(hw_profile)
                        if hw_profile is not None else 1024.0)
...
def _auto_queue_depths(
    ...
    # [FIX-T3-MEMCAP / FIX-POOL-AUTOSCALE] 依据 PinnedPool 内存上限约束 result_depth。
    ...
    # [FIX-POOL-AUTOSCALE] 上限依 GPU tier 自动缩放（T4=2048 MiB，而非硬编码 1024）
    _MAX_POOL_MB = _pool_limit_mb_for_profile(profile)
...
class IFRNetVideoProcessor:
    ...
    def process_video(
        ...
        if _gpu_stats.sample_count > 0:
            ...
            # [FIX-POOL-AUTOSCALE] 传入 hw_profile 以使用 GPU 档位动态上限
            _hw_prof_for_sug = getattr(self, '_hw_profile_cache', None)
            pair_gpu_sug, result_gpu_sug = self._gpu_monitor.get_queue_suggestions(
                ...
                hw_profile=_hw_prof_for_sug,  # [FIX-POOL-AUTOSCALE] GPU 档位动态上限
            )
            ...
            if _is_t3:
                ...
                _MAX_POOL_MB = _pool_limit_mb_for_profile(_hw_prof_for_sug)  # 按 GPU 型号动态上限
                if _slot_mb > 0.0:
                    _max_rq_mem = max(8, int(_MAX_POOL_MB / _slot_mb) - 2)
                    if final_result_q > _max_rq_mem:
                        print(
                            f'[ADAPTIVE-QUEUE] PinnedPool 内存上限截断: '
                            f'result_queue {final_result_q} → {_max_rq_mem}'
                            f'  (slot={_slot_mb:.1f} MB × {_max_rq_mem+2} ≈ {_MAX_POOL_MB:.0f} MB上限)'
                        )
