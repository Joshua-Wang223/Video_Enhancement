...
class GPUMonitor:
    ...
    def get_queue_suggestions(
        ...
        · [FIX-POOL-AUTOSCALE] 上限依 hw_profile GPU tier 自动缩放，不再硬编码。
        """
        # [FIX-POOL-AUTOSCALE] 动态上限：有 profile 时按 GPU 档位，否则保守回退 1024 MiB
        _PINNED_POOL_MAX_MB = (_pool_limit_mb_for_profile(hw_profile)
...
def _auto_queue_depths(
    ...
    _PINNED_POOL_MAX_MB = _pool_limit_mb_for_profile(profile)
    ...
    def process_video(
        ...
        if _gpu_stats.sample_count > 0:
            ...
            if _is_t3:
                ...
                if _slot_mb > 0.0:
                    _pool_limit_dyn = _pool_limit_mb_for_profile(
                        getattr(self, '_hw_profile_cache', None)
                        or type('_P', (), {'gpu_tier': 1.0})()
                    )
                    _max_rq_mem = max(8, int(_pool_limit_dyn / _slot_mb) - 2)
                    if final_result_q > _max_rq_mem:
                        print(
                            f'[ADAPTIVE-QUEUE] PinnedPool 内存上限截断: '
                            f'result_queue {final_result_q} → {_max_rq_mem}'
                            f'  (slot={_slot_mb:.1f} MB × {_max_rq_mem+2} ≈ {_pool_limit_dyn:.0f} MB上限)'
                        )
