...
def _process_single(
    ...
    if skip_interpolate:
        ...
        try:
            ...
            traceback.print_exc()
            return False
        proc.quiet = getattr(args, "quiet_esrgan", False)  # 透传静默开关
        try:
    ...
    if skip_upscale:
        ...
        try:
            ...
            proc.quiet            = getattr(args, "quiet_ifrnet", False)  # 透传静默开关
        except Exception as e:
    ...
    try:
        ...
        ifrnet_proc.quiet            = getattr(args, "quiet_ifrnet", False)  # 透传静默开关
        esrgan_proc = RealESRGANVideoProcessor(config)
        esrgan_proc.quiet            = getattr(args, "quiet_esrgan", False)  # 透传静默开关
    except Exception as e:
        print(f"❌ 初始化处理器失败: {e}")
        traceback.print_exc()
        return False
...
""",
    ...
    g.add_argument("--quiet-ifrnet", action="store_true",
                   help="静默 IFRNet 底层冗余输出（含 [FFmpegWriter] 命令行；底层 --quiet 参数）")
    g.add_argument("--quiet-esrgan", action="store_true",
                   help="静默 ESRGAN 底层冗余输出（含 [FFmpegWriter] 命令行；底层 --quiet 参数）")
