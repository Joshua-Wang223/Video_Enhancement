"""
IFRNet 视频插帧处理脚本 —— 终极优化版 v6.3.3（多卡版）
==========================================================
...
class IFRNetVideoProcessor:
    ...
    def _process_segment(
        ...
        # ── 主处理 ────────────────────────────────────────────────────────────
        if not _use_queue and self.device.type == 'cuda':
    ...
    def process_video(
        ...
        if n_workers > 1 and num_gpus > 1:
            return self._process_multi_gpu(input_path, output_path, scale, num_gpus, nwpg)
...
def _ifrnet_segment_worker(
    ...
    try:
        ...
        processor = IFRNetVideoProcessor(
            ...
            model_name     = 'IFRNet_S_Vimeo90K',  # Worker 进程 spawn 时默认模型
            quiet          = True,
...
def main():
    parser = argparse.ArgumentParser(
        description='IFRNet 视频插帧 —— 终极优化版 v6.3.3（多卡版）',
    ...
    # 打印启动信息
    print('=' * 65)
    print('  IFRNet 视频插帧 —— 终极优化版 v6.3.3（多卡版）')
    print('=' * 65)
    num_gpus  = torch.cuda.device_count()
