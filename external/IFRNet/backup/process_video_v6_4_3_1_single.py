...
【v6.4.3 Phase 1-3 修复（GPU-STAY 性能退化）】
  ...
  [TODO-PHASE4] NVENC 异步流水线（未来）：
               · 多 input buffer (4-8) + 多 bitstream buffer 实现帧间流水线
               · EncodePicture 设置 completionEvent (CUDA event @ offset 56)
               · 提交帧 N+1 后再 Harvest 帧 N 的 LockBitstream
               · 目标: N 帧吞吐从 N×T 降至 ~T + (N-1)×T_small
               · 约束: 需多 surface → 更大 VRAM (每 720p input buf ≈ 1.4MB)
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        # [V6431-RC] NV_ENC_RC_PARAMS at offset 40 in NV_ENC_CONFIG
        ...
        # [V6431-TAQ] Enable AQ + Temporal AQ via rcParams bitfield (offset 36)
        ...
        # [V6431-LOOKAHEAD] Optional: lookahead for VBR_HQ/QVBR (matching Level 2 -rc-lookahead N)
...
#     · [GPU 验证中] targetQuality @rcParams+116 offset; mode 从 4(VBR_MINQP) 修复为 32(VBR_HQ)
#
...
class IFRNetPipelineRunner:
    ...
    def _writer_loop(self, writer, pbar, n_seg_est, meter, timing_ref):
        ...
        try:
            while self.running or not self.result_queue.empty():
                ...
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) >= 2 and _item[0] == 'GPU_RAW':
                        ...
                        n_interp = rB * rT
                        # 逐帧 NVENC SDK 同步编码（编码期间 T2 可并行推理）
                        # 必须按交叉交错顺序编码：对于每个 pair，
                        # 先输出 T 个插值帧再输出该 pair 的原始帧。
                        # 全插值帧 → 全原始帧的顺序会导致周期性画面重复。
                        self.__dict__.setdefault('_diag_empty_h264', 0)
                        self.__dict__.setdefault('_diag_nvenc_frames', 0)
                        for bi in range(rB):
                            for tj in range(rT):
                                h264_data = _nvenc.encode_frame(all_nv12[bi * rT + tj])
                                if not h264_data:
                                    # Tier 1-B: 重编码重试（最多 2 次）
                                ...
                                if not h264_data:
                                    ...
                                    # Tier 1-A fallback: 复制前一帧，保持帧数和 A/V 同步
                            ...
                            h264_data = _nvenc.encode_frame(all_nv12[n_interp + bi])
                            if not h264_data:
                                # Tier 1-B: 重编码重试（最多 2 次）
                                for _retry in range(2):
                                    h264_data = _nvenc.encode_frame(all_nv12[n_interp + bi])
            ...
            # [FIX-GPU-STAY] NVENC flush 在 Writer 线程 SENTINEL 之后执行，
