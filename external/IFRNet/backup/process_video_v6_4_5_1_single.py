...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        # [V6451-RC] NV_ENC_RC_PARAMS at offset 40 in NV_ENC_CONFIG
        ...
        # [V6451-TAQ] Enable AQ + Temporal AQ via rcParams bitfield (offset 36)
        ...
        # [V6451-LOOKAHEAD] Optional: lookahead for VBR_HQ/QVBR (matching Level 2 -rc-lookahead N)
    ...
    def encode_frames_batch(self, nv12_tensors: list, force_idr_first: bool = False) -> list:
        ...
        Uses the same proven synchronous pattern as encode_frame (blocking
        EncodePicture + blocking LockBitstream, no completionEvent), but
        distributes frames across pipeline slots round-robin to reduce
        buffer contention.
        ...
        # 防止 push 静默失败时 _need_pop=True 错误触发 cuCtxPopCurrent。
        _need_pop = False
        ...
        try:
            with self._lock:
                ...
                for fi in range(n_frames):
                    ...
                    if status != NV_ENC_SUCCESS:
                        raise RuntimeError("[NVENCEncoder] EncodePicture[%d] failed, code=%d" % (slot_idx, status))
                    if not h264_data:
                        self.__dict__.setdefault('_diag_empty', 0)
                        self._diag_empty += 1
                        # Tier 0: 增强诊断 — 记录 NV12 输入统计以区分根因类别
                        ...
                        if self._diag_empty <= 5 or self._diag_empty % 50 == 0:
                            print(f'[NVENC-Enc] ⚠️ 空帧 #{self._diag_empty} (fi={fi}, slot={slot_idx}) '
            ...
            try:
                ...
                for i, h264_data in enumerate(h264_list):
                    if not h264_data:
                        ...
                        _force_idr_retry = force_idr and i == 0
                        for _retry in range(2):
                            h264_data_retry = self._nvenc.encode_frame(nv12_list[i], force_idr=_force_idr_retry)
                            if h264_data_retry:
