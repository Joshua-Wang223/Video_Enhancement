"""
IFRNet 视频插帧处理脚本 —— 四级回退统一编码版 v6.4.4（单卡版）
============================================================
...
【v6.4.3 Phase 1-3 修复（GPU-STAY 性能退化）】
  ...
  [PHASE4] NVENC 批编码同步流水线（completionEvent async 因 ~7% 空帧率已移除）：
               ...
               · NVENCEncoder.encode_frames_batch(): 逐帧同步编码（blocking EncodePicture + LockBitstream）
               · 与 encode_frame 使用相同验证过的同步编码范式
               · CUDA context 保护 + SPS/PPS 跨段缓存预挂
               · 内部保留 _submit_to_slot / _harvest_slot 供未来异步方案参考
...
class NVENCEncoder:
    ...
    def __init__(self, width: int, height: int, fps: float,
        ...
        # Single input/bitstream buffer (verified stable — multi-slot caused frame corruption)
        ...
        # [V6441-RC] NV_ENC_RC_PARAMS at offset 40 in NV_ENC_CONFIG
        # SDK 13.0 NV_ENC_RC_PARAMS is SEQUENTIAL (not a union):
        #   version@0, rateControlMode@4, constQP{qpInterP,qpInterB,qpIntra}@8..19
        #   averageBitRate@20, maxBitRate@24, vbvBufferSize@28, vbvInitialDelay@32
        #   bitfield@36: bits 0-7=enable{MinQP,MaxQP,InitialRCQP,AQ,Lookahead,...TemporalAQ}
        #                bits 10-11=multiPass
        #   minQP@40, maxQP@52, initialRCQP@64, temporalLayerCount@76
        #   reserved0[9]@80, targetQuality@116, targetQualityLSB@120, lookaheadDepth@121(uint16)
        ...
        if self._rate_mode == 'vbr_hq':
            ...
            # [FIX-RC-MODE] SDK 13.0 NV_ENC_PARAMS_RC_VBR_HQ = 32 (0x20), NOT 4!
            #     SDK 12.0+ restructured RC enum: 4=VBR_MINQP(deprecated), 32=VBR_HQ.
            #     Passing 4 (=VBR_MINQP) caused targetQuality to be completely ignored.
            rc_ptr[1] = 32                           # NV_ENC_PARAMS_RC_VBR_HQ (SDK 12.0+)
            # constQP fields @rc_ptr[2..4] — VBR_HQ ignores, left as preset defaults
            # [FIX-VBR-TQ] targetQuality range 1 (worst) → 51 (best), inverse of CRF (0=best → 51=worst).
            # Map: targetQuality = max(1, 51 - CRF). CRF≈0 (lossless) → tq=51 (max quality).
            # With VBR_HQ, averageBitRate=0 lets targetQuality be the sole quality controller.
            rc_ptr[5] = 0                            # averageBitRate @offset 20 (0=TQ-driven)
            rc_ptr[6] = 0                            # maxBitRate @offset 24 (0=unconstrained)
            _tq = max(1, 51 - _qp_val)
            rc_ptr[29] = _tq                         # targetQuality @offset 116
            print(f"[NVENCEncoder] VBR_HQ: crf={_qp_val} targetQuality={_tq} avgBitrate=0 (TQ-driven)", flush=True)
        elif self._rate_mode == 'qvbr':
            # QVBR mode: NV_ENC_PARAMS_RC_QVBR + qvbrQuality (SDK 12.2+, matches Level 2 -rc:v qvbr)
            rc_ptr[1] = 32                           # NV_ENC_PARAMS_RC_QVBR
            rc_ptr[5] = 0                            # averageBitRate @offset 20 (QVBR: quality-driven by default)
            rc_ptr[6] = 0                            # maxBitRate @offset 24 (0=unconstrained)
            # vbv buffer (QVBR requires these; 4MB buffer, 2MB initial)
            if rc_ptr[7] == 0:
                rc_ptr[7] = 4194304                  # vbvBufferSize @offset 28 (= 4 MB)
            if rc_ptr[8] == 0:
                rc_ptr[8] = 2097152                  # vbvInitialDelay @offset 32 (= 2 MB)
            # [FIX-V6431-TQ] qvbrQuality 同样用减法（标度与 targetQuality 一致，越高=质量越高）
            _tq = max(1, _qp_val - _NVENC_VBR_QUALITY_OFFSET)
            rc_ptr[31] = _tq                         # qvbrQuality @offset 124
            print(f"[NVENCEncoder] QVBR: crf={_qp_val} offset={_NVENC_VBR_QUALITY_OFFSET} qvbrQuality={_tq}", flush=True)
        else:
        ...
        # [V6441-TAQ] Enable AQ + Temporal AQ via rcParams bitfield (offset 36)
        # GetEncodePresetConfig already filled reasonable defaults — OR to preserve.
        # bit 3 = enableAQ, bit 7 = enableTemporalAQ
        rc_ptr[9] = rc_ptr[9] | (1 << 3) | (1 << 7)

        # [V6441-LOOKAHEAD] Optional: lookahead for VBR_HQ/QVBR (matching Level 2 -rc-lookahead N)
        if self._la_depth > 0 and self._rate_mode in ('vbr_hq', 'qvbr'):
            _rc_bf = rc_ptr[9]                       # read-modify-write the bitfield word
            _rc_bf |= (1 << 4)                       # enableLookahead (bit 4)
            # multiPass: 0=disabled, 1=1pass, 2=2pass_quarter, 3=2pass_full
            _rc_bf = (_rc_bf & ~(0x3 << 10)) | (1 << 10)  # multiPass=1pass
            rc_ptr[9] = _rc_bf
            # lookaheadDepth — uint16 at rcParams+121
            _la_ptr = cast(byref(preset_config, 8 + 40 + 121), ctypes.POINTER(c_uint16))
        ...
        # 10. Create single input buffer + bitstream buffer
        #     Multi-slot was removed in v6.4.4 (caused intermittent frame corruption);
        #     single buffer with synchronous encode_frame is verified stable since v6.4.3.
        ...
        if self._rate_mode in ('vbr_hq', 'qvbr'):
            _tq = max(1, _qp_val - _NVENC_VBR_QUALITY_OFFSET)
            _extra += " tq=%d" % _tq
        print("[NVENCEncoder] Ready: %dx%d@%.1ffps H.264 %s QP=%d preset=%s%s (GPU direct SDK 13.0)" %
              (width, height, fps, _mode_label, _qp_val, self._preset_name, _extra), flush=True)

    def _destroy_buffers(self):
        """Destroy the single input buffer + bitstream buffer."""
        if self._input_buf_handle.value is not None:
            try:
                ...
                pass
            self._input_buf_handle = c_void_p(None)
        if self._bs_handle.value is not None:
            try:
    ...
    def _lock_bitstream_with_retry(self, bs_handle, max_retries: int = 3, backoff_us: int = 500):
    ...
    def flush(self) -> bytes:
        ...
        # 在正确上下文执行。与 encode_frame() 中的保护逻辑一致。
        _need_pop = False
        _primary = getattr(self, '_primary_ctx', None)
        if _primary is not None and _primary.value is not None:
            try:
...
# 将 NVENC 编码（encode_frame）与 T3 Writer 的 RGB→NV12 kernel 并行化。
# T3 Writer 完成 event.sync + NV12 转换后将帧列表提交到队列，
# 编码线程独立调用 encode_frame + muxer.write，
...
class _NVENCEncodeThread:
    ...
    设计约束：
      ...
      · encode_queue_depth=2：限制 VRAM 中积压的 NV12 tensor 批次数。
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 2):
    ...
    def _loop(self):
        ...
        # encode_frame 内的 cuCtxPushCurrent 若静默失败会导致
        ...
        while True:
            ...
            try:
                for i, nv12_tensor in enumerate(nv12_list):
                    _force_idr = force_idr and i == 0  # 仅批次首帧可能 force_idr
                    h264_data = self._nvenc.encode_frame(nv12_tensor, force_idr=_force_idr)
                    if not h264_data:
                        # Tier 1-B: 对同一 NV12 tensor 重编码重试（最多 2 次）
                        for _retry in range(2):
                            h264_data_retry = self._nvenc.encode_frame(nv12_tensor, force_idr=_force_idr)
                            if h264_data_retry:
                                h264_data = h264_data_retry
                                break
                    if not h264_data:
                        self._empty += 1
                        # Tier 1-A fallback: 复制前一帧 H.264 数据，保持帧数和 A/V 同步
                        if self._prev_h264 is not None:
                            self._writer.write(self._prev_h264)
                            self._written += 1
                        # _prev_h264 保持不变（上一次正常帧）
                    else:
                        self._writer.write(h264_data)
                        self._written += 1
                        self._prev_h264 = h264_data  # 缓存正常帧供后续空帧补偿
            except Exception as e:
                self.error = e
                return  # 停止消费队列；error 将在 flush_and_join() 中 raise
        # [FIX-ENC-FLUSH] 编码线程在退出前执行 EOS flush，
        # 确保 flush 与 encode_frame 在同一 CUDA context 内执行，
        # 消除跨线程 context 栈差异导致的静默 flush 失败。
        try:
            leftover = self._nvenc.flush()
            if leftover:
                self._writer.write(leftover)
        except Exception:
            pass
    ...
    def flush_and_join(self, timeout: float = 120.0):
        ...
        # 确保与 encode_frame 在同一 CUDA context。此处不再重复 flush。
...
#     · [需 GPU 验证] targetQuality @rcParams+116 offset 来自头文件推断
#
#   "qvbr" — NV_ENC_PARAMS_RC_QVBR (mode=32) + qvbrQuality (SDK 12.2+)
...
#     · [GPU 验证通过] qvbrQuality @rcParams+124 offset (test_nvenc_vbr_hq_offsets.py)
#     · 需同时设置 vbvBufferSize (@+28) 和 vbvInitialDelay (@+32)
#
...
#     · [GPU 验证通过] lookaheadDepth 为 uint16 @rcParams+121，非 4 字节对齐
#
# ── 常用配置示例 ──
#
...
_NVENC_LEVEL1_LOOKAHEAD: int = 16
            ...
            while self.running or not self.result_queue.empty():
                ...
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) >= 2 and _item[0] == 'GPU_RAW':
                        ...
                        # [FIX-ENC-THREAD] 惰性初始化编码线程（首次 GPU_RAW 时创建）
...
class IFRNetVideoProcessor:
    ...
    def _get_or_create_nvenc_encoder(self, W: int, H: int, fps: float,
                                      preset: str, qp: int,
                                      rate_mode: str = "constqp", la_depth: int = 0) -> 'NVENCEncoder':
        """跨段复用 NVENC 编码器，参数不变时跳过 11 行初始化日志和 DLL 加载。"""
        key = (W, H, fps, preset, qp, rate_mode, la_depth)
        ...
        if 'nvenc' in use_codec:
            try:
                _nvenc_encoder = self._get_or_create_nvenc_encoder(
                    ...
                    rate_mode=_NVENC_LEVEL1_RATE_MODE,
                    la_depth=_NVENC_LEVEL1_LOOKAHEAD if self.crf > 0 else 0,
                )
                writer = FFmpegMuxer(
                    output_path, new_fps,
                    audio_src=audio_src,
