...
class IFRNetVideoProcessor(TensorRTAccelMixin):
    ...
    def _load_model(self, device: torch.device, use_compile: bool = True):
        ...
        # [P0-FIX-MODEL-ARCH] 按实例 model_name 动态解析网络架构。
        # 原实现使用模块级全局 Model（硬编码 IFRNet_S_Vimeo90K），库内调用路径
        # （processor 层 import）不会执行 main() 中的 global Model 重绑定，
        # 导致配置 IFRNet_L / IFRNet_Vimeo90K 时用 S 架构 load_state_dict(strict) 必然失败。
        # CLI 独立入口因已传 model_name 且此处按名解析，行为不变；global Model 仅保留向后兼容。
        _model_cls, _ = _load_ifrnet_module(self.model_name or 'IFRNet_S_Vimeo90K')
        model = _model_cls()
    ...
    def _safe_infer(self, img0_list, img1_list, timesteps, orig_H, orig_W,
        ...
        _first_attempt = True
        # [P1-FIX-BS1-CIRCUIT] bs=1 仍持续 OOM 的熔断计数：原实现"深度清理→估算→
        # continue"无退出条件，显存被他方进程长期占用时会形成打印风暴式活锁
        # （流水线路径靠 join 超时兜底，CPU 回退路径可真挂死）。
        _bs1_oom_streak = 0
        _BS1_OOM_LIMIT = 3
        ...
        while True:
            try:
                ...
                # [P1-FIX-WATCHDOG] 通知流水线 Writer：T2 正在 OOM 级联恢复
                # （深度清理+降批重试可能远超其 120s 空转阈值），宽限 5 分钟，
                # 防止看门狗误杀健康分段。时间戳自动过期，无需显式清除。
                try:
                    import time as _t_oom
                    self._oom_pause_until = _t_oom.time() + 300.0
                except Exception:
                    pass
                torch.cuda.empty_cache()
                self._pool.clear()
                ...
                if self.batch_size <= 1:
                    ...
                    # [P1-FIX-BS1-CIRCUIT] 连续多轮 bs=1 深度清理后仍 OOM → 判定
                    # 显存被外部进程长期占用，fail-fast 而非无限重试。
                    _bs1_oom_streak += 1
                    if _bs1_oom_streak > _BS1_OOM_LIMIT:
                        try:
                            _free, _total = torch.cuda.mem_get_info(self.device)
                            _diag = (f"free={_free/1024**3:.2f}GiB/"
                                     f"total={_total/1024**3:.2f}GiB") \
                                    if torch.cuda.is_available() else "n/a"
                        except Exception:
                            _diag = "n/a"
                        raise RuntimeError(
                            f'[OOM] batch_size=1 连续 {_BS1_OOM_LIMIT} 轮深度清理后仍显存不足 '
                            f'({_megapixels_oom:.1f}MP, {_diag})；'
                            f'可能存在其他进程长期占用显存，段处理失败')
                    print(f'\n[OOM] batch_size=1 仍 OOM ({_megapixels_oom:.1f}MP，'
                          f'第{_bs1_oom_streak}/{_BS1_OOM_LIMIT}次)，深度清理后按剩余显存估算恢复...')
    ...
    def _get_or_create_result_pool(self, pool_size: int, max_BT: int,
        ...
        # [P1-FIX-CACHE-ROLLBACK] 先置空缓存引用再构造：构造中途抛异常（锁页内存
        # 分配失败等）时不会留下"已 free 的旧对象+旧 key"的悬空缓存——原实现下
        # 后续段若 key 恰好匹配会命中已释放的池（acquire 永超时被误诊 T3 卡死）。
    ...
    def _get_or_create_ring_buf(self, num_slots: int, max_frames_per_slot: int,
        ...
        # [P1-FIX-CACHE-ROLLBACK] 同 result_pool：先失效缓存再构造，防悬空命中。
    ...
    def _process_segment(
        ...
        effective_bs = self._decide_effective_batch(W, H, bytes_per_frame)  # [P3.1-SPLIT]
        ...
        # [P3.1-SPLIT] scale 合法性 + timesteps 构建；None = 非法 scale（调用方收尾判败）
        _scale_res = self._resolve_scale_timesteps(scale, fps, worker_label)
        if _scale_res is None:
            reader.close()
            return False, 0, 0
        scale_frac, n_interp, timesteps, new_fps = _scale_res

        # [P3.1-SPLIT] 编码器选择（best_encoder/lossless 升级路径）
        ...
        # ── 读取第一帧 ────────────────────────────────────────────────────────
        # [P1-FIX-F0-PROTECT] 首帧读取与 f0 编码写入区此前无异常保护：该区间
        # 抛出（损坏文件/ffprobe 失败/编码器异常等）会跳过 writer/reader.close()，
        # 遗留两个 ffmpeg 僵尸子进程直至解释器退出。现统一捕获并清理后判败。
        try:
            pair = reader.read()
            if pair is None:
            ...
            import traceback as _tb_f0
            print(f'[{worker_label}] ❌ 首帧读取/编码失败: '
                  f'{type(_f0_err).__name__}: {_f0_err}', flush=True)
            _tb_f0.print_exc()
            try:
                writer.close()
            except Exception:
        ...
        if self.device.type == 'cuda':
            pipeline = IFRNetPipelineRunner(
                ...
                auto_tune    = True,
                codec        = use_codec,
                x264_preset  = self.encode_preset,
                crf          = self.crf,
    ...
    def process_video(
        ...
        ok, fc, oc = False, 0, 0
        # [P1-FIX-MONITOR-STOP] _process_segment 抛出异常（如 reader 探测失败）时，
        # 原实现 monitor.stop() 永不执行 → 采样线程永续堆积、统计窗口重叠。
        try:
            ...
            if _is_t3:
                ...
                if (not _nvenc_already_active
                    ...
                    _t3_fps_est = _software_encode_fps(
                        ...
                        self.codec, self.encode_preset, self.crf,
                    )
                if _verbose_report:
                    _diag_parts = []
                    if _t3_fps_meas > 0:
                    ...
                    if _nvenc_already_active:
                        _encoder_tip = (
                            ...
                            f'建议：1) 尝试 --encode-preset p1（最快 NVENC preset） '
                        ...
                        _encoder_tip = (
                            '考虑降低编码参数：--encode-preset veryfast --crf 18'
