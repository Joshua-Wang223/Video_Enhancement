...
class PinnedBufferPool:
    ...
    def __init__(self):
        ...
        # [P1-FIX-H2D-EVENT-SYNC] 每个 slot 的最近异步 H2D Event。
        # 覆写 pinned 宿主缓冲前必须等待，否则上一批 non_blocking 拷贝可能仍占用内存。
        self._in_events: List[Optional[torch.cuda.Event]] = []
        self._pending_issue_slot: Optional[int] = None

    @staticmethod
    def _wait_slot_safe(event: Optional[torch.cuda.Event]) -> None:
        """[P1-FIX-H2D-EVENT-SYNC] 等待指定 slot 最近一次 H2D 完成后再覆写。"""
        if event is not None:
            event.synchronize()

    def mark_issued(self, slot: Optional[int] = None,
                    stream: Optional[torch.cuda.Stream] = None) -> None:
        """[P1-FIX-H2D-EVENT-SYNC] 发起 non_blocking H2D 后调用，记录该 slot Event。

        slot 缺省时标记最近一次 get_for_frames() 返回的 slot。
        """
        if slot is None:
            slot = self._pending_issue_slot
        if slot is None or slot < 0 or slot >= len(self._bufs):
            return
        ev = torch.cuda.Event()
        ev.record(stream if stream is not None else torch.cuda.current_stream())
        while len(self._in_events) <= slot:
            self._in_events.append(None)
        self._in_events[slot] = ev
        self._pending_issue_slot = None
    ...
    def get_for_frames(self, frames: List[np.ndarray],
        ...
        # [P1-FIX-H2D-EVENT-SYNC] 复用前必须保护宿主缓冲；正常路径事件早已完成。
        while len(self._in_events) <= slot:
            self._in_events.append(None)
        self._wait_slot_safe(self._in_events[slot])
        ...
        dst.copy_(src_f)
        # 记录待发射 slot；实际 H2D 后由调用方 mark_issued() 精确记录事件。
        self._pending_issue_slot = slot
        return dst
...
def frames_to_tensor(frames, device, stream=None, dtype=torch.float32, slot: int = 0):
    ...
    with ctx:
        out_t = cpu_t.to(device, non_blocking=True, dtype=dtype)
    # [P1-FIX-H2D-EVENT-SYNC] 异步 H2D 发起后记录 slot Event，防止下轮覆写竞态。
    if stream is not None and torch.cuda.is_available():
        pool.mark_issued(slot, stream=stream)
    else:
        pool.mark_issued(slot) if hasattr(pool, "mark_issued") else None
    return out_t
