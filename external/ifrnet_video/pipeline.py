...
class IFRNetPipelineRunner:
            ...
            try:
                if stream_h2d is not None:
                    with torch.cuda.stream(stream_h2d):
                        ...
                        # [P1-FIX-H2D-EVENT-SYNC] H2D 已发起，立刻记录 slot Event。
                        pool.mark_issued(slot_base, stream=stream_h2d)
                        ...
                        pool.mark_issued(slot_base + 1, stream=stream_h2d)
                else:
        ...
        try:
            while self.running or not self.result_queue.empty():
                try:
                    ...
                    # [P1-FIX-WATCHDOG] T2 处于 OOM 级联恢复宽限期内不计时：
                    # 深度清理+降批重试可能远超 120s 空转阈值，避免误杀健康分段。
                    if time.time() < getattr(self.proc, '_oom_pause_until', 0.0):
                        _idle_since = None
                        continue
                    all_empty = (
                ...
                for _item in items:
                    ...
                    if isinstance(_item, tuple) and len(_item) >= 2 and _item[0] == 'GPU_RAW':
                        ...
                        if _nvenc._la_depth > 0:
                            # [P2.3-LA-PINNED-REUSE] 形状键控的 pinned 旋转池：
                            # 原实现每批 torch.empty(pin_memory=True) 触发一对
                            # cudaHostAlloc/Free。安全前提：编码线程按 FIFO 消费，
                            # 在途批 ≤ 队列深+1，故池深取 队列深+2 时被复用缓冲必然
                            # 已脱离在途集合（严格无数据竞争）。
                            # 池深 = 编码队列深 + 2（首批编码线程尚未创建时按
                            # [FIX-LA-QUEUE-DEPTH] 的自动值 16 计）
                            _qdepth = 16
                            _et_qd = getattr(self, '_enc_thread_ref', None)
                            if _et_qd is not None:
                                try:
                                    _qdepth = int(_et_qd._q.maxsize)
                                except Exception:
                                    _qdepth = 16
                            _cache = getattr(self, '_la_pinned_pool', None)
                            if _cache is None:
                                _cache = {}
                                self._la_pinned_pool = _cache
                            _key = tuple(all_nv12.shape)
                            _entry = _cache.get(_key)
                            if _entry is None:
                                _entry = {'bufs': [], 'idx': 0}
                                _cache[_key] = _entry
                            _pos = _entry['idx'] % (_qdepth + 2)
                            try:
                                if _pos >= len(_entry['bufs']):
                                    _entry['bufs'].append(torch.empty(
                                        all_nv12.shape, dtype=all_nv12.dtype,
                                        device='cpu', pin_memory=True))
                                _pinned = _entry['bufs'][_pos]
                    ...
                    if isinstance(_item, tuple) and len(_item) == 2 and _item[0] == 'RING':
                        ...
                        # [P2.3-ZERO-COPY] b"".join 单次拼接替代 bytearray 逐段 extend
                        _parts = []
                        mv_flat = _mv.cast('B')
                        for i in range(_rB):
                            interp_start = i * _rT * frame_sz
                            interp_end   = (i + 1) * _rT * frame_sz
                            _parts.append(mv_flat[interp_start:interp_end])
                            written += _rT
                            # img1_raw: 来自 reader (RGB)；[P2.3] tobytes 本身即拷贝，
                            # 前置 .copy() 为冗余第二次拷贝，已消除
                            if i < len(_img1l):
                                _parts.append(_img1l[i].tobytes())
                                written += 1
                        writer.write_direct(b"".join(_parts))
                    ...
                    if isinstance(_item, _PinnedResultItem):
                        ...
                        for i in range(_item.B):
                            for j in range(_item.T):
                                ...
                                writer.write(fr)
                                written += 1
                            # img1_raw: 来自 reader 每帧独立 np.frombuffer 缓冲，
                            # [P2.3] 无复用风险，去掉逐帧 .copy() 冗余拷贝
    ...
    def run(
        ...
        if (H_pad > 0 and W_pad > 0
            ...
            # [P3-FIX-POOL-INVARIANT] 结果池必须至少覆盖 result_queue 中全部未消费条目。
            # 输入 pinned slot 已由 Event 保护；输出池再提供队列深度+2 的最小余量。
            _pool_size = max(2, self.result_queue.maxsize + 2)
            if _pool_size < self.result_queue.maxsize + 2:
                raise RuntimeError("PinnedResultPool size below queue-depth invariant")
            try:
