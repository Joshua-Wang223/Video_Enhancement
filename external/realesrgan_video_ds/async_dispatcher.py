#!/usr/bin/env python3
"""
Real-ESRGAN Video Enhancement - 异步 GFPGAN 调度器模块
"""
...
class AsyncGFPGANDispatcher:
    """异步 GFPGAN 调度器，将 GFPGAN IPC 从同步调用改为异步提交+后台收集。"""
    ...
    def _submit_blocking(self, crops_list, task_id=None):
        if task_id is None:
        ...
        if (self.shm_buf is not None
            ...
            try:
                ...
                self.subprocess.task_queue.put((task_id, n, _slot), timeout=10.0)
            except Exception:
                self.shm_buf.release_slot(_slot)
                _slot = None
                raise
        else:
            self.subprocess.task_queue.put((task_id, crops_list), timeout=10.0)
    ...
    def submit_validate(self, val_id: int):
        try:
            self.subprocess.task_queue.put(('__validate__', val_id), timeout=5.0)
        except queue.Full:
            pass
