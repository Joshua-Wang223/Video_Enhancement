...
def _sr_tile_forward(upsampler, batch_t):
    """[P2.1-TILE-RESTORE] 批级 tile 平铺前向：使 tile_size/tile_pad/pre_pad 配置
    真实生效（此前活跃管线直接 upsampler.model(batch_t) 完全绕过 tile 能力）。

    算法复刻官方 RealESRGANer.pre_process/tile_process/post_process：
      pre_pad(reflect) → mod_scale 整除补边 → tile 切块(tile_pad 重叠) 逐块推理
      → 按输出坐标回贴(裁掉 pad 区) → 逆序裁掉 mod/pre pad。
    与逐帧调用官方 enhance() 的区别：本函数直接作用于已在 GPU 上的 NCHW 批张量，
    保留批量推理与异步 D2H/defer_resolve 架构——enhance() 是单帧 numpy API，
    逐帧调用会引入每帧同步 D2H+H2D 回环并把批退化成 B=1。

    仅 eager/compile 路径使用；TRT 静态形状 engine 与动态 tile 数不兼容。
    """
    import math as _math
    model = upsampler.model
    scale = int(upsampler.scale)
    x = batch_t
    B, C, H, W = x.shape

    pre_pad = int(getattr(upsampler, 'pre_pad', 0) or 0)
    if pre_pad != 0:
        x = F.pad(x, (0, pre_pad, 0, pre_pad), mode='reflect')
    ...
    _, _, Hp, Wp = x.shape
    tile = int(upsampler.tile_size)
    tp = int(getattr(upsampler, 'tile_pad', 10) or 10)
    out = x.new_zeros((x.shape[0], C, Hp * scale, Wp * scale))
    tiles_x = _math.ceil(Wp / tile)
    tiles_y = _math.ceil(Hp / tile)

    for ty in range(tiles_y):
        for tx in range(tiles_x):
            ofs_x, ofs_y = tx * tile, ty * tile
            in_sx, in_ex = ofs_x, min(ofs_x + tile, Wp)
            in_sy, in_ey = ofs_y, min(ofs_y + tile, Hp)
            px0, px1 = max(in_sx - tp, 0), min(in_ex + tp, Wp)
            py0, py1 = max(in_sy - tp, 0), min(in_ey + tp, Hp)
            in_tile = x[:, :, py0:py1, px0:px1]
            # [P2.1-TILE-RESTORE] 不吞错：官方实现 except 后继续会用未定义变量，
            # 这里让 OOM 上抛给调用方的 OOM 阶梯处理。
            o_tile = model(in_tile)
            osy = (in_sy - py0) * scale
            osx = (in_sx - px0) * scale
            oh = (in_ey - in_sy) * scale
            ow = (in_ex - in_sx) * scale
            out[:, :, in_sy * scale:in_sy * scale + oh,
                in_sx * scale:in_sx * scale + ow] = \
                o_tile[:, :, osy:osy + oh, osx:osx + ow]

    if mod_scale is not None:
        _, _, h_c, w_c = out.shape
        out = out[:, :, 0:h_c - mod_pad_h * scale, 0:w_c - mod_pad_w * scale]
    if pre_pad != 0:
        _, _, h_c, w_c = out.shape
        out = out[:, :, 0:h_c - pre_pad * scale, 0:w_c - pre_pad * scale]
    return out
...
def _sr_infer_batch(
    ...
    if trt_accel is not None and trt_accel.available:
        ...
        # [P2.2-COMPILE-IMPL] 原 cuda_graph_accel 分支已摘除：该对象从未被任何
        # 调用方创建（恒 None），属死代码。CUDA Graph 能力现由 main.py 在
        # create_video_enhancer 阶段以 torch.compile(mode='reduce-overhead') 实现，
        # 编译后的 callable 仍从 upsampler.model 进入本分支，无需特殊处理。
        _use_tile = int(getattr(upsampler, 'tile_size', 0) or 0) > 0
        if transfer_stream is not None and compute_stream is not None:
            compute_stream.wait_stream(transfer_stream)
            with torch.cuda.stream(compute_stream):
                with torch.no_grad():
                    output_t = (_sr_tile_forward(upsampler, batch_t)
                                if _use_tile else upsampler.model(batch_t))
        else:
            with torch.no_grad():
                output_t = (_sr_tile_forward(upsampler, batch_t)
                            if _use_tile else upsampler.model(batch_t))
