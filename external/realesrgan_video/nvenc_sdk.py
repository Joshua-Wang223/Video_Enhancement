...
class NVENCEncoder:
    ...
    def _apply_drained_entries(self, drained: list, chunk_start_global: int,
        ...
        """
        for _est_fi, _h264_data in drained:
    ...
    def _ensure_slot_free(self, slot_idx: int, chunk_start_global: int,
        ...
        while self._slot_pending.get(slot_idx):
            ...
            if not _drained:
                ...
                # 真触发这个上限说明编码器状态已经不正常，交给上层报错。
                _guard += 1
                if _guard > self._slot_count * 4:
                    print(f'[NVENC-Enc] ⚠️⚠️ _ensure_slot_free 等待 slot={slot_idx} '
                          f'排空超过预期轮次仍未完成，可能存在编码器状态异常，'
                          f'放弃等待以避免死锁（可能导致该 slot 数据被覆盖丢失）',
                          flush=True)
                    break
                continue
            ...
            with self._lock:
                ...
                for fi in range(n_frames):
                    ...
                    if _ep_status == NV_ENC_ERR_NEED_MORE_INPUT:
                        ...
                        self._slots_warmed.add(slot_idx)
    ...
    def _stream_begin(self, force: bool = False):
        ...
        重置跨 chunk 持久的 per-segment 状态：slot 未决表、per-slot IDR 热身表、
        跨块延迟输出缓存。对齐 IFRNet v6.4.5.1 的同名方法。
