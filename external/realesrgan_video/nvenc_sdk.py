...
class NVENCEncoder:
        ...
        try:
            with self._lock:
                ...
                if send_eos:
                    ...
                    # 的 LockBitstream 永久阻塞（test4 实测），EOS 后 pending 帧
                    ...
                    for _ds in _drain_slots:
                        ...
                        while True:
                            if _hevc_eos and not self._slot_pending.get(_ds):
        ...
        try:
            with self._lock:
                ...
                for _slot_idx in _drain_slots:
                    ...
                    _first_lock = True
                    while True:
                        if _hevc_eos_flush and not self._slot_pending.get(_slot_idx):
                        ...
                        if bitstream_ptr_val:
                            ...
                            if _hevc_eos_flush and \
                                ...
                                _dq = self._slot_pending.get(_slot_idx)
                                if _dq:
                                    _dq.popleft()
                                    if not _dq:
                                        del self._slot_pending[_slot_idx]
    ...
    def _stream_begin(self, force: bool = False):
        ...
        if force:
            ...
            # [FIX-HEVC-COUNTED] 段首对齐输出指针与流基线（对齐 IFRNet）：
    ...
    # [FIX-SKIP-REOPEN] reopen() 已删除。Phase B 验证跨段复用可行后，
...
class _NVENCEncodeThread:
    ...
    def __init__(self, nvenc_encoder, writer, encode_queue_depth: int = 4,
        ...
        if getattr(self._nvenc, '_needs_reopen', False):
            ...
            print(f"[FIX-SKIP-REOPEN] 跨段复用，会话跳过重建 "
    ...
    def _loop(self):
        ...
        while True:
            ...
            if _la_mode:
                ...
                try:
                    # [FIX-SKIP-REOPEN] 跨段复用已验证可行，段首 batch 直接走 CE pipeline
    ...
    def flush_and_join(self, timeout: float = 120.0):
        ...
        # 不再触发会话重建（[FIX-SKIP-REOPEN] 跨段复用已验证）。
...
class NVENCWriter:
    ...
    def __init__(self, nvenc_encoder, args, audio, width, height, output_path, fps,
        ...
        # Create muxer for ES → MP4 muxing（codec 由 encoder 决定）
        # [FIX-CODEC-SUPPORT] 透传 encoder 的 codec，HEVC/AV1 走 -f hevc / -f obu。
        ...
        self._muxer = FFmpegMuxer(
            ...
            audio_src=audio_src,
            quiet=_qf,
            codec=getattr(self._enc, '_codec', 'h264'),
        )
