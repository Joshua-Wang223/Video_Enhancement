...
def create_video_enhancer(args):
    ...
    # 结果存入 args._hw_profile 供 FFmpegWriter.best_encoder() 使用。
...
def run_pipeline_for_video(enhancer, input_video, output_video):
    ...
    # [FIX-NVENC-UNIFIED] 确保 _hw_profile 存在（create_video_enhancer 已设置，兜底保护）
    if not hasattr(args, '_hw_profile') or args._hw_profile is None:
        args._hw_profile = _detect_hw_profile(device)
    ...
    # [FIX-HIGHRES-RC] 高分辨率输出（≥1080p）自动切换 CONSTQP + LA=0：
    # VBR_HQ/QVBR + LA=8 在高分辨率下，NVENC lookahead 缓冲的 drain 会阻塞
    ...
    if out_h >= 1080:
        ...
        if _cur_rate != 'constqp' or _cur_la != 0:
            print(f'[FIX-HIGHRES-RC] 输出 {out_w}x{out_h} ≥1080p：'
    ...
    if _nvenc_available and 'nvenc' in args.video_codec:
        try:
