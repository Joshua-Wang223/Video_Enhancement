"""
RealESRGAN 视频超分处理脚本 —— 终极优化版 v6.1（多卡版）
==========================================================
                        ...
                        try:
                            ...
                            if is_oom:
                                ...
                                if bs <= 1:
                                    ...
                                    bs = max(1, bs // 2)
                                    print(f'\n[Worker {worker_idx}] SR OOM，降级 bs → {bs}')
                            else:
                                print(f'\n[Worker {worker_idx}] SR Error: {e}')
                ...
                try:
                    ...
                    if is_oom:
                        ...
                        if bs > 1:
                            ...
                            print(f'\n[Worker {worker_idx}] batch_size=1 OOM，深度清理...')
                            torch.cuda.synchronize()
                            torch.cuda.empty_cache()
                            try:
                                torch._dynamo.reset()
...
def run(args):
    ...
    if num_workers > 1 and args.input_type_is_video:
        print(f'\n[V6] 多 GPU 模式：{num_gpus} GPU × {nw_per_g} Worker = {num_workers} 总 Worker')
