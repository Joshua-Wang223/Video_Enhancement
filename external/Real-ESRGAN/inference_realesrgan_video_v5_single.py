...
==========================================================
⚠️  历史版本说明：此为 v5++(v5_single) 脚本，当前推荐使用
    inference_realesrgan_video_v6_single.py（v6），v6 在本脚本基础上修复了
    face_enhance 的关键 bug 并新增 CPU-GPU 流水线并行。
    非 face_enhance 场景下 v5++ 与 v6 行为一致，可继续使用本脚本。

基于 Real-ESRGAN 的视频超分脚本，面向单 GPU 生产环境的高性能实现。

【最终功能特性】
  推理加速：
    · FP16 半精度推理（默认开启，--fp32 可禁用）
    · torch.compile 可选加速（--use_compile, mode=reduce-overhead）
    · TensorRT 可选加速（--use_tensorrt；TRT 8.x / 10.x 双 API 兼容）
  ...
  I/O 加速：
    ...
    · 异步帧预取（后台线程 + PinnedBufferPool，默认 prefetch=16）
    · 批量推理（默认 batch_size=8）/ 异步 D2H / 批量写帧

  face_enhance 路径（v5++ 改进，v6 进一步修复）：
    · 批量 GFPGAN 推理：所有帧的 face crops 堆叠为单次前向（v5++）
    · 原始帧检测：在低分辨率帧上运行 RetinaFace，比 SR 帧快 4×（v5++）
    · 无人脸帧跳过：检测为空时直接跳过 GFPGAN（v5++）
    · GFPGAN FP16：torch.autocast 包裹（v5++）
    · ⚠️  注意：v5++ 中 face_enhance 输出尺寸存在边缘情况 bug，v6 已完全修复
    · ⚠️  注意：v5++ 中流水线串行（无 CPU-GPU 并行），GPU 利用率 <30%，v6 已改进

【命令行使用示例】
  # 基础超分（2× 放大，FP16 + NVDEC/NVENC 自动启用）
  python inference_realesrgan_video_v5_single.py \\
  ...
  # 开启 face_enhance（推荐改用 v6 以获得更好效果）
  python inference_realesrgan_video_v5_single.py \\
      -i input.mp4 -o results/ -s 2 --face_enhance
...
【关键参数说明】
  ...
  --face_enhance       启用 GFPGAN 人脸增强（建议使用 v6 脚本）
...
【保留全部优化标记（供代码内部定位）】
  FIX-TRT10（TRT 10.x API 兼容）
  FIX-1（cudnn.benchmark）、FIX-2（异步D2H）、FIX-3（TRT全程GPU内存）
  FIX-4（去掉重复copy）、FIX-5（batch/prefetch提升）
  FIX-6（face_enhance SR批处理解耦）、FIX-TRT-STREAM（TRT专用流）
  FIX-7（原始帧检测）、FIX-8（GFPGAN FP16）
  M2（NVDEC/NVENC）、M3（TRT）
  X1-X5、B1-B7、N1-N9（各类内部优化标记）
"""
