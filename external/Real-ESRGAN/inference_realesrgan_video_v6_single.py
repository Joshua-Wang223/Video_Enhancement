...
def _process_batch(
    ...
    if trt_accel is not None and trt_accel.available:
        ...
        # [BUG-A FIX] CUDA Graph 路径：replay 预录图，消除 CPU kernel dispatch 开销
        if transfer_stream is not None and compute_stream is not None:
            compute_stream.wait_stream(transfer_stream)
    ...
    while i < len(frames):
        ...
        try:
            ...
            outputs, gfp_bs = _process_batch(
                ...
                face_enhancer, face_fp16, gfpgan_weight, gfp_bs,  # [FIX-6/8/ADAPT] 透传
            )
            ...
            if not in_oom_cascade:
                ...
                in_oom_cascade = True
            if bs <= 1:
                tqdm.write('[OOM] batch_size=1 仍 OOM，深度清理后估算恢复...')
            ...
            if 'out of memory' in err_str:
                ...
                if not in_oom_cascade:
                    ...
                    in_oom_cascade = True
                if bs <= 1:
                    # [BUG-D FIX] RuntimeError OOM bs=1 深度清理，与 OutOfMemoryError 分支对齐
                    ...
                    recovered_bs = _estimate_safe_batch_size(_H, _W, max_bs[0])
                    if recovered_bs < max_bs[0]:
                        tqdm.write(f'[OOM] 估算安全 batch_size={recovered_bs}，'
...
def inference_video_single(args, video_save_path: str, device=None):
    ...
    # [BUG-B FIX] programmatic 调用兼容：use_compile/use_cuda_graph 默认 True，与 main() 一致
            ...
            while True:
                ...
                if (len(batch_frames) == bs) or (end and batch_frames):
                    ...
                    try:
                        ...
                        # 当前批 OOM/Error，废弃 detect future
