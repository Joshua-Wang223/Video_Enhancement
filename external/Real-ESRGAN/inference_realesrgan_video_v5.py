"""
RealESRGAN 视频超分处理脚本 —— 终极优化版 v5++（多卡版）
==========================================================
在 v5 多卡版基础上，全面移植 v5++ 单卡版的所有升级和修复，
彻底消除已知问题，充分释放多 GPU 强大算力。

[V5++ 新增升级（从单卡版移植）]

  FIX-1.  [cuDNN benchmark] 主进程 + 每个 Worker 进程均启用
          torch.backends.cudnn.benchmark = True，自动为固定输入尺寸选择最优卷积算法。
          多卡场景收益翻倍：N 个 Worker 各自独立收益，不相互干扰。

  FIX-2.  [异步 D2H] PinnedBufferPool 新增 _out_buf + get_output_buf()。
          output_t → pinned buffer.copy_(non_blocking=True) → numpy，
          彻底消除 .cpu() 触发的隐式 cudaDeviceSynchronize，
          每个 Worker 独立受益，多卡总 D2H 带宽利用率显著提升。

  FIX-3.  [TRT 全程 GPU 内存] TensorRT infer() 改用 Tensor.data_ptr() 直接传 GPU 指针，
          完全消除 GPU→CPU→GPU 来回搬运，是原版 pycuda 路径的 2-3x 加速。

  FIX-4.  [去掉双重 copy] _read_loop 移除 arr.copy()，
          PinnedBufferPool.get_for_frames 内的 np.stack 完整复制，无需预先 copy。

  FIX-5.  [batch/prefetch 提升] batch_size 默认 4→8，prefetch_factor 默认 8→16，
          Worker 内批量也随之提升，GPU 利用率更稳定。

  FIX-6.  [face_enhance 解耦] 多卡 Worker 现在完全支持 face_enhance：
          · SR 阶段享受完整 batch/TRT 加速（不再强制 batch=1）
          · GFPGAN 在 Worker 内独立实例化，不跨进程传递不可序列化对象
          · 每个 Worker 维护独立的 gfpgan_bs，OOM 降级值进程内持久化

  FIX-7.  [原始帧检测] GFPGAN 在原始低分辨率帧（如 720p）上运行 RetinaFace，
          比在 SR 帧（1440p）上快 4×，大幅降低 face_enhance 耗时。

  FIX-8.  [GFPGAN FP16] torch.autocast 包裹 GFPGAN 推理，
          充分利用 Tensor Core 加速，多卡环境下每 Worker 均享受。

  FIX-STREAM. [流间数据竞争修复] compute_stream 推理完成后，
          显式让 default stream 等待（GPU 侧事件屏障，不阻塞 CPU），
          彻底消除画面抖动 / 黑影 / 花屏根因。

  FIX-TRT3.  [TRT 8.x / 10.x 双 API 兼容]
          · 移除 pycuda.autoinit（与 PyTorch CUDA context 冲突的根因）
          · execute_async_v3（TRT 10.x）/ execute_async_v2（TRT 8.x）自动切换
          · 专用非默认 Stream（_trt_stream），消除 TRT 隐式全局同步
          · parse_from_file 替代 parse(bytes)，支持大模型 .onnx.data sidecar
          · Engine None 检测 + 自动删除过期缓存 + 重新构建，消除静默失败

  FIX-NVDEC. [NVDEC 探测增强] 真实 H.264 编码+解码双阶段探测，
          消除 lavfi 测试源在部分 FFmpeg build 中误报"可用"的问题。

  FIX-PATH.  [动态路径] 去掉硬编码 /workspace/Video_Enhancement，
          以脚本所在目录为基准动态计算项目根，跨环境无缝运行。

  FIX-OUTPUT. [灵活输出路径] -o 支持直接指定输出视频文件路径（如 out.mkv），
          不再强制只能传目录，_output_is_file() 格式无关判断。

  FIX-NVML.  [NVML 噪声抑制] stderr _NVMLFilter 过滤 NVML_SUCCESS /
          CUDACachingAllocator 无害断言，多 Worker 并发时日志更清晰。

  FIX-OOM-GFP. [GFPGAN OOM 自动降级] 批量推理 OOM 时对半拆分 sub_bs 重试，
          直至 sub_bs=1（逐张），降级值在 Worker 内跨批次持久化。

  FIX-WRITE.  [FFmpegWriter 尺寸校验] write_frame 自动检测并修正尺寸 / dtype，
          防止 face_enhance 路径下的尺寸异常导致 rawvideo 字节流错位。
...
[V5 原有多卡架构（全部保留）]
  ...
  M3. [TensorRT 可选加速]（已大幅强化，见 FIX-TRT3）
  M4. [有界队列 + heapq 无锁帧重排]
  M5. [跨进程异常传播]

【命令行使用示例】
  # 双卡自动分配（每卡 1 Worker）
  python inference_realesrgan_video_v5.py -i input.mp4 -o results/ -s 2

  # 4× 放大 + 每卡 2 Worker（24G+ 显存）
  python inference_realesrgan_video_v5.py -i input.mp4 -o results/ -s 4 \\
      --num_process_per_gpu 2

  # 开启 face_enhance（多卡并行，每 Worker 独立 GFPGAN）
  python inference_realesrgan_video_v5.py -i input.mp4 -o results/ -s 2 \\
      --face_enhance --gfpgan_batch_size 8
  ...
  # 直接指定输出文件路径
  python inference_realesrgan_video_v5.py -i input.mp4 -o output_4k.mkv -s 2
...
【保留全部优化标记（供代码内部定位）】
  FIX-1~8、FIX-STREAM、FIX-TRT3、FIX-NVDEC、FIX-PATH、FIX-OUTPUT
  FIX-NVML、FIX-OOM-GFP、FIX-WRITE
  M1~M5（多卡架构）、X1~X5、B1~B7、N1~N9
"""
...
class HardwareCapability:
    ...
    def _probe_nvdec() -> bool:
        """[FIX-NVDEC] 真实 H.264 编码+解码双阶段探测，消除 lavfi 误报。"""
        try:
            # 阶段1：用 FFmpeg 软编码生成一段真实 H.264 流
            enc_cmd = [
                'ffmpeg', '-f', 'lavfi',
            ...
            if enc.returncode != 0 or not enc.stdout:
                return False
            # 阶段2：用 NVDEC 解码该 H.264 流，验证真实 GPU 解码路径
            dec_cmd = [
                'ffmpeg', '-hwaccel', 'cuda',
                '-f', 'h264', '-i', 'pipe:0',
                ...
                while i_face < n_faces:
                    ...
                    try:
                        ...
                        if 'out of memory' in str(_oom_e).lower() and sub_bs > 1:
                            ...
                            print(f'\n[OOM-GFPGAN] 不可恢复错误（sub_bs={sub_bs}），'
...
def _sr_worker_fn(
    ...
    [FIX-6/7/8]  face_enhance 完整支持：Worker 内独立初始化 GFPGANer，
                 原始帧检测 + 批量推理 + FP16，gfpgan_bs 进程内持久化。
    """
    try:
        ...
        bs      = batch_size
        gfp_bs  = gfpgan_batch_size  # [FIX-OOM-GFP] 进程内持久化的 GFPGAN 子批量
        ...
        def flush():
            ...
            try:
                ...
                if 'out of memory' in err_str and bs > 1:
                    ...
                    # [BUGFIX] 无害 NVML/CUDACachingAllocator 断言不影响正确性
                    for idx, frm in zip(local_indices, local_frames):
                        # 帧已经处理完毕，补发空结果（此处不应到达，但防御性处理）
                        pass
                else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Real-ESRGAN 视频超分 —— 终极优化版 v5++（多卡版）',
    ...
    # 启动前打印硬件状态
    print('=' * 65)
    print('  RealESRGAN 视频超分 —— 终极优化版 v5++（多卡版）')
    print('=' * 65)
    num_gpus = torch.cuda.device_count()
    ...
    print(f'  batch_size: {args.batch_size} | prefetch: {args.prefetch_factor} | '
          f'num_process_per_gpu: {args.num_process_per_gpu}')
    print(f'  face_enhance: {args.face_enhance} '
          f'(model={getattr(args, "gfpgan_model", "1.4")} | '
          f'weight={getattr(args, "gfpgan_weight", 0.5)} | '
          f'GFPGAN-batch={getattr(args, "gfpgan_batch_size", 12)} | '
          f'v5++: 批量GFPGAN+原始帧检测+无脸跳过+Worker独立实例)')
    print(f'  [v5++ 升级] cudnn.benchmark | 异步D2H | TRT 8/10双API | FIX-STREAM | '
          f'批量GFPGAN | Worker独立face_enhance | 动态路径 | 灵活输出路径')
    print()
