"""
RealESRGAN 视频超分处理脚本 —— 终极优化版 v6.1（多卡版）+ 实时预览
====================================================================
...
【命令行使用示例】
  ...
  # 直接指定输出文件路径 + FP32 模式
  python inference_realesrgan_video_v6_1.py -i input.mp4 -o output_4k.mkv -s 2 --no-fp16
  ...
  # 启用预览，每 50 帧刷新一次
  python inference_realesrgan_video_v6_1.py \\
        ...
        if use_pipeline:
            ...
            try:
                while True:
                    ...
                    if (len(local_frames) >= bs) or (sentinel_received and local_frames):
                        ...
                        # [v6.1 OOM-CASCADE] 级联标志（每个 Worker 独立）
                        ...
                        try:
                            sr_results = _sr_infer_batch(
                                ...
                                cuda_graph_accel)
                            _worker_oom_cascade = False  # 推理成功，重置
            ...
            def flush():
                ...
                try:
                    ...
                    if is_oom:
                        torch.cuda.empty_cache()
                        # [v6.1 OOM-CASCADE] 使用闭包级 cascade 标志
                        if not getattr(flush, '_oom_cascade', False):
                            safe_ceiling = max(1, bs - 1)
                            # (无局部 max_bs 引用：直接用 bs 限制恢复上限)
                            flush._oom_cascade = True
                        if bs > 1:
                            bs = max(1, bs // 2)
                            print(f'[Worker {worker_idx}] OOM，降级 batch_size → {bs}')
                        else:
                            # batch_size=1 仍 OOM → 深度清理
                            print(f'[Worker {worker_idx}] batch_size=1 OOM，深度清理...')
                            torch.cuda.synchronize()
                            torch.cuda.empty_cache()
                            try:
                                torch._dynamo.reset()
...
# flush_batch_safe（OOM 级联保护 + 恢复，单卡路径用）
...
def flush_batch_safe(
    ...
    while i < len(frames):
        ...
        try:
            ...
            if bs <= 1:
                tqdm.write(f'[OOM] batch_size=1 仍 OOM，深度清理后按剩余显存估算恢复...')
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                    torch.cuda.empty_cache()
                    try:
            ...
            if 'out of memory' in err_str:
                ...
                if not in_oom_cascade:
                    ...
                    in_oom_cascade = True
                if bs <= 1:
                    # [BUG-C FIX] RuntimeError OOM bs=1 深度清理，与 OutOfMemoryError 分支对齐
...
def inference_video_single(args, video_save_path: str, device=None):
    ...
    # [BUG-B FIX] programmatic 调用兼容守卫：main() 设置了这些值，但 programmatic 调用不经过 main()
    ...
    if getattr(args, 'use_compile', True) and getattr(args, 'use_tensorrt', False):
        args.use_compile = False   # TRT 优先，静默禁用 compile
    ...
    if getattr(args, 'use_cuda_graph', True) and getattr(args, 'use_compile', True):
        args.use_cuda_graph = False  # compile 优先，静默禁用 cuda graph
...
def main():
    parser = argparse.ArgumentParser(
        description='Real-ESRGAN 视频超分 —— 终极优化版 v6.1（多卡版）+ 实时预览',
    ...
    # 启动前打印硬件状态
    print('=' * 65)
    print('  RealESRGAN 视频超分 —— 终极优化版 v6.1（多卡版）+ 实时预览')
    print('=' * 65)
    num_gpus = torch.cuda.device_count()
