...
- [本容器 ffmpeg/ffprobe 的两个环境坑](env-ffmpeg-ffprobe-gotchas.md) — 2026-09-14：① ffmpeg 在后台进程组启动时对 fd0 调 `ioctl(TCSETS)` 被 `SIGTTOU` 停住（秒卡 + 0% CPU + 无输出）；已落地永久修复 `stdin_hardening.detach_background_stdin()`（入口 + 后端 ffmpeg_io 模块导入时接入；只加固读帧器 Popen 不够，`HardwareCapability._probe_nvdec` 自己也会拉 ffmpeg）；② 本 build 的 ffprobe 无 `-hwaccel` 选项（只有 `-hwaccel_flags`），传了则 rc=1；**2026-09-16 补**：门禁进程自身也曾整组被 SIGTTOU 停住（`[FIX-STDIN-TTOU-GATE]`，`plan_implementation_gate.py` 的 R7 NVENC 探测零输出挂死，`ps` 见主进程与 ffmpeg 子进程**双双为 `T`**）——修复为 `run_cmd()` 加 `stdin=subprocess.DEVNULL` + `main()` 入口补加固（两层）；判别口诀「跑得异常久 + 完全无输出」先看 `ps -o stat` 的 `T` 与 `wchan=do_signal_stop`；`Accessory/` 下仍有 28 文件/102 处未显式传 `stdin=`（测试基建债，宜抽共享 helper）
...
- [分析类请求先给「综合分析 + 方案」，等确认再落地](feedback_analysis_plan_before_implement.md) — 2026-09-23 用户明确「提醒：先综合分析后给出方案」；分析回答要给现状取证→路线对比→推荐→显式询问，别夹带未确认的代码改动
- [内容依赖的测试假阳性：保严格判据 + 标注](feedback_keep_strict_criteria_annotate.md) — 出现内容相关假阳性时保留判据不变并标注"合成素材已知边界/真实素材已 PASS"，不为消警而放宽阈值或改产品参数
- [verify_plan 门禁基线与断言写法](gate-verify-plan-known-failures.md) — 2026-09-15 全量基线 94/88/0（旧「93/84/7」作废）；2026-09-23 静态子集 49/47/0/2 → **2026-09-24 50/48/0/2（新增 `[FIX-PRESCAN-RECEIVE]`，含负向校验）** + **Linux/GPU 全量套件基线：门禁 95/93/0/2、行为别名 46/46、隔离跑 6 PASS/0 FAIL/0 CRASH/16 EMPTY（EMPTY=独立脚本非故障）**；含覆盖自动化(BEH-E2)、H1/H3 环境降级、以及「新断言必须用 AST/结构切片，禁止匹配注释文案」的实证教训
...
- [门禁覆盖扩到 Accessory/（54→112 文件）](gate-tests-coverage-automation.md) — 2026-09-15：`COVERAGE_ROOTS` 加 `"tests"`，`BEH-H2` 立刻抓到 `nvenc_rc_mode_diagnose.py:178` 的 `capture_output` 与 `stderr=DEVNULL` 互斥（被 `except` 吞掉多年、py_compile 看不见）；清点表 `Accessory/docs/TEST_ASSET_INVENTORY.md`（59 文件 + v4/v5 反例 + 换行符以 Linux 侧为准）
...
- [NVENC 硬件测试隔离](nvenc-hardware-test-isolation.md) — 2026-09-15：方案 A 就绪（`Accessory/run_all_isolated.sh` 逐文件独立进程 + PASS/FAIL/CRASH/EMPTY 五态；`pytest.ini` 只注册 `hw` 标记**不设默认排除**）；SIGSEGV 复现与二分定位、方案 B/C 仍需 Linux+GPU；与读帧器/解码链路**无耦合**
- [NVENC qp=0（CRF=0 无损）段错误 —— 生产规避](nvenc-qp0-crash-workaround.md) — 2026-09-16 T4/CUDA13 实测：`constqp + qp=0 + la=0` 约 35–67% 概率 SIGSEGV（同配置仅把 qp 改 23 → 0/10 崩，唯一触发条件就是 `qp=0`）；崩溃在编码阶段、faulthandler 落点游走、gdb 显示堆已损坏；已排除 6 类假设（`_NvEncPresetConfig`/`_NvEncConfig` 尺寸、`_FUNC_IDX` 函数表、`from_address` 越界、`_slot_pending` 形状、close 后释放张量、缓冲区尺寸）→ 定性为**驱动/SDK 缺陷**；规避 = 关 `nvenc._NVENC_CRF0_FORCE_CONSTQP`（改走 CRF 路径，代价是非数学无损）/ 显式 `--nvenc-qp>=1` / 启用 LA>0；复现工具 `Accessory/probe/nvenc_qp0_segv_repro.py`（退出码 1=复现）
...
- [门禁与测试资产纳管清理 完成](门禁与测试资产纳管清理_立项Prompt.md) — 2026-09-17：门禁基线 92 PASS（BEH-H1/H3 WARN/SKIP→PASS），覆盖扩展到 Accessory/（54→112 文件），`BEH-H2` 抓到 `nvenc_rc_mode_diagnose.py` 互斥参数被吞掉的多年隐蔽 bug
...
- [GitHub 推送流程（SSH-over-443）与密钥红线](github-push-workflow-and-secrets.md) — 2026-09-23：仓库本身即普通 git 仓库（可直接 commit+push），`force_push_github.sh` 用于全量覆盖路径（容器 github.com→ssh.github.com:443，无 gh/令牌可走 HTTPS）；**铁律：任何推送之后一律 `git ls-remote origin refs/heads/main` 独立复核**（本地提交成功 ≠ 远程已更新；脚本曾因 pipefail+SIGPIPE 静默中止且退出码被 tee 掩盖，已修 `[FIX-PIPEFAIL-SIGPIPE]`）；**[FIX-IGNORE-CANONICAL] `.gitignore`/`.gitattributes`/`tar_excludes.txt` 一律以 origin 为准**（本地差异只告警，`IGNORE_LOCAL=1` 可绕过；改这三个文件必须先推送才生效；勿把脚本自身纳入该清单）；**快照已改用 `bash make_snapshot.sh`（git archive，只含已跟踪文件）**，`tar_excludes.txt` 已 DEPRECATED（其 `./` 锚定语法仅作历史参考：`/models` 与裸 `models` 均错）；config/cc-switch-*.md 四文件含真实 Key 已 gitignore（GitHub 只认 OpenRouter，DeepSeek 无检测器）；**读日志判定：0 报错 ≠ 推送成功，真成功=有第6节+2 行 `-> main`+0 行 rejected；⚠️ models.del/.bak 类改名残留同时绕过 .gitignore 与大目录哨兵（128MB 会进树且哨兵仍打 ✅），清理时别停在改名**；**2026-09-24 新增「Windows 开发机推送路径」一节**——本机 remote 已由内嵌失效 token 的 HTTPS 改为 `git@github.com` SSH（本机原无任何私钥，新生成 `~/.ssh/id_ed25519` 并注册；那个 leaked token 应撤销），并记录本机能力边界：无 torch/pytest → 凡 `import ifrnet_video.pipeline` 的测试（含 `test_prescan_cache_persistence`）只能在 Linux+GPU 跑
...
- [P3-2/PROBE-OPT 预扫描缓存断点恢复](probe-scene-cut-cache-persistence.md) — 2026-09-23 已落地路线 B：两缓存落盘 checkpoint 同目录 sidecar（probe_cache.json/scene_cuts.json）跨进程命中；三陷阱已规避（失败不固化/metadata 不落盘/阈值+判据版本入 key）；**process 模式修复 `[PROBE-CACHE-PERSIST-PROC]`**（fork 子进程各自覆盖写→最后写者胜只剩 1 条，改 flock 读-合并-写 + pid tmp 后累积 8/8）；test 18/18（含 process E2E + 负向校验）+ 门禁 49/47/0/2 → **2026-09-24 50/48/0/2（新增静态断言 `[FIX-PRESCAN-RECEIVE]`，已做负向校验）**；**收段入口补齐 `[P3-2-RECEIVE]`/`[PROBE-OPT-RECEIVE]`**——`process_segments_directly` 原先无预扫描（sidecar 配了无人用）；本轮假象真根因 = 用户跑的是 `--skip-interpolate`，看到的分割日志其实是 Real-ESRGAN 阶段（`🔪 分割视频...`/`✅ 共 N 个片段` 两侧文案一字不差，看下一行是否紧跟 `[P3-2]` 才能分辨）
- [性能探测「最佳点」的三项判据](feedback_probe_best_point_criteria.md) — 2026-09-26：寻优脚本必须同时给 FPS / GPU 平均利用率 / 处理时长÷视频时长，不能只看吞吐
- [测试资产已迁入 Accessory/ 并按用途分类](project_accessory_layout.md) — 2026-09-26：`tests/` 已不存在；找测试/验收脚本去 Accessory/{analyze,benchmark,probe,test,verify,infra,docs,archive}，旧→新映射见 Accessory/README.md
- [三套路径别名（Windows / WSL / 生产）](project_environment.md) — 开发 `D:\Workspace_Python\Video_Enhancement`、WSL `/mnt/d/Workspace_Python/Video_Enhancement`、生产 `/workspace/Video_Enhancement`；脚本禁止硬编码，用 `__file__` 推导项目根
