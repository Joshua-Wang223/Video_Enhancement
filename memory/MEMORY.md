...
- [GitHub 推送流程（SSH-over-443）与密钥红线](github-push-workflow-and-secrets.md) — 2026-09-23：`GH_URL='git@github.com:...' bash force_push_github.sh`（容器 github.com→ssh.github.com:443，无 gh/令牌可走 HTTPS）；**铁律：任何推送之后一律 `git ls-remote origin refs/heads/main` 独立复核**（本地提交成功 ≠ 远程已更新；脚本曾因 pipefail+SIGPIPE 静默中止且退出码被 tee 掩盖，已修 `[FIX-PIPEFAIL-SIGPIPE]`）；config/cc-switch-*.md 四文件含真实 Key 已 gitignore（GitHub 只认 OpenRouter，DeepSeek 无检测器）；**读日志判定：0 报错 ≠ 推送成功（第 1 次 0 命中却未推出），dry-run 行与真实成功行格式相同、须在"预演通过"之后，真成功=有第6节+2 行 `-> main`+0 行 rejected；⚠️ models.del/.bak 类改名残留同时绕过 .gitignore 与大目录哨兵（128MB 会进树且哨兵仍打 ✅），清理时别停在改名**
