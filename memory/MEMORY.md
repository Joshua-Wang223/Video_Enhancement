...
- [P3-2/PROBE-OPT 预扫描缓存断点恢复](probe-scene-cut-cache-persistence.md) — 2026-09-23 已落地路线 B：两缓存落盘 checkpoint 同目录 sidecar（probe_cache.json/scene_cuts.json）跨进程命中；三陷阱已规避（失败不固化/metadata 不落盘/阈值+判据版本入 key）；test 14/14 + 双进程 E2E + 门禁 49/47/0/2
