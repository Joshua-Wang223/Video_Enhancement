...
description: 全量 pytest tests/ SIGSEGV 的隔离处置：方案 A 逐文件独立进程入口已就绪（run_all_isolated.sh + pytest.ini 的 hw 标记），崩溃定位与方案 B/C 需 Linux+GPU
type: project
---
...
全量 `pytest tests/` → `.........`（9 passed）后 **EXIT=139 (SIGSEGV)**；
...
| 交付 | 说明 |
|---|---|
| `tests/run_all_isolated.sh` | **方案 A**：每个测试文件单独起一个 pytest 进程，任一崩溃不影响其余；汇总 PASS / FAIL / **CRASH** / EMPTY / OTHER 五态；支持 `--timeout` / `--file` / 透传 `-m` |
...
2. 二分定位**最小两类组合**：`pytest tests/A.py tests/B.py -x --tb=short`
...
4. `bash tests/run_all_isolated.sh` 全量跑，确认**不再产生 core dump**
