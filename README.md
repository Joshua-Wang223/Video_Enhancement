...
> **当前版本**: v2.0.0（优化版）｜IFRNet 后端 **v6.4.5.1**（NVENC SDK Level 1 GPU 直通 + CE-Pipeline 异步编码）/ Real-ESRGAN 后端 v6.4（深度模块化架构 + 独立优化版）  
> **最后更新**: 2026-06-15
...
graph TD
    ...
    %% ENTRY LAYER
    %% ===================================================================
    MAIN["📋 main_video_optimized.py<br/>VideoProcessor<br/>CLI 入口 · 流程编排"]
    CONFIG_JSON["config/default_config.json<br/>★ 默认真源"]

    %% ===================================================================
    %% CONFIG & UTILS LAYER
    %% ===================================================================
    CONFIG_MGR["⚙️ config_manager.py<br/>Config 类<br/>加载 · 合并 · 路径派生 · CLI 覆盖"]
    VIDEO_UTILS["🔧 video_utils.py<br/>VideoInfo · split_video_by_time<br/>merge_videos_by_codec · smart_extract_audio<br/>verify_video_integrity"]
    ...
    %% ===================================================================
    subgraph IFRNET_BACKEND["external/IFRNet/ — IFRNet 后端 v6.4.5.1"]
        IFRNET_CORE["process_video_v6_4_5_1_single.py<br/>IFRNetVideoProcessor<br/>NVENC SDK Level 1 GPU 直通"]
        T1["T1: FFmpegFrameReader<br/>NVDEC 解码 · 帧预取"]
        T2["T2: GPU 推理<br/>IFRNet 模型 · FP16 / compile / TRT"]
        T3["T3: _NVENCEncodeThread<br/>CE-Pipeline 异步 · FFmpegMuxer"]
        ...
        IFRNET_CORE --> NVENC_DIRECT
        IFRNET_CORE --> GPU_MON
    end

    %% ===================================================================
    %% Real-ESRGAN BACKEND (v6.4)
    %% ===================================================================
    subgraph ESRGAN_BACKEND["external/realesrgan_video/ — Real-ESRGAN 后端 v6.4"]
        ESRGAN_MAIN["main.py<br/>create_video_enhancer()<br/>run_pipeline_for_video()<br/>GPU 硬件型号探测 (20+ GPU)"]
        PIPELINE["pipeline.py<br/>DeepPipelineOptimizer<br/>4 级并行流水线"]
        R1["读帧: FFmpegReader<br/>NVDEC + 异步预取"]
        R2["SR: _sr_infer_batch<br/>Real-ESRGAN · tile · FP16"]
        R3["GFPGAN: AsyncGFPGANDispatcher<br/>人脸检测 · 增强 · 融合"]
        R4["写帧: FFmpegWriter<br/>NVENC 编码"]
        FFMPEG_IO["ffmpeg_io.py<br/>FFmpegReader · FFmpegWriter<br/>HardwareCapability"]
        TRT_ACCEL["tensorrt_accel.py<br/>TensorRTAccelerator<br/>SR TRT Engine 构建/推理"]
        GFPGAN_SUB["gfpgan_subprocess.py<br/>GFPGANSubprocess<br/>TRT 子进程 · 共享内存双缓冲"]
        FACE_UTILS["face_utils.py<br/>人脸检测 · 贴回融合<br/>_detect_faces_batch · _paste_faces_batch"]
        ESRGAN_CONFIG["config.py<br/>MODEL_CONFIG<br/>模型路径 · 下载 URL"]
        ASYNC_DISP["async_dispatcher.py<br/>AsyncGFPGANDispatcher<br/>CPU-GPU 异步调度"]

        ESRGAN_MAIN --> PIPELINE
        ESRGAN_MAIN --> FFMPEG_IO
        ESRGAN_MAIN --> TRT_ACCEL
        ESRGAN_MAIN --> GFPGAN_SUB
        ESRGAN_MAIN --> FACE_UTILS
        ESRGAN_MAIN --> ESRGAN_CONFIG
    ...
    %% MODEL WEIGHTS
    %% ===================================================================
    subgraph MODELS["模型权重"]
        M_IFRNET["models_IFRNet/<br/>IFRNet_S/L_Vimeo90K.pth"]
        M_ESRGAN["models_RealESRGAN/<br/>realesr-general-x4v3.pth<br/>RealESRGAN_x4plus.pth 等"]
        M_GFPGAN["models_GFPGAN/<br/>GFPGANv1.4.pth<br/>facexlib 辅助模型"]
    end

    %% ===================================================================
    %% SHARED INFRASTRUCTURE
    %% ===================================================================
    TRT_CACHE[".trt_cache/<br/>TRT Engine 统一缓存<br/>IFRNet & ESRGAN 共享"]
    CHECKPOINT["temp/ 断点文件<br/>checkpoint.json<br/>分段级别断点续传"]

    %% ===================================================================
    %% CONNECTIONS — Entry → Config/Utils
    %% ===================================================================
    MAIN --> CONFIG_MGR
    MAIN --> VIDEO_UTILS
    ...
    %% ===================================================================
    %% CONNECTIONS — Entry → Processors
    %% ===================================================================
    MAIN --> IFRNET_PROC
    MAIN --> ESRGAN_PROC

    %% ===================================================================
    %% CONNECTIONS — Processors → Backends
    %% ===================================================================
    IFRNET_PROC --> IFRNET_CORE
    ESRGAN_PROC --> ESRGAN_MAIN

    %% ===================================================================
    %% CONNECTIONS — Shared Infrastructure
    %% ===================================================================
    IFRNET_PROC -.-> CHECKPOINT
    ESRGAN_PROC -.-> CHECKPOINT
    IFRNET_CORE -.-> TRT_CACHE
    ESRGAN_MAIN -.-> TRT_CACHE
    IFRNET_CORE -.-> M_IFRNET
    ESRGAN_MAIN -.-> M_ESRGAN
    ESRGAN_MAIN -.-> M_GFPGAN

    %% ===================================================================
    %% CONNECTIONS — Utils used by Processors
    %% ===================================================================
    IFRNET_PROC -.-> VIDEO_UTILS
    ESRGAN_PROC -.-> VIDEO_UTILS

    %% ===================================================================
    %% DATA FLOW (thick arrows)
    %% ===================================================================
    INPUT[("📹 输入视频")] ==> MAIN
    MAIN ==>|"模式 A: interpolate_then_upscale"| IFRNET_PROC
    IFRNET_PROC ==>|"插帧分段"| ESRGAN_PROC
    ESRGAN_PROC ==>|"超分分段"| OUTPUT[("📤 输出视频")]

    MAIN ==>|"模式 B: upscale_then_interpolate"| ESRGAN_PROC
    ESRGAN_PROC ==>|"超分分段"| IFRNET_PROC
    IFRNET_PROC ==>|"插帧分段"| OUTPUT

    %% ===================================================================
    %% STYLING
    %% ===================================================================
    classDef entry fill:#1a1a2e,stroke:#e94560,stroke-width:3px,color:#eee
    classDef config fill:#16213e,stroke:#0f3460,stroke-width:2px,color:#eee
    classDef processor fill:#0f3460,stroke:#533483,stroke-width:2px,color:#eee
    classDef backend fill:#1a1a2e,stroke:#e94560,stroke-width:2px,color:#eee
    classDef module fill:#16213e,stroke:#533483,stroke-width:1px,color:#eee
    classDef storage fill:#1e1e1e,stroke:#666,stroke-width:1px,color:#aaa
    classDef io fill:#2d2d2d,stroke:#e94560,stroke-width:2px,color:#eee
...
**模式 A — `interpolate_then_upscale`（默认）：**
```
输入视频 → 提取音频 → 分段 → IFRNet 插帧 ──直连分段──→ Real-ESRGAN 超分 → FIX-C 归一化 → 合并 + 音频回写 → 输出
```

**模式 B — `upscale_then_interpolate`：**
```
输入视频 → 提取音频 → 分段 → Real-ESRGAN 超分 ──直连分段──→ IFRNet 插帧 → FIX-C 归一化 → 合并 + 音频回写 → 输出
```

> **直连分段优化：** 两个处理器之间直接传递分段文件列表，跳过中间合并+重新分割步骤，节省约 30% I/O。

### IFRNet 后端内部架构 (v6.4.5.1)
...
| **T2 GPU** | IFRNet 模型推理 | pair_queue → FP16/compile/TRT 推理 → result_queue (GPU tensor) |
| **T3 Writer+Encoder** | `_NVENCEncodeThread` | result_queue → RGB→NV12 GPU kernel → CE-Pipeline 异步编码 → H.264 ES → FFmpegMuxer |
...
- **GPU 监测线程**：2 秒采样，运行结束后输出利用率/显存统计与 batch_size 推荐

### Real-ESRGAN 后端内部架构 (v6.4)

4 级并行流水线 + 多片段复用：

| 阶段 | 组件 | 功能 |
|------|------|------|
| **读帧** | `FFmpegReader` | NVDEC 解码 + 异步预取队列 (prefetch_factor) |
| **SR 推理** | `_sr_infer_batch` | Real-ESRGAN 超分 + tile 分块 + H2D 预取重叠 |
| **GFPGAN** | `AsyncGFPGANDispatcher` | 人脸检测 → GFPGAN 增强 → 人脸融合（可选） |
| **写帧** | `FFmpegWriter` | NVENC 编码输出 |

- **`create_video_enhancer()`**：一次性加载模型/TRT Engine/GFPGAN，返回可复用 enhancer
- **`run_pipeline_for_video()`**：使用预建 enhancer 处理单个视频/分段
- **GFPGAN TRT 子进程**：独立进程隔离，共享内存双缓冲，防止主进程 OOM
- **GPU 硬件型号探测**：20+ GPU 型号静态表，自动选择最优 NVENC 编码器
...
│   │
│   ├── processors/
│   │   ├── ifrnet_processor_v6_1_single.py          # ★ IFRNet 处理器 v6.1（对接 v6.4.5.1 后端）
│   │   ├── ifrnet_processor_video_optimized.py      # ★ IFRNet 处理器优化版（对接 v6.4.5.1 后端）
...
├── external/
│   ├── IFRNet/
│   │   ├── process_video_v6_4_5_1_single.py  # ★ IFRNet v6.4.5.1 后端核心（当前对接，NVENC SDK Level 1）
...
```bash
# ── IFRNet 独立插帧（v6.1 处理器，对接 v6.4.5.1 后端）──────────────────────────
