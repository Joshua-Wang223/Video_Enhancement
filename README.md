...
一套完整的 AI 视频增强解决方案，整合了**视频插帧（IFRNet v6.3.3）**与**视频超分辨率（Real-ESRGAN v6.4）**两大模块，并针对单 GPU 环境进行了深度优化。IFRNet 后端在 v6.3.0 双流架构（H2D 预取 / D2H 独立流 + CudaEventPool）之上，v6.3.3 进一步改进段完成后 RETUNE 统计、队列与 PinnedPool 可观测性、软编并行与 NVENC pipe 场景参数等。Real-ESRGAN 侧通过 `realesrgan_video_ds`（深度模块化、4 级并行 + **多片段复用**：模型/TRT 一次初始化、分段复用）与独立优化版 v6.4 并存。
...
> - `output`：最终合并与音频提取（`audio_format: smart`、`audio_codec: copy` 等为默认智能/无损回写策略）。
...
A: `realesrgan_video_ds` 已在入口设置 `PYTORCH_NVML_BASED_CUDA_CHECK=0` 以避免部分驱动与 NVML 不匹配导致的检测失败。若仍遇 NVML 问题，请检查 NVIDIA 驱动与 CUDA 版本是否与 PyTorch wheel 一致。
