...
> **版本**: v2.0.0（优化版）｜IFRNet 后端 **v6.4.5.1**｜Real-ESRGAN 路径 **`realesrgan_video`（v6.4 深度流水线）**  
> **最后更新**: 2026-05-14
...
- **IFRNet（v6.4.5.1 后端）**：光流插帧，帧率提升 2x–16x；NVENC SDK Level 1 GPU 直通 + CE-Pipeline 异步编码、可选 TensorRT / CUDA Graph / `torch.compile`。
...
└────────────────────────────┬────────────────────────────────┘
    ...
    │ video_optimized.py  │       │ optimized.py                  │
    └─────────┬───────────┘       └─────────┬──────────────────────┘
              │                              │
    ┌─────────▼───────────┐       ┌──────────▼─────────────────────┐
    │ IFRNet 后端          │       │ realesrgan_video           │
    │ external/ifrnet_    │       │ external/realesrgan_video/ │
    │ video/main.py       │       │ main.py（main_optimized）      │
    │ IFRNetVideoProcessor │       │   create_video_enhancer()     │
...
| IFRNet 处理器 | `src/processors/ifrnet_processor_video_optimized.py` | 对接 `external/ifrnet_video/main.py` |
| Real-ESRGAN 处理器 | `src/processors/realesrgan_processor_video_optimized.py` | 动态加载并调用 `realesrgan_video/main.py` |
| IFRNet 后端 | `external/ifrnet_video/` | v6.4.5.1 模块化子项目（main / pipeline / nvenc_sdk / ffmpeg_io / tensorrt_accel / config / ifrnet_utils），NVENC SDK Level 1 + CE-Pipeline |
...
| 路径 | 说明 |
|------|------|
| `external/ifrnet_video/` | **当前对接**（v6.4.5.1，模块化子项目；NVENC SDK Level 1 GPU 直通 + CE-Pipeline） |
| `process_video_v6_4_5_1_single.py` | 已逐字拆分至 `external/ifrnet_video/`，原文件保留为参考源 |
...
main_video_optimized.py
  ...
  ├── video_utils
  ├── ifrnet_processor_video_optimized
  │     └── external/ifrnet_video/main.py + pipeline + nvenc_sdk + ffmpeg_io + tensorrt_accel
