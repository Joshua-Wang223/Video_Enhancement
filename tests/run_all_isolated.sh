...
#
# 问题：`pytest tests/` 全量跑会在 NVENC 编码器测试阶段 SIGSEGV（EXIT=139，
...
#
# 用法：
#   bash tests/run_all_isolated.sh                 # 跑全部 test_*.py
#   bash tests/run_all_isolated.sh -m hw           # 只跑标记为硬件测试的
#   bash tests/run_all_isolated.sh -m "not hw"     # 显式排除硬件测试
#   bash tests/run_all_isolated.sh --timeout 300   # 单文件超时（秒，默认 600）
#   bash tests/run_all_isolated.sh --file tests/test_nvenc_la_frame_conservation.py ...
#
...
if [ "${#FILES[@]}" -eq 0 ]; then
    while IFS= read -r f; do FILES+=("$f"); done < <(
        find tests -name 'test_*.py' -not -path '*/__pycache__/*' \
...
if [ "${#FAIL[@]}" -gt 0 ] || [ "${#CRASH[@]}" -gt 0 ] || [ "${#OTHER[@]}" -gt 0 ]; then
    ...
    echo "       下一步用二分法缩小："
    echo "         pytest tests/<A>.py tests/<B>.py -x --tb=short"
