...
v5 FRAME CONSERVATION FIXES (2026-07-01):
  Based on tests/test_nvenc_la_frame_conservation.py and the NVENC Lookahead summary.
...
Usage (on GPU server):
  python test_nvenc_completion_event_v5.py
  python test_nvenc_completion_event_v5.py --rate-modes vbr_hq --la 8 --frames 200
  python test_nvenc_completion_event_v5.py --rate-modes constqp,vbr_hq,qvbr --frames 300
...
class NVENCEncoder:
    ...
    # ── [v5-FIX] Frame conservation methods (from test_nvenc_la_frame_conservation.py) ──
        ...
        try:
            with self._lock:
                ...
                for fi in range(n_frames):
                    ...
                    # 参见 test_nvenc_la_frame_conservation.py MinimalTestEncoder。
...
class NVENCEncoderMode8(NVENCEncoder):
    ...
    来源于 tests/test_nvenc_la_frame_conservation.py 的 MinimalTestEncoder 架构。
    ...
    def encode_frames_sync_drain_loop(self, nv12_tensors, force_idr_first=False):
        ...
        [v5-FIX-FLAT] 返回扁平 H.264 列表（drain 产出顺序），不再 1:1 映射。
        与 test_nvenc_la_frame_conservation.py MinimalTestEncoder 架构完全对齐：
...
TECHNIQUES = {
    ...
    'sync-drain-loop': {
        ...
        'per_frame': False,
        'ce_info': 'sync-drain',
        'desc': 'Sync drain loop — per-frame EncodePicture (no CE) + blocking LockBitstream drain until NEED_MORE_INPUT (reference implementation from test_nvenc_la_frame_conservation.py)',
    },
}
