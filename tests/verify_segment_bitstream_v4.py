...
v5 新增 (资源自适应 + 检查 1+3 策略化/分块并行，本文件原地升级):
  ...
  - 检查 1+3 策略化解码 (--decode-strategy):
      * auto: GPU 路径 → gpu_dual（-f null 无 filter 解码取 frames/packets/
        decode errors + ffprobe -show_frames 逐帧 pts，均不下载像素，消除
        showinfo 强制 D2H 的 GPU 路径瓶颈）；CPU 路径 → showinfo（与 v4 语义
        一致）。
      * showinfo: 原 v4 单次 -vf showinfo 解码（兼容默认）。
      * gpu_dual: 强制 GPU 双进程路径（不可用自动回退 showinfo）。
    ...
    for raw in r.stdout.splitlines():
        ...
        if len(parts) >= 6:
            try:
                ...
                pts_t = pts * tb[0] / tb[1] if tb else None
                pts_rows.append((len(pts_rows), pts, pts_t))
            except ValueError:
                continue
...
def _decode_single_gpu_dual(path):
    ...
    def _show_frames():
        cmd = [ffprobe, '-v', 'error', '-hwaccel', 'cuda',
...
def _decode_chunk_worker(path, start_pts, count, threads):
    """解码单个分块（输入 -ss + -copyts + -frames:v），返回原始 pts 行与错误行。"""
    ffmpeg = shutil.which('ffmpeg')
    if not ffmpeg:
        return {'rc': -1, 'pts_rows': [], 'issues': [], 'err_tail': 'ffmpeg 不可用'}
    cmd = [ffmpeg, '-hide_banner', '-v', 'verbose',
           '-threads', str(threads), '-copyts',
           '-ss', '%.6f' % start_pts, '-i', str(path),
           '-frames:v', str(count), '-an', '-fps_mode', 'passthrough',
           '-vf', 'showinfo', '-f', 'null', '-']
    res = _run_decode_stream(cmd)
    return {'rc': res['rc'], 'pts_rows': res['pts_rows'],
            'issues': res['issues'], 'err_tail': res['err_tail']}


def _check_decode_chunked(path, plan, cpu_count=None):
    """分块并行解码检查 1+3（仅 CPU 软解）；任何校验失败返回 None 由上层回退。
    ...
    with ThreadPoolExecutor(max_workers=n) as ex:
        futs = [ex.submit(_decode_chunk_worker, str(path), s, c + _CHUNK_MARGIN, threads)
                for s, c in chunks]
        for i, f in enumerate(futs):
            try:
                results[i] = f.result()
            except Exception as e:
                results[i] = {'rc': -1, 'pts_rows': [], 'issues': [], 'err_tail': str(e)}
...
def check_decode_integrity(path, hwaccel='off', strategy='auto', chunks=1,
    ...
    # ── 分块路径（仅 CPU 软解 + showinfo 语义；GPU 由 gpu_dual 覆盖）──
    if chunks > 1 and hwaccel == 'off' and strategy in ('auto', 'showinfo'):
        plan = chunk_plan
        if plan is None:
    ...
    # ── 策略化单次解码 ──
    used = strategy
    if strategy == 'framecrc':
        r = _decode_single_framecrc(path, hwaccel)
    elif strategy in ('auto', 'gpu_dual') and hwaccel != 'off':
...
def check_chroma_corruption(path, hwaccel=False, n_shards=1, shard_plan=None,
    ...
    # [v5] 艰度分块并行（仅 CPU 软解路径）: 复用关键帧分块计划，各分块
    # rawvideo 解码 + 逐 ROI std 归约，坏帧索引全局聚合；失败回退单流。
    if n_shards > 1 and not hwaccel:
        plan = shard_plan
        if plan is None:
...
def _verify_one_video(video_path, hwaccel='auto', dump_nal=None, skip_chroma=False,
    ...
    if skip_chroma:
        ...
        # [v5] 色度分块并行仅 CPU 软解路径（NVDEC 路径保持单流以尊重 GPU 闸门）
        shards_eff = chroma_shards if (not chroma_gpu) else 1
        shard_plan = None
        if shards_eff > 1 and chunk_plan is None:
            shard_plan = _build_chunk_plan(video_path, shards_eff)
        chroma_gpu_acquired = False
        if chroma_gpu and gpu_sem is not None:
            try:
                gpu_sem.acquire(timeout=_GPU_WAIT_TIMEOUT)
                chroma_gpu_acquired = True
            except Exception:
                chroma_gpu_acquired = False
            if not chroma_gpu_acquired:
...
def _auto_parallel_counts(cpu_count, ram_avail_gb, n_active_files, gpu_enabled):
    ...
    decode_chunks 仅 CPU 软解路径有效（GPU 路径用 gpu_dual 单次解码）;
    chroma_shards 仅 CPU 色度路径有效。均为 per-file 分块数，
    受 CPU 核数（按活跃文件均分）与可用 RAM（每解码 ~300MB）共同约束。
    """
    ...
    decode_chunks = 1 if gpu_enabled else max(1, min(cap, 8))
...
''',
    ...
    ap.add_argument('--chroma-shards', default='auto', metavar='N|auto',
                    help='检查4 色度分块并行数 (auto=自动; 仅CPU软解路径生效; 1=单流)')
    ap.add_argument('--decode-strategy', choices=['auto', 'showinfo', 'gpu_dual', 'framecrc'],
                    default='auto',
                    help='检查1+3 解码策略: auto(默认; GPU→gpu_dual, CPU→showinfo) | showinfo | gpu_dual | framecrc')
    ...
    print("分块计划: decode-chunks=%d%s | chroma-shards=%d | decode-strategy=%s | 活跃文件=%d%s" % (
        decode_chunks, (' (GPU 路径单次解码)' if gpu_enabled else ''),
